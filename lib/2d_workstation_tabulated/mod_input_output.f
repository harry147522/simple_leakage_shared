...
contains
  ...
  subroutine read_arguments()
    ...
    call kracken('cmd','-i gmunu.par -if ' // undefined // &
  ...
  subroutine read_par_files()
    ...
    namelist /filelist/ base_filename,restart_from_file, typefilelog,&
       firstprocess,reset_grid,snapshotnext, convert,convert_type,usr_filename,&
       nwauxio,nocartesian, w_write,writelevel,writespshift,&
    ...
    namelist /stoplist/ it_init,time_init,it_max,time_max,dtmin,reset_it,&
       reset_time,wall_time_max

    namelist /methodlist/ time_stepper,time_integrator, source_split_usr,&
       typesourcesplit,dimsplit,typedimsplit,flux_scheme,typepred1,limiter,&
       gradient_limiter,cada3_radius,loglimit,typetvd,typeentropy,entropycoef,&
       typeaverage, typegrad,typediv,typecurl,nxdiffusehllc, flathllc,&
        tvdlfeps,positivity_preserving,flatcd,flatsh,rk2_alfa,ssprk_order,&
       rk3_switch,imex_switch,small_values_method, small_values_daverage,&
        fix_small_values, small_values_fix_iw, check_NaN_values

    namelist /boundlist/ nghostcells,typeboundary,typeghostfill,&
       prolongation_method,internalboundary, typeboundary_min1,&
       typeboundary_min2,typeboundary_max1,typeboundary_max2,&
    ...
    namelist /meshlist/ refine_max_level,nbufferx1,nbufferx2,refine_threshold,&
       ...
       xprobmin2,xprobmax1,xprobmax2, w_refine_weight, typeprolonglimit,&
    ...
    allocate(typeboundary_min1(nprimbc))
    allocate(typeboundary_max1(nprimbc))
    typeboundary_min1 = undefined
    typeboundary_max1 = undefined
    
    allocate(typeboundary_min2(nprimbc))
    allocate(typeboundary_max2(nprimbc))
...
'Output type | tsavestart |  dtsave   | ditsave | itsave(1) | tsave(1)'
    ...
    case ("onestep")
       ...
       use_imex_scheme=(time_integrator=='IMEX_Euler'.or.time_integrator==&
          'IMEX_SP')
       if ((time_integrator/='Forward_Euler'.and.time_integrator/=&
          'IMEX_Euler').and.time_integrator/='IMEX_SP') then
    ...
    case ("twostep")
       ...
       use_imex_scheme=(time_integrator=='IMEX_Midpoint'.or.time_integrator==&
          'IMEX_Trapezoidal')
       if (((time_integrator/='Predictor_Corrector'.and.time_integrator/=&
          'RK2_alfa').and.(time_integrator/='ssprk2'.and.time_integrator/=&
          'IMEX_Midpoint')).and.time_integrator/='IMEX_Trapezoidal') then
    ...
    case ("threestep")
       ...
       if(time_integrator=='IMEX_232') then
           select case(imex_switch)
             ...
             case default
                call mpistop("Unknown imex_switch")
           end select
           imex_c2=imex_a21
           imex_c3=imex_a31+imex_a32
           imex_b3=1.0d0-imex_b1-imex_b2
       endif
       use_imex_scheme=(time_integrator=='IMEX_ARS3'.or.time_integrator==&
          'IMEX_232')
       if ((time_integrator/='ssprk3'.and.time_integrator/=&
          'RK3_BT').and.(time_integrator/='IMEX_ARS3'.and.time_integrator/=&
          'IMEX_232')) then
    ...
    case ("fourstep")
       ...
       if ((time_integrator/='ssprk4'.and.time_integrator/=&
          'rk4').and.time_integrator/='jameson') then
    ...
    if (any(typeboundary_min1 /= undefined)) then
      typeboundary(1:nprimbc, 2*1-1) = typeboundary_min1(1:nprimbc)
    end if

    if (any(typeboundary_max1 /= undefined)) then
      typeboundary(1:nprimbc, 2*1) = typeboundary_max1(1:nprimbc)
    end if
    
    if (any(typeboundary_min2 /= undefined)) then
      typeboundary(1:nprimbc, 2*2-1) = typeboundary_min2(1:nprimbc)
    end if

    if (any(typeboundary_max2 /= undefined)) then
      typeboundary(1:nprimbc, 2*2) = typeboundary_max2(1:nprimbc)
    end if
    ...
    if (nprimbc<nprim) then
      ...
      ! take the same boundary type as veloc
      if (nvector > 1) then
        do iw = 2, nvector
          do i = 1, ndir
    ...
    do idim=1,ndim
       ...
       if (periodB(idim).or.aperiodB(idim)) then
          do iw=1,nprim
             ...
             if (typeboundary(iw,2*idim-1) /= 'periodic' .and. typeboundary(iw,&
                2*idim-1) /= 'aperiodic') then
    ...
    do idim=1,ndim
      if(any(typeboundary(:,2*idim-1)=='pole')) then
        if(any(typeboundary(:,2*idim-1)/='pole')) typeboundary(:,&
           2*idim-1)='pole'
          windex=2
 ...
 !if(physics_type=='mhd') typeboundary(ndir+windex+2:ndir+windex+ndir,2*idim)='asymm'
    ...
    if (any([ domain_nx1/block_nx1,domain_nx2/block_nx2 ] == 1) .and. mype == &
       0) then
...
'will try default value for qstretch_baselevel in dimension', 2
  ...
  subroutine snapshot_read_header(fh, offset_tree, offset_block)
    ...
    if (version > 4) then
      ...
      if (any(periodic .neqv. periodB)) call &
  ...
  subroutine block_shape_io(igrid, n_ghost, ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
    ...
    n_values = count_ix(ixOmin1,ixOmin2,ixOmax1,ixOmax2) * nprim
  ...
  subroutine write_snapshot
    ...
    n_values = count_ix(ixGlo1,ixGlo2,ixGhi1,ixGhi2) * nprim
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGslo2,ixGshi1,ixGshi2) * nprims
    end if
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      call block_shape_io(igrid, n_ghost, ixOmin1,ixOmin2,ixOmax1,ixOmax2,&
          n_values)
      if(stagger_grid) then
        w_buffer(1:n_values) = pack(ps(igrid)%prim(ixOmin1:ixOmax1,&
           ixOmin2:ixOmax2, 1:nprim), .true.)
        ixOsmin1 = ixOmin1 -1
        ixOsmin2 = ixOmin2 -1
        ixOsmax1 = ixOmax1 
        ixOsmax2 = ixOmax2 
        n_values_stagger= count_ix(ixOsmin1,ixOsmin2,ixOsmax1,ixOsmax2)*nprims
        w_buffer(n_values+1:n_values+n_values_stagger) = &
           pack(ps(igrid)%prims(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2, 1:nprims),&
            .true.)
  ...
  subroutine read_snapshot
    ...
    double precision, allocatable :: w_buffer(:)
    double precision, dimension(:,:,:), allocatable :: w, ws
    ...
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGslo2,ixGshi1,ixGshi2) * nprims
      allocate(ws(ixGslo1:ixGshi1,ixGslo2:ixGshi2,1:nprims))
    end if
    allocate(w_buffer(n_values))
        ...
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          if (mype == ipe) then ! Root task
            ...
            if(stagger_grid) then
              ...
              ps(igrid)%prims(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2,&
                 1:nprims)=reshape(w_buffer(n_values_stagger+1:n_values),&
                  shape(ws(ixOsmin1:ixOsmax1,ixOsmin2:ixOsmax2, 1:nprims)))
            else 
              w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
            ...
            if (nw_found<nprim) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
                   ixMhi1,ixMhi2,nw_found,w,ps(igrid)%x,ps(igrid)%prim)
              else
                ps(igrid)%prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
              ...
              if (associated(usr_transform_w)) then
                ...
                ps(igrid)%prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                   1:nprim)=w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nprim)
              end if
            else
              ps(igrid)%prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
                 1:nprim)=w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,1:nprim)
  ...
  subroutine read_snapshot_old()
    ...
    double precision              :: wio(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim)
  ...
  subroutine get_volume_average(power, mode, volume)
    ...
    do iigrid = 1, igridstail
       ...
       wsum(nprim+1) = wsum(nprim+1) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1,&
          ixMlo2:ixMhi2))

       ! Compute the modes of the cell-centered variables, weighted by volume
       do iw = 1, nprim
          wsum(iw) = wsum(iw) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2)*ps(igrid)%prim(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
             iw)**power)
       end do
    end do

    ! Make the information available on all tasks
    call MPI_ALLREDUCE(wsum, dsum_recv, nprim+1, MPI_DOUBLE_PRECISION, MPI_SUM,&
        icomm, ierrmpi)
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       do i1 = ixMlo1, ixMhi1
       do i2 = ixMlo2, ixMhi2
       wsum(1) = wsum(1) + ps(igrid)%dvolume(i1,i2) * func(ps(igrid)%prim(i1,&
          i2, :), nprim)
       end do
       end do
    end do
  ...
  subroutine get_global_maxima(wmax)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nprim
          wmax_mype(iw)=max(wmax_mype(iw),maxval(ps(igrid)%prim(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do
  ...
  subroutine get_global_minima(wmin)
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       do iw = 1, nprim
          wmin_mype(iw)=min(wmin_mype(iw),minval(ps(igrid)%prim(ixMlo1:ixMhi1,&
             ixMlo2:ixMhi2,iw)))
       end do
    end do
