module mod_grhd_ccsn_leakage_phys_convert
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_phys_convert_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_leakage_phys_convert_init()
    ! initialize the k_max and v_max
    v_max = dsqrt(1.0d0 - 1.0d0 / lfac_max**2)
    k_max = 2.0d0 * v_max / (1.0d0 + v_max**2) ! k_max < 1
    phys_to_conserved        => grhd_ccsn_leakage_to_conserved
    phys_to_primitive        => grhd_ccsn_leakage_to_primitive
  end subroutine grhd_ccsn_leakage_phys_convert_init

  !> Transform primitive variables into conservative ones
  subroutine grhd_ccsn_leakage_to_conserved(ixI^L, ixO^L, cons, prim, x)
    ...
    if ( fix_small_values ) then
       call grhd_ccsn_leakage_handle_small_values(prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), ixI^L, ixO^L, .True., 'grhd_ccsn_leakage_to_conserved')
    end if
...
!  write(*,*) cons(5, coolingsource(:))
  ...
  end subroutine grhd_ccsn_leakage_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grhd_ccsn_leakage_to_primitive(ixI^L, ixO^L, cons, prim, x)
    use mod_global_parameters
    use mod_geometry
...
!            eps_bar = max(small_eps, eos_epsmin)
         ...
         enddo
         call grhd_ccsn_leakage_update_eos_one_point(prim(ix^D, 1:nprim))       
    ...
    if (any(flag(ixO^S) < 0)) then
       flag(ixO^S) = rho_
       call small_values_error(prim, x, ixI^L, ixO^L, flag, 'grhd_ccsn_leakage_to_primitive: (NaN z)')
    end if
...
!          if (present(ye_in)) ye_bar = ye_in / D
  ...
  end subroutine grhd_ccsn_leakage_to_primitive
  
end module mod_grhd_ccsn_leakage_phys_convert
