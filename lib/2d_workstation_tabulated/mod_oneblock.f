...
if (mype == 0) write(*,*) 'mod_oneblock: reading ',nprim,' variables on unit:',&
    unit
...
if (mype == 0) then 
   ...
   allocate(woneblock(nc1,nc2,1:nprim))
   
   do ix2=1,nc2
   do ix1=1,nc1  
   
   read(unit,*) xoneblock(ix1,ix2,1:2), woneblock(ix1,ix2,1:nprim)
...
if (npe>1) then 
   ...
   call MPI_BCAST(woneblock,nc1*nc2*nprim,MPI_DOUBLE_PRECISION,0,icomm,&
      ierrmpi)
end if! npe>1
