module mod_grhd_ccsn_leakage_phys_parameters
  use mod_physics
  implicit none
  public
 ...
 ! double precision             :: lfac_max = 1.1d1
  ...
  subroutine grhd_ccsn_leakage_update_eos_one_point(prim)
...
!    call eos_get_cs2_one_grid(prim(cs2_),prim( rho_),prim( eps_), ye=prim( ye_))
  ...
  end subroutine grhd_ccsn_leakage_update_eos_one_point

  !> get some useful variables from primitive
  subroutine grhd_ccsn_leakage_get_intermediate_variables(ixImin1,ixImax1,&
      ixOmin1,ixOmax1, prim, x, gamma, lfac2, lfac, v2, h )
    use mod_global_parameters
    use mod_geometry
  ...
  end subroutine grhd_ccsn_leakage_get_intermediate_variables

  subroutine grhd_ccsn_leakage_get_lambda(ixImin1,ixImax1, ixOmin1,ixOmax1,&
      idim, prim, x, lambda)
     use mod_global_parameters
     ...
     call grhd_ccsn_leakage_get_intermediate_variables(ixImin1,ixImax1,&
         ixOmin1,ixOmax1, prim(ixImin1:ixImax1, 1:nprim), x(ixImin1:ixImax1,&
         1:ndim), gamma=gamma(ixImin1:ixImax1,1:3,1:3), v2=v2(ixImin1:ixImax1),&
     ...
     lambda(ixOmin1:ixOmax1,2) = prim(ixOmin1:ixOmax1,&
         ...
         beta(idim)) 
  end subroutine grhd_ccsn_leakage_get_lambda

  !> This subroutine fix the abnormal values in conserved variables !
  subroutine grhd_ccsn_leakage_handle_small_values(prim, x, ixImin1,ixImax1,&
      ixOmin1,ixOmax1, update_eos, subname)
    use mod_global_parameters
...
!             prim(ix^D, eps_) = max(small_eps, eps_min)
          ...
          if ( need_to_fix_eos .and. update_eos ) then
             call grhd_ccsn_leakage_update_eos_one_point(prim(ix1,1:nprim))
          end if
       enddo
  ...
  end subroutine grhd_ccsn_leakage_handle_small_values

end module mod_grhd_ccsn_leakage_phys_parameters
