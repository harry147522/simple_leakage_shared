...
contains
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         do i = 1, nc
            ...
            cc(i, j, i_out) = cc(i, j, i_out) + cc(i, j, f1) * ( 1.0d0 + cc(i,&
                j, n) )
         end do
      end do
    end associate
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         do i = 1, nc
            ...
            pre_fac(2) = r_cc(i)**2
            call Laplacian(cc(i, j, i_out), u, idr2,&
                face_coeff_in = face_coeff, pre_fac_in = pre_fac)
            ! nonlinear source terms
            cc(i, j, i_out) = cc(i, j, i_out) + r_cc(i)**2 * cc(i, j,&
                f1) * ( 1.0d0 + cc(i, j, n) )
         end do
      end do
    end associate
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            call Laplacian(Lop, u, idr2, dLdu = dLdu,&
                face_coeff_in = face_coeff, pre_fac_in = pre_fac)
            ! nonlinear source terms
            Lop = Lop + r_cc(i)**2 * cc(i, j, f1) * ( 1.0d0 + cc(i, j, n) )
  ...
  subroutine box_slalp(mg, id, nc, i_out)
    ...
    r_face = mg%boxes(id)%r_min(1) + dr(1) * [(i-1.0_dp, i=1,nc+1)]
    r_cc  = (mg%boxes(id)%r_min(1) + dr(1) * [(i-0.5_dp, i=1,nc)])
    sin_theta_face  = dsin(mg%boxes(id)%r_min(2) + dr(2) * [(i-1.0_dp, i=1,&
       nc+1)])
    sin_theta_cc    = dsin(mg%boxes(id)%r_min(2) + dr(2) * [(i-0.5_dp, i=1,&
       nc)])
      ...
      do j = 1, nc
         do i = 1, nc
            ...
            call Laplacian(cc(i, j, i_out), u, idr2,&
                face_coeff_in = face_coeff, pre_fac_in = pre_fac)
            ! nonlinear source terms
            cc(i, j, i_out) = cc(i, j, i_out) + r_cc(i)**2 * sin_theta_cc(j) * &
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            call Laplacian(Lop, u, idr2, dLdu = dLdu,&
                face_coeff_in = face_coeff, pre_fac_in = pre_fac)
            ! nonlinear source terms
            Lop = Lop + r_cc(i)**2 * sin_theta_cc(j) * cc(i, j,&
                f1) * ( 1.0d0 + cc(i, j, n) )
  ...
  subroutine box_clbeta(mg, id, nc, i_dir, i_out)
    ...
    do j = 1, nc
       do i = 1, nc
          ...
          pre_fac(1) = r_cc(i)
          pre_fac(2) = r_cc(i)**2
          call Laplacian(cc(i, j, i_out), u(:,:,i_dir), idr2,&
              face_coeff_in = face_coeff, pre_fac_in = pre_fac)
       end do
    end do
    ...
    case (1)
      do j = 1, nc
         do i = 1, nc
            ...
            call mixed_derivatives_2nd(d2u(2,1,2), u_tmp_2, dr(1), dr(2), i, j,&
                nc)
            ...
            cc(i, j, i_out) = cc(i, j, i_out) - cc(i, j, mg_vec_iphi(1))  

            ! div_X term
            cc(i, j, i_out) = cc(i, j, i_out) + 1.0_dp/3.0_dp * ( du(1,&
               1) * r_cc(i) - cc(i, j, mg_vec_iphi(1)) + r_cc(i)**2 * ( d2u(1,&
               1,1) + d2u(2,1,2) )  )
         end do
      end do
    case (2)
      do j = 1, nc
         do i = 1, nc
            ...
            call mixed_derivatives_2nd(d2u(1,1,2), u_tmp_2, dr(1), dr(2), i, j,&
                nc)
            ...
            ! div_X term
            cc(i, j, i_out) = cc(i, j, i_out) + 1.0_dp/3.0_dp * ( du(1,&
               2) * r_cc(i) +  r_cc(i)**2 * ( d2u(1,1,2) + d2u(2,2,2) ) ) 
         end do
      end do
    case (3)
      do j = 1, nc
         do i = 1, nc
            ! extra Laplacian term for vectors
            cc(i, j, i_out) = cc(i, j, i_out) - cc(i, j, mg_vec_iphi(3)) 
         end do
      end do
     case default
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            do i_dir = 1, 3
               ...
               pre_fac(1) = r_cc(i)
               pre_fac(2) = r_cc(i)**2
               call Laplacian(Lop(i_dir), u(:,:,i_dir), idr2,&
                   dLdu = dLdu(i_dir), face_coeff_in = face_coeff,&
                   pre_fac_in = pre_fac)

               select case (i_dir)
               case (1)
                 u_tmp_2(-1:1,-1:1) = cc(i-1:i+1, j-1:j+1, mg_vec_iphi(2))
                 call mixed_derivatives_2nd(d2u(2,1,2), u_tmp_2, dr(1), dr(2),&
                     i, j, nc, dLdu=dLdu_d2u(2,1,2))
                 u_tmp_1(-1:1) = cc(i-1:i+1, j, mg_vec_iphi(1))
                 du(1,1) = dudr(u_tmp_1, dr(1))
                 call derivatives_2nd(d2u(1,1,1), u_tmp_1, idr2(1),&
                     dLdu=dLdu_d2u(1,1,1))
                 ...
                 ! div_X term
                 Lop(i_dir) = Lop(i_dir) + 1.0_dp/3.0_dp * ( du(1,&
                    1) * r_cc(i) - cc(i, j,&
                     mg_vec_iphi(1)) + r_cc(i)**2 * ( d2u(1,1,1) + d2u(2,1,&
                    2) )  )
                 ! div_X for dLdu
                 dLdu(i_dir) = dLdu(i_dir) + 1.0_dp/3.0_dp * ( r_cc(i)**2 * ( &
                    dLdu_d2u(1,1,1) + dLdu_d2u(2,1,2) ) - 1.0_dp )  
               case (2)
                 u_tmp_1(-1:1) = cc(i, j-1:j+1, mg_vec_iphi(1))
                 du(1,2) = dudr(u_tmp_1, dr(2))
                 u_tmp_2(-1:1,-1:1) = cc(i-1:i+1, j-1:j+1, mg_vec_iphi(1))
                 call mixed_derivatives_2nd(d2u(1,1,2), u_tmp_2, dr(1), dr(2),&
                     i, j, nc, dLdu=dLdu_d2u(1,1,2))
                 u_tmp_1(-1:1) = cc(i, j-1:j+1, mg_vec_iphi(2))
                 call derivatives_2nd(d2u(2,2,2), u_tmp_1, idr2(2),&
                     dLdu=dLdu_d2u(2,2,2))
            ...
            do i_dir = 1, 3
               cc(i,j, mg_vec_iphi(i_dir)) = cc(i,j,&
                   mg_vec_iphi(i_dir)) - (Lop(i_dir) - cc(i,j,&
                   mg_vec_irhs(i_dir))) / dLdu(i_dir)
            end do ! end i_dir
  ...
  subroutine box_slbeta(mg, id, nc, i_dir, i_out)
    ...
    sin_theta_cc    = dsin(mg%boxes(id)%r_min(2) + dr(2) * [(i-0.5_dp, i=0,&
       nc+1)])
    sin_theta_face  = dsin(mg%boxes(id)%r_min(2) + dr(2) * [(i-1.0_dp, i=1,&
       nc+1)])
    cos_theta_cc    = dcos(mg%boxes(id)%r_min(2) + dr(2) * [(i-0.5_dp, i=0,&
       nc+1)]) 
    ...
    case (1)
      do j = 1, nc
         do i = 1, nc
            ...
            face_coeff(0:1, 1) = r_face(i:i+1)**2
            call derivatives_2nd(d2u(1,1,1), u_tmp_1, idr2(1),&
                face_coeff_in=face_coeff(0:1, 1))

            ! extra Laplacian term for vectors
            u_tmp_1(-1:1) = sin_theta_cc(j-1:j+1) * cc(i, j-1:j+1,&
                mg_vec_iphi(2))
            cc(i, j, i_out) = cc(i, j, i_out) - 2.0_dp * ( + &
               sin_theta_cc(j)**2 * cc(i, j,&
                mg_vec_iphi(1)) + sin_theta_cc(j) * dudr(u_tmp_1, dr(2)) ) 

            ! div_X term
            cc(i, j, i_out) = cc(i, j, i_out) + 1.0_dp/3.0_dp * ( &
               sin_theta_cc(j)**2 * ( d2u(1,1,1) - 2.0_dp * cc(i, j,&
                mg_vec_iphi(1)) - du(2,2) )   - sin_theta_cc(j) * &
               cos_theta_cc(j) * cc(i, j,&
                mg_vec_iphi(2))  + r_cc(i) * ( sin_theta_cc(j)**2 * d2u(2,1,&
               2) + sin_theta_cc(j) * cos_theta_cc(j) * du(2,1) ) )
         end do
      end do
    case (2)
      do j = 1, nc
         do i = 1, nc
            ...
            ! extra Laplacian term for vectors
            cc(i, j, i_out) = cc(i, j, i_out) + ( + 2.0_dp * &
               sin_theta_cc(j)**2 * du(1,2)  - cc(i, j, mg_vec_iphi(2)) ) 

            ! div_X term
            cc(i, j, i_out) = cc(i, j, i_out) + 1.0_dp/3.0_dp * ( &
               sin_theta_cc(j)**2 * ( r_cc(i) * d2u(1,1,2) + 2.0_dp * du(1,&
               2) ) + sin_theta_cc(j) * d2u(2,2,2) - cc(i, j,&
                mg_vec_iphi(2))  ) 
         end do
      end do
    case (3)
         ...
         do i = i0, nc, di
            ...
            do i_dir = 1, 3
               ...
               case (1)
                 ...
                 u_tmp_1(-1:1) = sin_theta_cc(j-1:j+1) * cc(i, j-1:j+1,&
                     mg_vec_iphi(2))
                 Lop(i_dir) = Lop(i_dir) - 2.0_dp * ( + sin_theta_cc(j)**2 * &
                    cc(i, j, mg_vec_iphi(1)) + sin_theta_cc(j) * dudr(u_tmp_1,&
                     dr(2)) ) 
                 dLdu(i_dir) = dLdu(i_dir) - 2.0_dp * sin_theta_cc(j)**2

                 ! div_X term
                 Lop(i_dir) = Lop(i_dir) + 1.0_dp/3.0_dp * ( &
                    sin_theta_cc(j)**2 * ( d2u(1,1,1) - 2.0_dp * cc(i, j,&
                     mg_vec_iphi(1)) - du(2,&
                    2) )   - sin_theta_cc(j) * cos_theta_cc(j) * cc(i, j,&
                     mg_vec_iphi(2))  + r_cc(i) * ( sin_theta_cc(j)**2 * d2u(2,&
                    1,2) + sin_theta_cc(j) * cos_theta_cc(j) * du(2,1) ) )
                 ! div_X for dLdu
                 dLdu(i_dir) = dLdu(i_dir) + 1.0_dp/3.0_dp * ( &
                    sin_theta_cc(j)**2 * ( dLdu_d2u(1,1,&
                    1) - 2.0_dp + r_cc(i) * dLdu_d2u(2,1,2) ) ) 
               case (2)
                 u_tmp_1(-1:1) = cc(i, j-1:j+1, mg_vec_iphi(1))
                 du(1,2) = dudr(u_tmp_1, dr(2))
                 u_tmp_2(-1:1,-1:1) = cc(i-1:i+1, j-1:j+1, mg_vec_iphi(1))
                 ...
                 ! extra Laplacian term for vectors
                 Lop(i_dir) = Lop(i_dir) + ( + 2.0_dp * sin_theta_cc(j)**2 * &
                    du(1,2)  - cc(i, j, mg_vec_iphi(2)) ) 
                 dLdu(i_dir) = dLdu(i_dir) - 1.0_dp

                 ! div_X term
                 Lop(i_dir) = Lop(i_dir) + 1.0_dp/3.0_dp * ( &
                    sin_theta_cc(j)**2 * ( r_cc(i) * d2u(1,1,&
                    2) + 2.0_dp * du(1,2) ) + sin_theta_cc(j) * d2u(2,2,&
                    2) - cc(i, j, mg_vec_iphi(2))  ) 

                 ! div_X for dLdu
                 dLdu(i_dir) = dLdu(i_dir)+ 1.0_dp/3.0_dp * ( &
                    sin_theta_cc(j)**2 * ( r_cc(i) * dLdu_d2u(1,1,&
                    2) ) + sin_theta_cc(j) * dLdu_d2u(2,2,2) - 1.0_dp  ) 
               case (3)
                 ! extra Laplacian term for vectors
                 Lop(i_dir) = Lop(i_dir) - cc(i, j, mg_vec_iphi(3)) 
                 dLdu(i_dir) = dLdu(i_dir) - 1.0_dp
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         do i = 1, nc
            ...
            ! nonlinear source terms
            cc(i, j, i_out) = cc(i, j, i_out) + ( cc(i, j,&
                f1) / ( 1.0d0 + cc(i, j, n) ) + cc(i, j, f2) / ( 1.0d0 + cc(i,&
                j, n) )**7)
         end do
      end do
    end associate
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            ! nonlinear source terms
            Lop = Lop + ( cc(i, j, f1) / ( 1.0d0 + cc(i, j, n) ) + cc(i, j,&
                f2) / ( 1.0d0 + cc(i, j, n) )**7)
            dLdu = dLdu + (- cc(i, j, f1) / ( 1.0d0 + cc(i, j,&
                n) )**2 - 7.0d0 * cc(i, j, f2) / ( 1.0d0 + cc(i, j, n) )**8 )
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            ! nonlinear source terms
            Lop = Lop + r_cc(i)**2 * ( cc(i, j, f1) / ( 1.0d0 + cc(i, j,&
                n) ) + cc(i, j, f2) / ( 1.0d0 + cc(i, j, n) )**7)
            dLdu = dLdu + r_cc(i)**2 * (- cc(i, j, f1) / ( 1.0d0 + cc(i, j,&
                n) )**2 - 7.0d0 * cc(i, j, f2) / ( 1.0d0 + cc(i, j, n) )**8 )
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do j = 1, nc
         do i = 1, nc
            ...
            ! nonlinear source terms
            cc(i, j, i_out) = cc(i, j, i_out) + r_cc(i)**2 * sin_theta_cc(j) * &
                ( cc(i, j, f1) / ( 1.0d0 + cc(i, j, n) ) + cc(i, j,&
                f2) / ( 1.0d0 + cc(i, j, n) )**7)
         end do
      end do
    end associate
      ...
      do j = 1, nc
         ...
         do i = i0, nc, di
            ...
            Lop = Lop + r_cc(i)**2 * sin_theta_cc(j) * ( cc(i, j,&
                f1) / ( 1.0d0 + cc(i, j, n) ) + cc(i, j, f2) / ( 1.0d0 + cc(i,&
                j, n) )**7)
            dLdu = dLdu + r_cc(i)**2 * sin_theta_cc(j) * (- cc(i, j,&
                f1) / ( 1.0d0 + cc(i, j, n) )**2 - 7.0d0 * cc(i, j,&
                f2) / ( 1.0d0 + cc(i, j, n) )**8 )
  ...
  subroutine Laplacian(lop, u_in, idr2, dLdu, face_coeff_in, pre_fac_in)
    ...
    if (present(dLdu)) then
       dLdu = 0.0d0
       do idir = 1, 2
          dLdu = dLdu + pre_fac(idir) * idr2(idir) * ( - face_coeff(1,&
              idir) - face_coeff(0, idir) )
       end do
    end if
    ...
    u = u_in
    do idir = 1, 2
       lop = lop + pre_fac(idir) * idr2(idir) * ( face_coeff(1, idir) * (u(1,&
           idir) - u(0, idir))   - face_coeff(0, idir) * (u(0, idir) - u(-1,&
           idir)) )
    end do
  ...
  subroutine derivatives_2nd(d2udr2, u, idr2, dLdu, face_coeff_in, pre_fac_in)
    ...
    if (present(dLdu)) then
       dLdu = pre_fac * idr2 * ( - face_coeff(1) - face_coeff(0) )
    end if

    d2udr2 = pre_fac * idr2 * ( face_coeff(1) * (u(1) - u(0))   - &
  ...
  subroutine mixed_derivatives_2nd(d2udxdy, u, dx, dy, i, j, nc, dLdu)
    ...
    if ( (i==1 .and. j==1).or.(i==nc .and. j==nc) ) then
       ...
       d2udxdy = - u(1,-1) - u(-1,1) + u(1,0) + u(0,1) + u(-1,0) + u(0,&
          -1) - 2.0_dp * u(0,0)
  ...
  subroutine bc_to_gc(mg, id, nc, iv, nb, bc_type)
    ...
    if ( bc_type == mg_bc_robin .and. mg%geometry_type /= mg_spherical ) then
  ...
  subroutine robin_bc_for_alppsi(mg, id, nc, iv, nb, c1)
    ...
    select case (nb)
    case (mg_neighb_lowx)
       mg%boxes(id)%cc(0, 1:nc, iv) = ( mg%boxes(id)%cc(1, 1:nc,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1)
    case (mg_neighb_highx)
       mg%boxes(id)%cc(nc+1, 1:nc, iv) = ( mg%boxes(id)%cc(nc, 1:nc,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1) * ( 1.0d0 - ( 1.0d0 + &
          mg%boxes(id)%cc(nc, 1:nc, iv ) ) / mg%boxes(id)%cc(nc, 1:nc,&
           mg_itmp2) )
    case (mg_neighb_lowy)
       mg%boxes(id)%cc(1:nc, 0, iv) = ( mg%boxes(id)%cc(1:nc, 1,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1)
    case (mg_neighb_highy)
       mg%boxes(id)%cc(1:nc, nc+1, iv) = ( mg%boxes(id)%cc(1:nc, nc,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1)
    end select
    ...
    case (mg_cylindrical)
       if ( nb <= 2 ) then
          ...
          do j = 1, nc
             ...
             tmp = get_tmp( mg%operator_type==mg_cfc_alp ) + x(2) * ( &
                mg%boxes(id)%cc(i, j+1, iv) - mg%boxes(id)%cc(i, j-1,&
                 iv) ) / (2.0_dp * dx(2))
             tmp = - 2.0_dp * dx(1) / x(1) * ( tmp ) * pm_one
             mg%boxes(id)%cc(i+pm_one, j, iv) = tmp + mg%boxes(id)%cc(i-pm_one,&
                 j, iv)
          end do
       else if ( nb <= 4 ) then
          ! lowy or highy
          ...
          do i = 1, nc
             x(1) = mg%boxes(id)%r_min(1) + dx(1) * (i-0.5_dp)
             tmp = get_tmp( mg%operator_type==mg_cfc_alp ) + x(1) * ( &
                mg%boxes(id)%cc(i+1, j, iv) - mg%boxes(id)%cc(i-1, j,&
                 iv) ) / (2.0_dp * dx(1))
             tmp = - 2.0_dp * dx(2) / x(2) * ( tmp ) * pm_one
             mg%boxes(id)%cc(i, j+pm_one, iv) = tmp + mg%boxes(id)%cc(i,&
                 j-pm_one, iv)
          end do
       else
    ...
    CONTAINS
       real(dp) function get_tmp(flag) 
          ...
          if (flag) then
             ...
             ! LHS = (u+1)(2 - 1/psi) - psi
             get_tmp = ( mg%boxes(id)%cc(i, j,&
                 iv) + 1.0_dp ) * ( 2.0_dp - 1.0_dp / mg%boxes(id)%cc(i, j,&
                 mg_itmp2) ) - mg%boxes(id)%cc(i, j, mg_itmp2)
          else
...
!   end subroutine box_jacobi_lpl
    ...
    do lvl = mg%lowest_lvl, mg%highest_lvl
       ...
       do i = 1, size(mg%lvls(lvl)%my_ids)
          ...
          if (.not.mg%vector_equation) then
             ...
             do idir =1, 3
                mg%boxes(id)%cc(:, :, mg_vec_iold(idir)) = mg%boxes(id)%cc(:,&
                    :, mg_vec_iphi(idir))
             end do
          end if
       end do
  ...
  real(dp) function max_residual_lvl(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_ids)
       ...
       if (.not.mg%vector_equation) then
          ...
          res = 0.0_dp
          do idir = 1,3
             res = max(res, maxval(abs(mg%boxes(id)%cc(1:nc, 1:nc,&
                 mg_vec_ires(idir)))) )
          end do
       end if
  ...
  subroutine update_coarse(mg, lvl)
    ...
    if (.not.mg%vector_equation) then
       ...
       if ((mg%operator_type==mg_cfc_alp) .or.(mg%operator_type==mg_cfc_psi) ) &
          then
       ...
       do i = 1, size(mg%lvls(lvl-1)%my_parents)
          ...
          do idir = 1,3
             ! Add the fine grid residual to rhs
             mg%boxes(id)%cc(1:nc_c, 1:nc_c,&
                 mg_vec_irhs(idir)) = mg%boxes(id)%cc(1:nc_c, 1:nc_c,&
                 mg_vec_irhs(idir)) + mg%boxes(id)%cc(1:nc_c, 1:nc_c,&
                 mg_vec_ires(idir))
          end do
  ...
  subroutine correct_children(mg, lvl)
    ...
    if (.not.mg%vector_equation) then
       ...
       do i = 1, size(mg%lvls(lvl)%my_parents)
          ...
          ! Store the correction in mg_ires
          do idir = 1, 3
             mg%boxes(id)%cc(:, :, mg_vec_ires(idir)) = mg%boxes(id)%cc(:, :,&
                 mg_vec_iphi(idir)) - mg%boxes(id)%cc(:, :, mg_vec_iold(idir))
          end do
       end do
       do idir = 1, 3
          call mg_prolong(mg, lvl, mg_vec_ires(idir), mg_vec_iphi(idir),&
