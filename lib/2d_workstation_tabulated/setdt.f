...
subroutine setdt()
  ...
  if (dtpar<=zero) then
     ...
     do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
        ...
        call getdt_courant(ps(igrid)%prim,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
           ixMlo2,ixMhi1,ixMhi2,qdtnew,ps(igrid)%x)
        dtnew=min(dtnew,qdtnew)

        call phys_get_dt(ps(igrid)%prim,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
           ixMlo2,ixMhi1,ixMhi2,qdtnew,dx1,dx2,ps(igrid)%x)
        dtnew=min(dtnew,qdtnew)

        if (associated(usr_get_dt)) then
           call usr_get_dt(ps(igrid)%prim,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
              ixMlo2,ixMhi1,ixMhi2,qdtnew,dx1,dx2,ps(igrid)%x)
        end if
