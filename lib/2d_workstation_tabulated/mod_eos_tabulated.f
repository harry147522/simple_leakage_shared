...
!    if (leps<eos_epsmin) stop "nuc_eos: eps < epsmin"
    ...
    call intep3d(log_rho, log_temp, ye, prs, eos_tables(:,:,:,i_logpress),&
...
!
    ...
    call intep3d(log_rho, log_temp, ye, eps, eos_tables(:,:,:,i_logenergy),&
        nrho, ntemp, nye, logrho_table, logtemp_table, ye_table)
...
!    if (leps<eos_epsmin) stop "nuc_eos: eps < epsmin"
    ...
    call intep3d(log_rho, log_temp, ye, cs2, eos_tables(:,:,:,i_cs2), nrho,&
        ntemp, nye, logrho_table, logtemp_table, ye_table)
...
!    use mod_eos_tabulated_parameters
    ...
    double precision, intent(inout), optional :: dpderho,dpdrhoe,xa,xh,xn,xp,&
       abar,zbar
...
!    if (eps<eos_epsmin) stop "nuc_eos: eps < epsmin"
    ...
    call intep3d_many(log_rho, log_temp, ye, ffx, eos_tables,  nrho, ntemp,&
        nye, nvars,     logrho_table, logtemp_table, ye_table) !, irho, itemp, iye)
...
!    use mod_eos_tabulated_parameters
    ...
    call intep3d(lr, lt0, y, tmp, eos_tables(:,:,:,i_logenergy), nrho, ntemp,&
        nye, logrho_table, logtemp_table, ye_table, ix_out=irho, iy_out=itemp,&
        iz_out=iye)
...
!    call rootfinding_falsi(log_temp, logtemp_min, logtemp_max, eos_precision, iter_max, root_is_found)
 ...
 !   endif
    ...
    do it_root = 1, iter_max
       ...
       call intep3d(lr, z, y, f, eos_tables(:,:,:,i_logenergy), nrho, ntemp,&
           nye, logrho_table, logtemp_table, ye_table)
       f = f - epsin
...
!    use mod_eos_tabulated_parameters
  ...
  tol=1.0d-10 ! need to find energy to less than 1 in 10-10,10-10
  ...
  !preconditioning 1: do we already have the right temperature?
    ...
    call intep3d(lr, lt, y, eps, eos_tables(:,:,:,i_logenergy), nrho, ntemp,&
        nye, logrho_table, logtemp_table, ye_table, d1=d1,d2=d2,d3=d3)
   ...
   !write(*,*)  "i > itmax"
    ...
    if(keyerrt.eq.667) then
       if(lt.ge.log10(t_max_hack)) then
          ...
          write(*,*) "EOS: Did not converge in findtemp!"
          write(*,*) "rl,logrho,logtemp0,ye,lt,eps,eps0,abs(eps-eps0)/eps0"
          write(*,"(i4,i4,1P10E19.10)") i,rl,lr,lt0,y,lt,eps,eps0,&
             abs(eps-eps0)/eps0
...
subroutine bisection(lr,lt0,y,eps0,lt,bivar,keyerrt,keybisect)
  ...
  tol=1.d-9 ! need to find energy to less than 1 in 10-9,10-9
  itmax=50
  ...
  lt2 = dlog10(max(10.0d0**ltmin,0.90d0*(10.0d0**lt0)))
    ...
    call intep3d(lr, lt1, y, f1a, bivar, nrho, ntemp, nye, logrho_table,&
        logtemp_table, ye_table, d1=d1,d2=d2,d3=d3)
    call intep3d(lr, lt2, y, f2a, bivar, nrho, ntemp, nye, logrho_table,&
        logtemp_table, ye_table, d1=d1,d2=d2,d3=d3)
  ...
  do i=1,itmax
    ...
    call intep3d(lr, ltmid, y, f2a, bivar, nrho, ntemp, nye, logrho_table,&
        logtemp_table, ye_table, d1=d1,d2=d2,d3=d3)
...
!    eps_min = max(eos_epsmin, eps) 
  ...
  ! find eps_max
    ...
    call intep3d(log_rho, log_temp, lye, eps, eos_tables(:,:,:,i_logenergy),&
        nrho, ntemp, nye, logrho_table, logtemp_table, ye_table)
