...
integer, intent(in)                               :: dir
double precision,dimension(0:nprim+nwauxio),intent(in):: normconv 
...
{^DM& do ix^DMB = 1,myshape(^DMB)\}
   ...
   write(unitcollapse,'(200(1pe20.12))') &
        ...
        (collapsedData(ix^DM,iw)*normconv(iw),iw=1,nprim+nwauxio)
{^DM& enddo\}
}
close(unitcollapse)
...
case (1)
   allocate(collapsedData(&
        dim2,dim3,nprim+nwauxio))
case (2)
   allocate(collapsedData(&
        dim1,dim3,nprim+nwauxio))
case (3)
   allocate(collapsedData(&
        dim1,dim2,nprim+nwauxio))
case default
...
{^IFONED
   allocate(collapsedData(nprim+nwauxio))
}
collapsedData = zero
...
{^DM& ix^DM = int(dble(ix^DMorig-nghostcells+igdim^DM-1)*2.0d0**(collapseLevel-level))+1\}
         collapsedData(ix1,ix2,1:nprim+nwauxio) = collapsedData(ix1,ix2,1:nprim+nwauxio) &
              + ps_sub(jgrid)%prim(ix1orig,ix2orig,1:nprim+nwauxio) / 2.0d0**(2*(level-collapseLevel))
      end do
   end do
else
{^DM&
 ...
 collapsedData(ix1,ix2,1:nprim+nwauxio) = collapsedData(ix1,ix2,1:nprim+nwauxio) + ps_sub(jgrid)%prim(ix1orig,ix2orig,1:nprim+nwauxio)
 {^DM& enddo\}
end if
}
{^IFTWOD
...
if (level >= collapseLevel) then
   do ix1orig = ixMdimlo1,ixMdimhi1
{^DM& ix^DM = int(dble(ix^DMorig-nghostcells+igdim^DM-1)*2.0d0**(collapseLevel-level))+1\}
         collapsedData(ix1,1:nprim+nwauxio) = collapsedData(ix1,1:nprim+nwauxio) &
              + ps_sub(jgrid)%prim(ix1orig,1:nprim+nwauxio) / 2.0d0**(level-collapseLevel)
   end do
else
{^DM&
   do ix^DM = idim^DMtargetmin,idim^DMtargetmax\}
 {^DM& ix^DMorig = int(dble(ix^DM-idim^DMtargetmin)/2.0d0**(collapseLevel-level))+1+nghostcells \}
 collapsedData(ix1,1:nprim+nwauxio) = collapsedData(ix1,1:nprim+nwauxio) + ps_sub(jgrid)%prim(ix1orig,1:nprim+nwauxio)
 {^DM& enddo\}
end if
}
{^IFONED
collapsedData(1:nprim+nwauxio) = collapsedData(1:nprim+nwauxio) + ps_sub(jgrid)%prim(1:nprim+nwauxio)
}
...
case (1)
  if(slab_uniform) then
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%prim(ixMlo2:ixMhi2,ixMlo3:ixMhi3,1:nprim+nwauxio) = &
           ps_sub(jgrid)%prim(ixMlo2:ixMhi2,ixMlo3:ixMhi3,1:nprim+nwauxio) &
           + wCC_TMP(ix,ixMlo2:ixMhi2,ixMlo3:ixMhi3,1:nprim+nwauxio) * dx1
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo1,ixMhi1
        ps_sub(jgrid)%prim(ixMlo2:ixMhi2,ixMlo3:ixMhi3,iw) = &
             ps_sub(jgrid)%prim(ixMlo2:ixMhi2,ixMlo3:ixMhi3,iw) &
...
case (2)
  if(slab_uniform) then
    do ix=ixMlo2,ixMhi2
       ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo3:ixMhi3,1:nprim+nwauxio) = &
            ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo3:ixMhi3,1:nprim+nwauxio) &
            + wCC_TMP(ixMlo1:ixMhi1,ix,ixMlo3:ixMhi3,1:nprim+nwauxio) * dx2
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo2,ixMhi2
         ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo3:ixMhi3,iw) = &
              ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo3:ixMhi3,iw) &
...
case (3)
  if(slab_uniform) then
    do ix=ixMlo3,ixMhi3
       ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo2:ixMhi2,1:nprim+nwauxio) = &
            ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo2:ixMhi2,1:nprim+nwauxio) &
            + wCC_TMP(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ix,1:nprim+nwauxio) * dx3
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo3,ixMhi3
         ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo2:ixMhi2,iw) = &
              ps_sub(jgrid)%prim(ixMlo1:ixMhi1,ixMlo2:ixMhi2,iw) &
...
case (1)
  if(slab_uniform) then
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%prim(ixMlo2:ixMhi2,1:nprim+nwauxio) = &
           ps_sub(jgrid)%prim(ixMlo2:ixMhi2,1:nprim+nwauxio) &
           + wCC_TMP(ix,ixMlo2:ixMhi2,1:nprim+nwauxio) * dx1
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo1,ixMhi1
        ps_sub(jgrid)%prim(ixMlo2:ixMhi2,iw) = &
             ps_sub(jgrid)%prim(ixMlo2:ixMhi2,iw) &
...
case (2)
  if(slab_uniform) then
    do ix=ixMlo2,ixMhi2
       ps_sub(jgrid)%prim(ixMlo1:ixMhi1,1:nprim+nwauxio) = &
            ps_sub(jgrid)%prim(ixMlo1:ixMhi1,1:nprim+nwauxio) &
            + wCC_TMP(ixMlo1:ixMhi1,ix,1:nprim+nwauxio) * dx2
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo2,ixMhi2
         ps_sub(jgrid)%prim(ixMlo1:ixMhi1,iw) = &
              ps_sub(jgrid)%prim(ixMlo1:ixMhi1,iw) &
...
if(slab_uniform) then
  do ix=ixMlo1,ixMhi1
     ps_sub(jgrid)%prim(1:nprim+nwauxio) = ps_sub(jgrid)%prim(1:nprim+nwauxio) + wCC_TMP(ix,1:nprim+nwauxio) * dx1
  end do
else
  do iw=1,nprim+nwauxio
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%prim(iw) = ps_sub(jgrid)%prim(iw) + wCC_TMP(ix,iw) * ps(igrid)%dx(ix,1)
    end do
  end do
end if
