...
contains
  ...
  subroutine add_split_source(prior)
    ...
    do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
       qdt=dt_grid(igrid)
       block=>ps(igrid)
       call addsource1_grid(igrid,qdt,qt,ps(igrid)%prim,src_active)
    end do
    !$OMP END PARALLEL DO
  ...
  subroutine addsource1_grid(igrid,qdt,qt,w,src_active)
    ...
    double precision, intent(inout) :: w(ixGlo1:ixGhi1,nprim)
    logical, intent(inout) :: src_active

    double precision :: w1(ixGlo1:ixGhi1,nprim)
    ...
    w1(ixGlo1:ixGhi1,1:nprim)=w(ixGlo1:ixGhi1,1:nprim)

    select case (typesourcesplit)
    case ('sf')
       call addsource2(qdt  ,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nprim,qt,w1,qt,w,&
          ps(igrid)%x,.true.,src_active)
    case ('sfs')
       call addsource2(qdt/2,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nprim,qt,w1,qt,w,&
          ps(igrid)%x,.true.,src_active)
    case ('ssf')
       call addsource2(qdt/2,ixGlo1,ixGhi1,ixGlo1,ixGhi1,1,nprim,qt,w,qt,w1,&
          ps(igrid)%x,.true.,src_active)
       call addsource2(qdt  ,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nprim,qt,w1,qt,w,&
          ps(igrid)%x,.true.,src_active)
    case ('ssfss')
       call addsource2(qdt/4,ixGlo1,ixGhi1,ixGlo1,ixGhi1,1,nprim,qt,w,qt,w1,&
          ps(igrid)%x,.true.,src_active)
       call addsource2(qdt/2,ixGlo1,ixGhi1,ixMlo1,ixMhi1,1,nprim,qt,w1,qt,w,&
          ps(igrid)%x,.true.,src_active)
    case default
  ...
  subroutine addsource2(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,iwmin,iwmax,qtC,&
    ...
    double precision, intent(in)     :: wCT(ixImin1:ixImax1,1:nprim),&
        x(ixImin1:ixImax1,1:ndim)
    double precision, intent(inout)  :: w(ixImin1:ixImax1,1:ncons)
