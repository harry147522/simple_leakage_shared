...
contains
  ...
  subroutine prolong_2nd_stg(sCo,sFi,ixComin1in,ixComin2in,ixComax1in,&
    ...
    double precision :: bfluxCo(sCo%ixGsmin1:sCo%ixGsmax1,&
       sCo%ixGsmin2:sCo%ixGsmax2,nprims),bfluxFi(sFi%ixGsmin1:sFi%ixGsmax1,&
       sFi%ixGsmin2:sFi%ixGsmax2,nprims)
    double precision :: slopes(sCo%ixGsmin1:sCo%ixGsmax1,&
       sCo%ixGsmin2:sCo%ixGsmax2,ndim)
 ...
 !ixFisC For 'copy', to fill the information from the fine grid needed in the internal faces prolongation step.
    ...
    associate(wCos=>sCo%prims, wFis=>sFi%prims,wCo=>sCo%prim, wFi=>sFi%prim)
        ...
        if (neighbor_type(i1,i2,igrid)==neighbor_coarse) then
          ...
          if (refine(ineighbor,ipe_neighbor)) then
            ...
            if (iside==1) then !! left side
              pface(iside,1,igrid)%face(1,1:nx2)=ps(igrid)%prims(ixMlo1-1,&
                 ixMlo2:ixMhi2,1)
            else !! right side
              pface(iside,1,igrid)%face(1,1:nx2)=ps(igrid)%prims(ixMhi1,&
                 ixMlo2:ixMhi2,1)
            end if
     ...
     do iside=1,2
        ...
        if (neighbor_type(i1,i2,igrid)==neighbor_coarse) then
          ...
          if (refine(ineighbor,ipe_neighbor)) then
            ...
            !! Store the faces
            if (iside==1) then !! left side
              pface(iside,2,igrid)%face(1:nx1,1)=ps(igrid)%prims(ixMlo1:ixMhi1,&
                 ixMlo2-1,2)
            else !! right side
              pface(iside,2,igrid)%face(1:nx1,1)=ps(igrid)%prims(ixMlo1:ixMhi1,&
                 ixMhi2,2)
  ...
  subroutine already_fine(sFi,ichild,fine_min1,fine_min2,fine_max1,fine_max2)
   ...
   do iside=1,2
      ...
      if (ineighbor>0) then
        if (iside==1) then ! left side
          if (ipe_neighbor==mype) then
            sFi%prims(ixMlo1-1,ixMlo2:ixMhi2,1)=pface(iotherside,1,&
               ineighbor)%face(1,1:nx2)
          else
            ibufnext=ibuf_recv+isize(1)
            sFi%prims(ixMlo1-1,ixMlo2:ixMhi2,&
               1)=reshape(source=recvbuffer(ibuf_recv:ibufnext-1),&
               shape=shape(sFi%prims(ixMlo1-1,ixMlo2:ixMhi2,1)))
            ibuf_recv=ibufnext
          end if
          fine_min1=.true.
        else ! right side
          if (ipe_neighbor==mype) then
            sFi%prims(ixMhi1,ixMlo2:ixMhi2,1)=pface(iotherside,1,&
               ineighbor)%face(1,1:nx2)
          else
            ibufnext=ibuf_recv+isize(1)
            sFi%prims(ixMhi1,ixMlo2:ixMhi2,&
               1)=reshape(source=recvbuffer(ibuf_recv:ibufnext-1),&
               shape=shape(sFi%prims(ixMhi1,ixMlo2:ixMhi2,1)))
            ibuf_recv=ibufnext
          end if
          fine_max1=.true.
        end if
    ...
    do iside=1,2
      ...
      if (ineighbor>0) then
        if (iside==1) then ! left side
          if (ipe_neighbor==mype) then
            sFi%prims(ixMlo1:ixMhi1,ixMlo2-1,2)=pface(iotherside,2,&
               ineighbor)%face(1:nx1,1)
          else
            ibufnext=ibuf_recv+isize(2)
            sFi%prims(ixMlo1:ixMhi1,ixMlo2-1,&
               2)=reshape(source=recvbuffer(ibuf_recv:ibufnext-1),&
               shape=shape(sFi%prims(ixMlo1:ixMhi1,ixMlo2-1,2)))
            ibuf_recv=ibufnext
          end if
          fine_min2=.true.
        else ! right side
          if (ipe_neighbor==mype) then
            sFi%prims(ixMlo1:ixMhi1,ixMhi2,2)=pface(iotherside,2,&
               ineighbor)%face(1:nx1,1)
          else
            ibufnext=ibuf_recv+isize(2)
            sFi%prims(ixMlo1:ixMhi1,ixMhi2,&
               2)=reshape(source=recvbuffer(ibuf_recv:ibufnext-1),&
               shape=shape(sFi%prims(ixMlo1:ixMhi1,ixMhi2,2)))
            ibuf_recv=ibufnext
          end if
          fine_max2=.true.
        end if
