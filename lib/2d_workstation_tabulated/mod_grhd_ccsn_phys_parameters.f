...
!    call eos_get_cs2_one_grid(prim(cs2_),prim( rho_),prim( eps_), ye=prim( ye_))
  ...
  subroutine grhd_ccsn_get_intermediate_variables(ixImin1,ixImin2,ixImax1,&
     ixImax2, ixOmin1,ixOmin2,ixOmax1,ixOmax2, prim, x, gamma, lfac2, lfac, v2,&
      h )
    use mod_global_parameters
    use mod_geometry
  ...
  subroutine grhd_ccsn_get_lambda(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2, idim, prim, x, lambda)
     use mod_global_parameters
     integer, intent(in)             :: ixImin1,ixImin2,ixImax1,ixImax2,&
  ...
  subroutine grhd_ccsn_handle_small_values(prim, x, ixImin1,ixImin2,ixImax1,&
 ...
 ! avoid coordinate singularities
    ...
    if ( coordinate /= cartesian ) then
       ...
       if ( coordinate == spherical ) then
          where ( dabs(x(ixOmin1:ixOmax1,ixOmin2:ixOmax2,2)) < smalldouble ) 
             prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2, W_vel(2)) = 0.0d0
             !prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2, veloc(2)) = 0.0d0
             prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2, beta(2)) = 0.0d0
          end where
       end if
