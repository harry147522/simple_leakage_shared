...
module mod_physics
  ...
  abstract interface
     ...
     subroutine sub_modify_wLR(ixI^L, ixO^L, consL, consR, primL, primR, s, idir)
       use mod_global_parameters
     ...
     subroutine sub_get_var_from_state(ps_in, ixI^L, ixO^L, out)
       use mod_global_parameters
...
contains
  ...
  subroutine dummy_modify_wLR(ixI^L, ixO^L, consL, consR, primL, primR, s, idir)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L, idir
    double precision, intent(inout) :: consL(ixI^S,1:ncons), consR(ixI^S,1:ncons)
    double precision, intent(inout) :: primL(ixI^S,1:nprim), primR(ixI^S,1:nprim)
