...
contains
  ...
  subroutine grmhd_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    ...
    call grmhd_get_intermediate_variables(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        ...
        B_dot_v=B_dot_v(ixImin1:ixImax1), Ptot=Ptot(ixImin1:ixImax1),&
    ...
    do idir = 1, ndir
       cons(ixOmin1:ixOmax1, mom(idir)) = ( ( cons(ixOmin1:ixOmax1,&
           D_) * htot(ixOmin1:ixOmax1) - lfac(ixOmin1:ixOmax1) * &
          B_dot_v(ixOmin1:ixOmax1)**2 ) * prim(ixOmin1:ixOmax1,&
           W_vel(idir)) - B_dot_v(ixOmin1:ixOmax1) * prim(ixOmin1:ixOmax1,&
           Bvec(idir)) ) * gamma(ixOmin1:ixOmax1,idir,idir)
    end do

    ! Conserved energy - D = tau
    cons(ixOmin1:ixOmax1, tau_) = cons(ixOmin1:ixOmax1,&
        D_) * ( lfac(ixOmin1:ixOmax1) * htot(ixOmin1:ixOmax1) - 1.0d0 ) - &
       Ptot(ixOmin1:ixOmax1) - ( lfac(ixOmin1:ixOmax1) * &
       B_dot_v(ixOmin1:ixOmax1) )**2
    ...
    do idir = 1, ndir
       ...
       cons(ixOmin1:ixOmax1, Bcons(idir)) = prim(ixOmin1:ixOmax1,&
           Bvec(idir)) * psi6(ixOmin1:ixOmax1)
    end do
    if (type_divb == divb_GLM)  cons(ixOmin1:ixOmax1,&
        Bphi_cons_) = prim(ixOmin1:ixOmax1, Bphi_) * psi6(ixOmin1:ixOmax1)
  end subroutine grmhd_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grmhd_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    use mod_global_parameters
    use mod_geometry
    ...
    integer                         :: ix1 
    double precision                :: gamma(ixImin1:ixImax1,1:3,1:3) !metric gamma_ij_hat
    ! rescaled variables
    double precision                :: q
    double precision                :: r_i(1:ndir) !r_i
    double precision                :: bi(1:ndir) !bi, note that this is a rescaled Bfield, do not mix with the one in prim2con
    ...
    do idir = 1, ndir
       ...
       prim(ixOmin1:ixOmax1, Bvec(idir)) = cons(ixOmin1:ixOmax1,&
           Bcons(idir)) / psi6(ixOmin1:ixOmax1)
    end do
    if (type_divb == divb_GLM) prim(ixOmin1:ixOmax1,&
        Bphi_) = cons(ixOmin1:ixOmax1, Bphi_cons_) / psi6(ixOmin1:ixOmax1)
    ...
    do ix1 = ixOmin1,ixOmax1 
      ...
      if ( (cons_tmp(ix1, D_) <= small_D) .or. (cons_tmp(ix1,&
         ...
         q = cons_tmp(ix1, tau_) / cons_tmp(ix1, D_)
         do idir = 1, ndir
            r_i(idir) = cons_tmp(ix1, mom(idir)) / cons_tmp(ix1, D_)
            bi(idir) = prim(ix1, Bvec(idir)) / dsqrt( cons_tmp(ix1, D_) )
         end do
         ...
         do idir = 1, ndir
            ...
            b2 = b2 + bi(idir)**2 * gamma(ix1,idir,idir)
            r2 = r2 + r_i(idir)**2 / gamma(ix1,idir,idir)
         end do
...
"Warning: Newton raphson failed when finding mu+, now try illinois"
      ...
      prim(ix1, rho_)          = rho_hat
      prim(ix1, eps_)          = eps_hat
      prim(ix1, press_)        = prs_hat
      prim(ix1, W_vel(1:ndir)) = vi_hat(1:ndir) * W_hat
      ! update cs2
      call eos_get_cs2_one_grid(prim(ix1,cs2_),prim(ix1, rho_),prim(ix1,&
          eps_))
      ...
      if ( adjustment ) then
         B_dot_v = 0.0d0
         do idir = 1, ndir
            B_dot_v = B_dot_v + prim(ix1,&
                Bvec(idir)) * vi_hat(idir) * gamma(ix1,idir,idir)
         end do
         b2 = B_dot_v**2
         do idir = 1, ndir
            b2 = b2 + prim(ix1, Bvec(idir))**2 * gamma(ix1,idir,&
               idir) / W_hat**2
         end do
         ...
         cons_tmp(ix1, D_) = W_hat * rho_hat
         cons_tmp(ix1, tau_) = cons_tmp(ix1,&
             D_) * ( W_hat * h_hat - 1.0d0 ) - prs_hat - ( W_hat * B_dot_v &
            )**2

         ! conformal transformation
         cons(ix1, tau_) = psi6(ix1) * cons_tmp(ix1, tau_) 

         ! we need to update all conserved variables
         if ( flag(ix1) /= 1 ) then
            do idir = 1, ndir
               cons(ix1, mom(idir)) = ( ( cons(ix1,&
                   D_) * h_hat - W_hat * B_dot_v**2 ) * prim(ix1,&
                   W_vel(idir)) - B_dot_v * prim(ix1,&
                   Bvec(idir)) ) * gamma(ix1,idir,idir)
            end do
            ! conformal transformation
            cons(ix1, D_) = psi6(ix1) * cons_tmp(ix1, D_) 
            cons(ix1, mom(1:ndir)) = psi6(ix1) * cons_tmp(ix1, mom(1:ndir)) 
         end if
