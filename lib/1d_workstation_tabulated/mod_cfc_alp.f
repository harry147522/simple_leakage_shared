module mod_cfc_alp
  ...
  subroutine cfc_solve_alp(A2)
    ...
    double precision               :: tilde_S(ixGlo1:ixGhi1)
    ...
    do iigrid = 1, igridstail
       ...
       mg%boxes(id)%cc(0:nc+1, mg_iphi) = (ps(igrid)%prim(ixmin1:ixmax1,&
           alp_) * ps(igrid)%prim(ixmin1:ixmax1, psi_) - 1.0d0)

       ! source term 1
       call phys_get_tilde_S(ps(igrid), ixGlo1,ixGhi1, ixMlo1,ixMhi1,&
           tilde_S(ixGlo1:ixGhi1))
       src1(ixMlo1:ixMhi1) = - 2.0d0 * dpi / ps(igrid)%prim(ixMlo1:ixMhi1,&
           psi_)**2 * ( ps(igrid)%cons(ixMlo1:ixMhi1,&
           tau_) + ps(igrid)%cons(ixMlo1:ixMhi1,&
           D_) + 2.0d0 * tilde_S(ixMlo1:ixMhi1) ) - 7.0d0/8.0d0 / &
          ps(igrid)%prim(ixMlo1:ixMhi1, psi_)**8 * ( A2(ixMlo1:ixMhi1,&
           iigrid) )
       ...
       mg%boxes(id)%cc(0:nc+1, mg_itmp2) = ps(igrid)%prim(ixmin1:ixmax1, psi_)
       ! No RHS 
    ...
    do iigrid = 1, igridstail
       ...
       ps(igrid)%prim(ixmin1:ixmax1, alp_) = ( mg%boxes(id)%cc(0:nc+1,&
           mg_iphi) + 1.0d0 ) / ps(igrid)%prim(ixmin1:ixmax1, psi_)
    end do
