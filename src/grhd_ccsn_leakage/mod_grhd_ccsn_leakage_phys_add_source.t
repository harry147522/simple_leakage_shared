...
contains
  ...
  subroutine grhd_ccsn_leakage_add_source(qdt, ixI^L, ixO^L, primCT, cons, x, qsourcesplit, active)
    use mod_global_parameters
    use mod_geometry
    ...
    call grhd_ccsn_leakage_get_intermediate_variables(ixI^L, ixO^L, primCT(ixI^S, 1:nprim), x(ixI^S, 1:ndim), &
...
!write(*,*) add_source(5,tau_) , x(5,r_), qdt,"normal ource"
  ...
  subroutine grhd_ccsn_leakage_add_source_geom(qdt, ixI^L, ixO^L, consCT, primCT, cons, x)
    use mod_global_parameters
    use mod_geometry
    integer, intent(in)             :: ixI^L, ixO^L
    double precision, intent(in)    :: qdt, x(ixI^S, 1:ndim)
    ...
    call grhd_ccsn_leakage_get_intermediate_variables(ixI^L, ixO^L, primCT(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lfac=lfac(ixI^S) )
