...
module mod_grhd_ccsn_leakage_mpi_ppm
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_mpi_ppm_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_leakage_mpi_ppm_init()
    use mod_physics_ppm

    phys_ppm_flatcd => grhd_ccsn_leakage_mpi_ppm_flatcd
    phys_ppm_flatsh => grhd_ccsn_leakage_mpi_ppm_flatsh
  end subroutine grhd_ccsn_leakage_mpi_ppm_init

  subroutine grhd_ccsn_leakage_mpi_ppm_flatcd(rec_from, rec_to, ixI^L, ixO^L, ixL^L, ixR^L, w, d2w, drho, dp)
    ...
    ! pressure is needed to be updated based on the d2w
       call eos_get_pressure_one_grid(d2w_press(ix^D), d2w(ix^D, rho_), d2w(ix^D, eps_), ye=d2w(ix^D, ye_),temp= d2w(ix^D,temp_))
    {enddo^D&\}
  ...
  end subroutine grhd_ccsn_leakage_mpi_ppm_flatcd

  subroutine grhd_ccsn_leakage_mpi_ppm_flatsh(ixI^L, ixO^L, ixLL^L, ixL^L, ixR^L, ixRR^L, idims, w, beta, z, dv)
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_leakage_mpi_ppm_flatsh

end module mod_grhd_ccsn_leakage_mpi_ppm
