...
contains
  ...
  subroutine gremhd_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    ...
    if ( fix_small_values ) then
       call gremhd_handle_small_values(prim(ixImin1:ixImax1, 1:nprim),&
           x(ixImin1:ixImax1, 1:ndim), ixImin1,ixImax1, ixOmin1,ixOmax1,&
           .True., 'gremhd_to_conserved')
    end if
    ...
    call gremhd_get_intermediate_variables(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        ...
        sqrt_gamma=sqrt_gamma(ixImin1:ixImax1), E2=E2(ixImin1:ixImax1),&
        B2=B2(ixImin1:ixImax1), lfac=lfac(ixImin1:ixImax1),&
        h=h(ixImin1:ixImax1) )
    ...
    do idir = 1, ndir
       ...
       do jdir = 1, ndir
          do kdir = 1, ndir
             cons(ixOmin1:ixOmax1, mom(idir)) = cons(ixOmin1:ixOmax1,&
                 mom(idir)) + sqrt_gamma(ixOmin1:ixOmax1) * lvc(idir,jdir,&
                kdir) * prim(ixOmin1:ixOmax1,&
                 Evec(jdir)) * prim(ixOmin1:ixOmax1, Bvec(kdir))
          end do
       end do
    end do
    ...
    cons(ixOmin1:ixOmax1, tau_) = cons(ixOmin1:ixOmax1,&
        D_) * ( lfac(ixOmin1:ixOmax1) * h(ixOmin1:ixOmax1) - 1.0d0 ) - &
       prim(ixOmin1:ixOmax1, press_) + 0.5d0 * ( E2(ixOmin1:ixOmax1) + &
       B2(ixOmin1:ixOmax1) )
    ...
    do idir = 1, ndir
       ...
       cons(ixOmin1:ixOmax1, Econs(idir)) = prim(ixOmin1:ixOmax1,&
           Evec(idir)) * psi6(ixOmin1:ixOmax1)
    end do
  ...
  subroutine gremhd_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    use mod_global_parameters
    use mod_geometry
    ...
    call get_sqrt_gamma_hat(x(ixImin1:ixImax1, 1:ndim), ixImin1,ixImax1,&
        ixOmin1,ixOmax1, sqrt_gamma_hat(ixImin1:ixImax1))
    sqrt_gamma(ixOmin1:ixOmax1) = psi6(ixOmin1:ixOmax1) * &
       sqrt_gamma_hat(ixOmin1:ixOmax1)
    ...
    do idir = 1, ndir
       ...
       prim(ixOmin1:ixOmax1, Evec(idir)) = cons(ixOmin1:ixOmax1,&
    ...
    do idir = 1, ndir
       cons_tmp(ixOmin1:ixOmax1, tau_) = cons_tmp(ixOmin1:ixOmax1,&
           tau_) - 0.5d0 * gamma(ixOmin1:ixOmax1, idir,&
           idir) * ( prim(ixOmin1:ixOmax1,&
           Evec(idir))**2 + prim(ixOmin1:ixOmax1, Bvec(idir))**2 )
    end do
    do idir = 1, ndir
       do jdir = 1, ndir
          do kdir = 1, ndir
             cons_tmp(ixOmin1:ixOmax1, mom(idir)) = cons_tmp(ixOmin1:ixOmax1,&
                 mom(idir)) - sqrt_gamma(ixOmin1:ixOmax1) * lvc(idir, jdir,&
                 kdir) * prim(ixOmin1:ixOmax1,&
                 Evec(jdir)) * prim(ixOmin1:ixOmax1, Bvec(kdir))
          end do
    ...
    do ix1 = ixOmin1,ixOmax1 
      ...
      if ( adjustment ) then
         h_bar = 1.0d0 + eps_bar + prim(ix1, press_) / rho_bar
         cons(ix1, D_) = lfac * rho_bar
         ! Convert velocity to covariant momentum
         do idir = 1, ndir
            ...
            do jdir = 1, ndir
               do kdir = 1, ndir
                  cons(ix1, mom(idir)) = cons(ix1,&
                      mom(idir)) + sqrt_gamma(ix1) * lvc(idir,jdir,&
                     kdir) * prim(ix1, Evec(jdir)) * prim(ix1, Bvec(kdir))
               end do
            end do
         end do
     
         ! Conserved energy - D = tau
         cons(ix1, tau_) = cons(ix1, D_) * ( lfac * h_bar - 1.0d0 ) - prim(ix1,&
             press_)
         do idir = 1, ndir
            cons(ix1, tau_) = cons(ix1, tau_) + 0.5d0 * ( prim(ix1,&
                Evec(idir))**2 + prim(ix1, Bvec(idir))**2 ) * gamma(ix1, idir,&
                idir)
         end do
