...
case default
 ...
 do ix1 = 1,myshape(1)
   ...
   write(unitcollapse,'(200(1pe20.12))')  dxdim1*dble(ix1)+xdimmin1,&
       (collapsedData(ix1,iw)*normconv(iw),iw=1,nprim+nwauxio)
 enddo
...
select case(dir)
case (1)
   allocate(collapsedData(dim2,nprim+nwauxio))
case (2)
   allocate(collapsedData(dim1,nprim+nwauxio))
case default
...
if (level >= collapseLevel) then
 ...
 ix1 = int(dble(ix1orig-nghostcells+igdim1-1)*2.0d0**(collapseLevel-level))+1
         collapsedData(ix1,1:nprim+nwauxio) = collapsedData(ix1,&
            1:nprim+nwauxio) + ps_sub(jgrid)%prim(ix1orig,&
            1:nprim+nwauxio) / 2.0d0**(level-collapseLevel)
   end do
else
  ...
  ix1orig = int(dble(ix1-idim1targetmin)/2.0d0**(collapseLevel-level))+1+&
     nghostcells 
 collapsedData(ix1,1:nprim+nwauxio) = collapsedData(ix1,&
    1:nprim+nwauxio) + ps_sub(jgrid)%prim(ix1orig,1:nprim+nwauxio)
  enddo
end if
  ...
  if(slab_uniform) then
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%prim(ixMlo2:ixMhi2,1:nprim+nwauxio) = &
         ps_sub(jgrid)%prim(ixMlo2:ixMhi2,1:nprim+nwauxio) + wCC_TMP(ix,&
         ixMlo2:ixMhi2,1:nprim+nwauxio) * dx1
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo1,ixMhi1
        ps_sub(jgrid)%prim(ixMlo2:ixMhi2,&
           iw) = ps_sub(jgrid)%prim(ixMlo2:ixMhi2,iw) + wCC_TMP(ix,&
           ixMlo2:ixMhi2,iw) * ps(igrid)%dx(ix,ixMlo2:ixMhi2,1)
      end do
    end do
  end if
  ...
  if(slab_uniform) then
    do ix=ixMlo2,ixMhi2
       ps_sub(jgrid)%prim(ixMlo1:ixMhi1,1:nprim+nwauxio) = &
          ps_sub(jgrid)%prim(ixMlo1:ixMhi1,&
          1:nprim+nwauxio) + wCC_TMP(ixMlo1:ixMhi1,ix,1:nprim+nwauxio) * dx2
    end do
  else
    do iw=1,nprim+nwauxio
      do ix=ixMlo2,ixMhi2
         ps_sub(jgrid)%prim(ixMlo1:ixMhi1,&
            iw) = ps_sub(jgrid)%prim(ixMlo1:ixMhi1,iw) + wCC_TMP(ixMlo1:ixMhi1,&
            ix,iw) * ps(igrid)%dx(ixMlo1:ixMhi1,ix,2)
      end do
    end do
  end if
