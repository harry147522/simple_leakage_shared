 ...
 contains
       ...
       do idims= idim^LIM
         select case (idims)
           ...
           do iside=1,2
             ...
             ! remove coarse flux
             if (slab_uniform) then
               psb(igrid)%prim(ix^D%ixM^T,nw0:nw1) &
                    = psb(igrid)%prim(ix^D%ixM^T,nw0:nw1) &
                    -pflux(iside,^D,igrid)%flux(1^D%:^DD&,1:nwfluxin)
             else
               do iw=nw0,nw1
                 psb(igrid)%prim(ix^D%ixM^T,iw)=psb(igrid)%prim(ix^D%ixM^T,iw)&
            ...
            {do ic^DDB=1+int((1-i^DDB)/2),2-int((1+i^DDB)/2)
               ...
               if (ipe_neighbor==mype) then
                 iotherside=3-iside
                 if (slab_uniform) then
                   psb(igrid)%prim(ix^S,nw0:nw1) &
                        = psb(igrid)%prim(ix^S,nw0:nw1) &
                        + pflux(iotherside,^D,ineighbor)%flux(:^DD&,1:nwfluxin)&
                        * CoFiratio
                 else
                   do iw=nw0,nw1
                     psb(igrid)%prim(ix^S,iw)=psb(igrid)%prim(ix^S,iw) &
                 ...
                 if (slab_uniform) then
                   ...
                   psb(igrid)%prim(ix^S,nw0:nw1) &
                        = psb(igrid)%prim(ix^S,nw0:nw1)+CoFiratio &
                        *reshape(source=recvbuffer(ibuf:ibufnext-1), &
                        shape=shape(psb(igrid)%prim(ix^S,nw0:nw1)))
                   ibuf=ibuf+isize(^D)
                 else
                   ibufnext=ibuf+isize(^D)
                   if(stagger_grid) then
                   ...
                   do iw=nw0,nw1
                     psb(igrid)%prim(ix^S,iw)=psb(igrid)%prim(ix^S,iw) &
                          +reshape(source=recvbuffer(ibuf:ibufnext-1), &
                          shape=shape(psb(igrid)%prim(ix^S,iw))) &
                          /ps(igrid)%dvolume(ix^S)
                     ibuf=ibuf+nbuf
                   end do
                   ibuf=ibufnext
   ...
   subroutine add_sub_circ(ixF^L,ixtE^L,ixE^L,ixfE^L,edge,idims,iside,add,s)
     ...
     do idim1=1,ndim
        ...
        if (add) then
          s%prims(ixC^S,idim1)=s%prims(ixC^S,idim1)-circ(ixC^S,idim1)
        else
          s%prims(ixC^S,idim1)=s%prims(ixC^S,idim1)+circ(ixC^S,idim1)
        end if
     end do
