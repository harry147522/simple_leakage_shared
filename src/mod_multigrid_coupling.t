  ...
  subroutine mg_copy_to_tree(iw_from, iw_to, restrict, restrict_gc, factor)
    ...
    do iigrid = 1, igridstail
       ...
       mg%boxes(id)%cc(0:nc+1, iw_to) = fac * &
            ps(igrid)%prim(ixMlo1-1:ixMhi1+1, iw_from)
       }
       {^IFTWOD
       mg%boxes(id)%cc(0:nc+1, 0:nc+1, iw_to) = fac * &
            ps(igrid)%prim(ixMlo1-1:ixMhi1+1, ixMlo2-1:ixMhi2+1, iw_from)
       }
       {^IFTHREED
       mg%boxes(id)%cc(0:nc+1, 0:nc+1, 0:nc+1, iw_to) = fac * &
            ps(igrid)%prim(ixMlo1-1:ixMhi1+1, ixMlo2-1:ixMhi2+1, &
...
{^IFONED
       ps(igrid)%prim(ixMlo1:ixMhi1, iw_to) = &
            mg%boxes(id)%cc(1:nc, iw_from)
}
{^IFTWOD
       ps(igrid)%prim(ixMlo1:ixMhi1, ixMlo2:ixMhi2, iw_to) = &
            mg%boxes(id)%cc(1:nc, 1:nc, iw_from)
}
{^IFTHREED
       ps(igrid)%prim(ixMlo1:ixMhi1, ixMlo2:ixMhi2, ixMlo3:ixMhi3, iw_to) = &
...
{^IFONED
       ps(igrid)%prim(ixMlo1-1:ixMhi1+1, iw_to) = &
            mg%boxes(id)%cc(0:nc+1, iw_from)
}
{^IFTWOD
       ps(igrid)%prim(ixMlo1-1:ixMhi1+1, ixMlo2-1:ixMhi2+1, iw_to) = &
