...
contains
  ...
  subroutine put_slice(dir,xslice)
    ...
    double precision,dimension(0:nprim+nwauxio)          :: normconv 
    ...
    case (1)
       ...
       size_subblock_io=nx2*nx3*(nprim+nwexpand)*size_double
    case (2)
       ...
       size_subblock_io=nx1*nx3*(nprim+nwexpand)*size_double
    case (3)
       ...
       size_subblock_io=nx1*nx2*(nprim+nwexpand)*size_double
    case default
    ...
    case (1)
       ...
       subsizes(1)=nx2
       start(1)=ixMlo2-1
       size_subblock_io=nx2*(nprim+nwexpand)*size_double
    case (2)
       ixsubGlo(1) = ixGlo1; ixsubGhi(1) = ixGhi1;
       sizes(1) = ixGhi1
       subsizes(1)=nx1
       start(1)=ixMlo1-1
       size_subblock_io=nx1*(nprim+nwexpand)*size_double
    case default
       call mpistop("slice direction not clear in put_slice")
    end select
    }
    {^IFONED
    size_subblock_io=(nprim+nwexpand)*size_double
    }
    ...
    sizes(ndim)=nprim+nwexpand
    subsizes(ndim)=nprim+nwexpand
    start(ndim)=0
  ...
  contains
         ...
         do ipe=1,npe-1
            do jgrid=1,Morton_sub_stop(ipe)-Morton_sub_start(ipe)+1
               ...
               if (ipe == mype ) then 
                  ...
                  call MPI_SEND(ps_sub(jgrid)%prim,1,type_subblock_io,0,itag+1,icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%xC,1,type_subblockC_x_io,0,itag+2,icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%primC,1,type_subblockC_io,0,itag+3,icomm,ierrmpi)
                  call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,0,itag+4,icomm,ierrmpi)
               end if
               if (mype == 0) then
                  call MPI_RECV(ps_sub(Njgrid+1)%x,1,type_subblock_x_io,ipe,itag,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%prim,1,type_subblock_io,ipe,itag+1,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%xC,1,type_subblockC_x_io,ipe,itag+2,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%primC,1,type_subblockC_io,ipe,itag+3,icomm,status,ierrmpi)
                  call MPI_RECV(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,ipe,&
    ...
    subroutine write_slice_vtk(jgrid,slice_fh,wnamei)
      ...
      character(len=name_len), intent(in) :: wnamei(1:nprim+nwauxio)
      {#IFNDEF D1
      ! .. local ..
      ...
      case('vtuCC') ! celldata
         ...
         do iw=1,nprim+nwauxio
            ...
            write(slice_fh,'(200(1pe14.6))') {^DM&(|}roundoff_minmax(ps_sub(jgrid)%prim(ix^DM,iw)*normconv(iw),minvalue,maxvalue),{ix^DM=ixCCmin^DM,ixCCmax^DM)}
            write(slice_fh,'(a)')'</DataArray>'
         enddo
         write(slice_fh,'(a)')'</CellData>'
      ...
      case('vtu') ! pointdata
         ...
         do iw=1,nprim+nwauxio
            ...
            write(slice_fh,'(a,a,a)')&
                 '<DataArray type="Float64" Name="',TRIM(wnamei(iw)),'" format="ascii">'
            write(slice_fh,'(200(1pe14.6))') {^DM&(|}roundoff_minmax(ps_sub(jgrid)%primC(ix^DM,iw)*normconv(iw),minvalue,maxvalue),{ix^DM=ixCmin^DM,ixCmax^DM)}
            write(slice_fh,'(a)')'</DataArray>'
         enddo
         write(slice_fh,'(a)')'</PointData>'
    ...
    subroutine put_slice_csv
      ...
      if (mype==0) then
         ...
         call getheadernames(wnamei,xandwnamei,outfilehead)
         line=''
         do iw=1,ndim+nprim+nwauxio-1
            line = trim(line)//trim(xandwnamei(iw))//', '
         end do
         line = trim(line)//trim(xandwnamei(ndim+nprim+nwauxio))
      ...
      if (npe>1) then
         do ipe=1,npe-1
            do jgrid=1,Morton_sub_stop(ipe)-Morton_sub_start(ipe)+1
               ...
               if (ipe == mype ) then 
                  call MPI_SEND(ps_sub(jgrid)%x,1,type_subblock_x_io,0,itag,icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%prim,1,type_subblock_io,0,itag,icomm,ierrmpi)
                  call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,0,itag,icomm,ierrmpi)
               end if
               if (mype == 0) then
                  call MPI_RECV(ps_sub(Njgrid+1)%x,1,type_subblock_x_io,ipe,itag,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%prim,1,type_subblock_io,ipe,itag,icomm,status,ierrmpi)
                  call MPI_RECV(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,ipe,&
      ...
      do ix2=ixsubMlo(2),ixsubMhi(2)
         do ix1=ixsubMlo(1),ixsubMhi(1)
            ...
            do ix1=ixsubMlo(1),ixsubMhi(1)
               ...
               do iw = 1,nprim+nwauxio-1
                  {^IFTHREED
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,ix2,iw)*normconv(iw),minvalue,maxvalue)
                  }
                  {^IFTWOD
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,iw)*normconv(iw),minvalue,maxvalue)
                  }
                  {^IFONED
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(iw)*normconv(iw),minvalue,maxvalue)
                  }
                  line = trim(line)//trim(data)//', '
               end do
               {^IFTHREED
               write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,ix2,nprim+nwauxio)*normconv(nprim+nwauxio),minvalue,maxvalue)
               }
               {^IFTWOD
               write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,nprim+nwauxio)*normconv(nprim+nwauxio),minvalue,maxvalue)
               }
               line = trim(line)//trim(data)
    ...
    subroutine put_slice_dat
      ...
      do jgrid=1,Njgrid
         ...
         call MPI_FILE_IWRITE_AT(slice_fh,offset,ps_sub(jgrid)%prim,1,type_subblock_io, &
...
!              MPI_DOUBLE_PRECISION,status,ierrmpi)
         ...
         call MPI_FILE_WRITE(slice_fh,nprim,1,MPI_INTEGER,status,ierrmpi)
!         call MPI_FILE_WRITE(slice_fh,neqpar+nspecialpar,1,MPI_INTEGER,status,ierrmpi)
    ...
    subroutine put_slice_zerod
      ...
      if (Njgrid > 0) then
         ...
         line =  trim(line)//', '//trim(data)
         do iw = 1,nprim+nwauxio
            write(data,"(es14.6)")ps_sub(1)%prim(iw)*normconv(iw)
            line = trim(line)//', '//trim(data)
         end do
         !
  ...
  subroutine fill_subnode(igrid,active,jgrid,dir,xslice,normconv)
    ...
    {^IFONED
    ps_sub(jgrid)%prim(1:nprim+nwexpand) = wCC_TMP(ixslice,1:nprim+nwexpand)
    ps_sub(jgrid)%x(1:ndim) = xCC_TMP(ixslice,1:ndim)
    ps_sub(jgrid)%primC(1:nprim+nwexpand) = wC_TMP(ixslice,1:nprim+nwexpand)
    ps_sub(jgrid)%xC(1:ndim) = xC_TMP(ixslice,1:ndim)
    }
    {^IFTWOD
    select case (dir)
    case (1)
       ps_sub(jgrid)%prim(ixCCmin2:ixCCmax2,1:nprim+nwexpand) &
            = wCC_TMP(ixslice,ixCCmin2:ixCCmax2,1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin2:ixCCmax2,1:ndim) &
            = xCC_TMP(ixslice,ixCCmin2:ixCCmax2,1:ndim)
       ps_sub(jgrid)%primC(ixCmin2:ixCmax2,1:nprim+nwexpand) &
            = wC_TMP(ixslice,ixCmin2:ixCmax2,1:nprim+nwexpand)
       ps_sub(jgrid)%xC(ixCmin2:ixCmax2,1:ndim) &
            = xC_TMP(ixslice,ixCmin2:ixCmax2,1:ndim)
    case (2)
       ps_sub(jgrid)%prim(ixCCmin1:ixCCmax1,1:nprim+nwexpand) &
            = wCC_TMP(ixCCmin1:ixCCmax1,ixslice,1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin1:ixCCmax1,1:ndim) &
            = xCC_TMP(ixCCmin1:ixCCmax1,ixslice,1:ndim)
       ps_sub(jgrid)%primC(ixCmin1:ixCmax1,1:nprim+nwexpand) &
            = wC_TMP(ixCmin1:ixCmax1,ixslice,1:nprim+nwexpand)
    ...
    select case (dir)
    case (1)
       ps_sub(jgrid)%prim(ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,1:nprim+nwexpand) = &
            wCC_TMP(ixslice,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,1:ndim) = &
            xCC_TMP(ixslice,ixCCmin2:ixCCmax2,ixCCmin3:ixCCmax3,1:ndim)
       ps_sub(jgrid)%primC(ixCmin2:ixCmax2,ixCmin3:ixCmax3,1:nprim+nwexpand) = &
            wC_TMP(ixslice,ixCmin2:ixCmax2,ixCmin3:ixCmax3,1:nprim+nwexpand)
       ps_sub(jgrid)%xC(ixCmin2:ixCmax2,ixCmin3:ixCmax3,1:ndim) = &
            xC_TMP(ixslice,ixCmin2:ixCmax2,ixCmin3:ixCmax3,1:ndim)
    case (2)
       ps_sub(jgrid)%prim(ixCCmin1:ixCCmax1,ixCCmin3:ixCCmax3,1:nprim+nwexpand) = &
            wCC_TMP(ixCCmin1:ixCCmax1,ixslice,ixCCmin3:ixCCmax3,1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin1:ixCCmax1,ixCCmin3:ixCCmax3,1:ndim) = &
            xCC_TMP(ixCCmin1:ixCCmax1,ixslice,ixCCmin3:ixCCmax3,1:ndim)
       ps_sub(jgrid)%primC(ixCmin1:ixCmax1,ixCmin3:ixCmax3,1:nprim+nwexpand) = &
            wC_TMP(ixCmin1:ixCmax1,ixslice,ixCmin3:ixCmax3,1:nprim+nwexpand)
       ps_sub(jgrid)%xC(ixCmin1:ixCmax1,ixCmin3:ixCmax3,1:ndim) = &
            xC_TMP(ixCmin1:ixCmax1,ixslice,ixCmin3:ixCmax3,1:ndim) 
    case (3)
       ps_sub(jgrid)%prim(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:nprim+nwexpand) = &
            wCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixslice,1:nprim+nwexpand) 
       ps_sub(jgrid)%x(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,1:ndim) = &
            xCC_TMP(ixCCmin1:ixCCmax1,ixCCmin2:ixCCmax2,ixslice,1:ndim)
       ps_sub(jgrid)%primC(ixCmin1:ixCmax1,ixCmin2:ixCmax2,1:nprim+nwexpand) = &
            wC_TMP(ixCmin1:ixCmax1,ixCmin2:ixCmax2,ixslice,1:nprim+nwexpand) 
  ...
  subroutine alloc_subnode(jgrid,dir,nwexpand)
    ...
    {^IFONED
    allocate(ps_sub(jgrid)%prim(1:nprim+nwexpand),ps_sub(jgrid)%x(1:ndim))
    !allocate(ps_sub(jgrid)%cons(1:ncons))
    allocate(ps_sub(jgrid)%primC(1:nprim+nwexpand),ps_sub(jgrid)%xC(1:ndim))
    }
    {^IFTWOD
    select case (dir)
    case (1)
       allocate(ps_sub(jgrid)%prim(ixGlo2:ixGhi2,1:nprim+nwexpand),&
            ps_sub(jgrid)%x(ixGlo2:ixGhi2,1:ndim), &
            ps_sub(jgrid)%primC(ixGlo2:ixGhi2,1:nprim+nwexpand),&
            ps_sub(jgrid)%xC(ixGlo2:ixGhi2,1:ndim))
    case (2)
       allocate(ps_sub(jgrid)%prim(ixGlo1:ixGhi1,1:nprim+nwexpand),&
            ps_sub(jgrid)%x(ixGlo1:ixGhi1,1:ndim), &
            ps_sub(jgrid)%primC(ixGlo1:ixGhi1,1:nprim+nwexpand),&
    ...
    select case (dir)
    case (1)
       allocate(ps_sub(jgrid)%prim(ixGlo2:ixGhi2,ixGlo3:ixGhi3,1:nprim+nwexpand),&
            ps_sub(jgrid)%x(ixGlo2:ixGhi2,ixGlo3:ixGhi3,1:ndim), &
            ps_sub(jgrid)%primC(ixGlo2:ixGhi2,ixGlo3:ixGhi3,1:nprim+nwexpand),&
            ps_sub(jgrid)%xC(ixGlo2:ixGhi2,ixGlo3:ixGhi3,1:ndim))
    case (2)
       allocate(ps_sub(jgrid)%prim(ixGlo1:ixGhi1,ixGlo3:ixGhi3,1:nprim+nwexpand),&
            ps_sub(jgrid)%x(ixGlo1:ixGhi1,ixGlo3:ixGhi3,1:ndim), &
            ps_sub(jgrid)%primC(ixGlo1:ixGhi1,ixGlo3:ixGhi3,1:nprim+nwexpand),&
            ps_sub(jgrid)%xC(ixGlo1:ixGhi1,ixGlo3:ixGhi3,1:ndim))
    case (3)
       allocate(ps_sub(jgrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim+nwexpand),&
            ps_sub(jgrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim), &
            ps_sub(jgrid)%primC(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim+nwexpand),&
  ...
  subroutine dealloc_subnode(jgrid)
    ...
    deallocate(ps_sub(jgrid)%prim,ps_sub(jgrid)%x,ps_sub(jgrid)%primC,ps_sub(jgrid)%xC)
