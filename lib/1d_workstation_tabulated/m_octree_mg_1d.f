...
contains
  ...
  subroutine box_lalp(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = 1, nc
         ...
         cc(i, i_out) = cc(i, i_out) + cc(i, f1) * ( 1.0d0 + cc(i, n) )
      end do
    end associate
  ...
  subroutine box_slalp(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = 1, nc
         ...
         ! nonlinear source terms
         cc(i, i_out) = cc(i, i_out) + r_cc(i)**2 * cc(i, f1) * ( 1.0d0 + cc(i,&
             n) )
      end do
    end associate
  ...
  subroutine box_slbeta(mg, id, nc, i_dir, i_out)
    ...
    case (1)
      do i = 1, nc
         ...
         face_coeff(0:1, 1) = r_face(i:i+1)**2
         call Laplacian(cc(i, i_out), u(:,:,1), idr2,&
             face_coeff_in = face_coeff)
  ...
  subroutine box_gs_slbeta(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc)
      ...
      do i = i0, nc, di
         ...
         u(-1:1, 1, 1) = cc(i-1:i+1, n)
         face_coeff(0:1, 1) = r_face(i:i+1)**2
         call Laplacian(Lop(1), u(:,:,1), idr2, dLdu = dLdu(1),&
             face_coeff_in = face_coeff)
  ...
  subroutine box_lpsi(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = 1, nc
         ...
         ! nonlinear source terms
         cc(i, i_out) = cc(i, i_out) +( cc(i, f1) / ( 1.0d0 + cc(i,&
             n) ) + cc(i, f2) / ( 1.0d0 + cc(i, n) )**7 )
      end do
    end associate
  ...
  subroutine box_gs_lpsi(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = i0, nc, di
         ...
         ! nonlinear source terms
         Lop = Lop +   ( cc(i, f1) / ( 1.0d0 + cc(i, n) ) + cc(i,&
             f2) / ( 1.0d0 + cc(i, n) )**7 )

         dLdu = dLdu + (- cc(i, f1) / ( 1.0d0 + cc(i, n) )**2 - 7.0d0 * cc(i,&
             f2) / ( 1.0d0 + cc(i, n) )**8 )
  ...
  subroutine box_slpsi(mg, id, nc, i_out)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = 1, nc
         ...
         ! nonlinear source terms
         cc(i, i_out) = cc(i, i_out) + r_cc(i)**2 *  ( cc(i,&
             f1) / ( 1.0d0 + cc(i, n) ) + cc(i, f2) / ( 1.0d0 + cc(i,&
             n) )**7 )
      end do
    end associate
  ...
  subroutine box_gs_slpsi(mg, id, nc, redblack_cntr)
    ...
    associate (cc => mg%boxes(id)%cc, n => mg_iphi, f1 => mg_itmp1,&
      ...
      do i = i0, nc, di
         ...
         ! nonlinear source terms
         Lop = Lop + r_cc(i)**2 *  ( cc(i, f1) / ( 1.0d0 + cc(i, n) ) + cc(i,&
             f2) / ( 1.0d0 + cc(i, n) )**7 )

         dLdu = dLdu + r_cc(i)**2 *   (- cc(i, f1) / ( 1.0d0 + cc(i,&
             n) )**2 - 7.0d0 * cc(i, f2) / ( 1.0d0 + cc(i, n) )**8 )
  ...
  subroutine robin_bc_for_alppsi(mg, id, nc, iv, nb, c1)
    ...
    select case (nb)
    case (mg_neighb_lowx)
       mg%boxes(id)%cc(0, iv) = ( mg%boxes(id)%cc(1,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1) * ( 1.0d0 - ( 1.0d0 + &
          mg%boxes(id)%cc(1, iv ) ) / mg%boxes(id)%cc(1, mg_itmp2) )
    case (mg_neighb_highx)
       mg%boxes(id)%cc(nc+1, iv) = ( mg%boxes(id)%cc(nc,&
           mg_itmp2) - 1.0d0 ) * (1.0d0 - c1) * ( 1.0d0 - ( 1.0d0 + &
          mg%boxes(id)%cc(nc, iv ) ) / mg%boxes(id)%cc(nc, mg_itmp2) )
    end select
    ...
    CONTAINS
       real(dp) function get_tmp(flag) 
          ...
          if (flag) then
             ...
             ! LHS = (u+1)(2 - 1/psi) - psi
             get_tmp = ( mg%boxes(id)%cc(i,&
                 iv) + 1.0_dp ) * ( 2.0_dp - 1.0_dp / mg%boxes(id)%cc(i,&
                 mg_itmp2) ) - mg%boxes(id)%cc(i, mg_itmp2)
          else
  ...
  real(dp) function max_residual_lvl(mg, lvl)
    ...
    do i = 1, size(mg%lvls(lvl)%my_ids)
       ...
       if (.not.mg%vector_equation) then
          ...
          res = 0.0_dp
          do idir = 1,3
             res = max(res, maxval(abs(mg%boxes(id)%cc(1:nc,&
                 mg_vec_ires(idir)))) )
          end do
       end if
  ...
  subroutine residual_box(mg, id, nc)
    ...
    if (.not.mg%vector_equation) then
       ...
       do idir = 1, 3
          ...
          mg%boxes(id)%cc(1:nc, mg_vec_ires(idir)) = mg%boxes(id)%cc(1:nc,&
              mg_vec_irhs(idir)) - mg%boxes(id)%cc(1:nc, mg_vec_ires(idir))
       end do
    end if
