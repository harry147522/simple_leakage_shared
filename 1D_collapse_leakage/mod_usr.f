...
!    character*128  ::  profile_path = './profile/s40WW95.dat'
...
!    call grhd_activate()
    call grhd_ccsn_leakage_mpi_activate()
    call grhd_simple_leakage_activate_mpi()
...
if (mype == 0) then
  ...
  subroutine ccsn_init_one_grid(ixGmin1,ixGmax1,ixmin1,ixmax1,w,x)
    ...
    w(ixmin1:ixmax1, alp_) = 1.0d0
    w(ixmin1:ixmax1, psi_) = 1.0d0
...
!       call eos_get_cs2_one_grid(w(ix1, cs2_),w(ix1, rho_),w(ix1, eps_))
       call eos_eps_get_all_one_grid(rho = w(ix1,rho_),temp = w(ix1,temp_),&
           ye = w(ix1,ye_),eps = w(ix1,eps_),prs = w(ix1,press_),ent = w(ix1,&
          ent_), cs2 = w(ix1,cs2_))
  ...
  subroutine ccsn_improve_initial_condition()
    ...
    do iigrid = 1, igridstail
       ...
       do ix1 = ixMlo1,ixMhi1 
          ! reset eps
          call eos_get_eps_one_grid(ps(igrid)%prim(ix1, press_),&
             ps(igrid)%prim(ix1, rho_),ps(igrid)%prim(ix1, eps_),&
             temp=ps(igrid)%prim(ix1, temp_), ye=ps(igrid)%prim(ix1, ye_))
          ps(igrid)%prim(ix1, eps_) = max( ps(igrid)%prim(ix1, eps_),&
              eos_epsmin )

          ! fill atmosphere
          if (ps(igrid)%prim(ix1,rho_) <= small_rho_thr ) then
             ps(igrid)%prim(ix1,rho_)   = small_rho
             ps(igrid)%prim(ix1,eps_)   = small_eps
             ps(igrid)%prim(ix1,press_)   = small_press
             ps(igrid)%prim(ix1,W_vel(1)) = 0.0d0
       ...
       call phys_update_eos(ixGlo1,ixGhi1, ixMlo1,ixMhi1,&
           ps(igrid)%prim(ixGlo1:ixGhi1,1:nprim))
    end do
...
! code test
     if (do_ye_of_rho .and. (global_time .lt. &
        t_bounce*time_gf+0.005d0*time_gf)) then
...
!    use mod_eos_hybrid, only: rho_nuc
    ...
    integer, intent(in)          :: iit
    double precision, intent(in) :: qt
    integer              :: ix1, ixmin1,ixmax1, idir, pns_radius_local_i,&
        veloc1_min_local_i
    integer              :: igrid, iigrid
    double precision     :: rho_max, ent_max, rho_max_local, ent_max_local
    double precision     :: shock_veloc, prev_shock_veloc, ye_min_local,&
        ye_min
    ...
    double precision     :: veloc_local(ixGlo1:ixGhi1,1:ndir),&
        diff(ixGlo1:ixGhi1)
    double precision     :: lfac(ixGlo1:ixGhi1)
    double precision     :: gamma(ixGlo1:ixGhi1,1:3,1:3)
...
!Bounce checker
  if (.not. bounce) then
    do iigrid = 1, igridstail
      ...
      do ix1 = ixMlo1,ixMhi1 
        if (ps(igrid)%x(ix1,1) .le. 3.0d6*length_gf) then
           if (ps(igrid)%prim(ix1,ent_) .ge. ent_max_local) then
           ent_max_local = ps(igrid)%prim(ix1,ent_)
...
!        t_dump = global_time/time_gf + 1d-5
       if (mype == 0) then
          write(*,'(a,i7,a,es12.4)') ' core bounce at it=',it,' global_time=',&
             global_time
          !write(*,'(a,i7,a,i7,a,es12.4)') ' save a snapshot No.',&
          !     snapshotnext,' at it=',it,' global_time=',global_time

          open(unit=666,file='t_bounce.dat',status="unknown",form='formatted',&
             position="append")
          write(666,*) 'it=',it,' global_time=',global_time
          close(666)

          open(unit=666,file='savenow',status="unknown",form='formatted',&
             position="append")
          write(666,*) ''
          close(666)
...
!         call grhd_ccsn_leakage_get_intermediate_variables(ixG^LL, ixM^LL, ps(igrid)%prim(ixG^T, 1:nprim), ps(igrid)%x(ixG^T, 1:ndim), &
         call grhd_ccsn_leakage_get_intermediate_variables(ixGlo1,ixGhi1,&
             ixMlo1,ixMhi1, ps(igrid)%prim(ixGlo1:ixGhi1, 1:nprim),&
             ps(igrid)%x(ixGlo1:ixGhi1, 1:ndim), gamma=gamma(ixGlo1:ixGhi1,1:3,&
            1:3), lfac=lfac(ixGlo1:ixGhi1) )
!   咁寫 出事
!        veloc1_min_local = minval(ps(igrid)%prim(ixM^T, W_vel(1)) / lfac(ixM^T))

        veloc1_min_local = min(minval(ps(igrid)%prim(ixMlo1:ixMhi1,&
            W_vel(1)) / lfac(ixMlo1:ixMhi1)), veloc1_min_local)
...
!          if (veloc_local(ix^D,1) .eq. veloc1_min ) then
          if ((ps(igrid)%prim(ix1, W_vel(1))/lfac(ix1)) .eq. veloc1_min ) then
             shock_radius_local = max(shock_radius_local, ps(igrid)%x(ix1,&
                 r_))
!             write(*,*) shock_radius, ix1,"shock-0radius inside loop"
    ...
    call MPI_ALLREDUCE(shock_radius_local, shock_radius, 1,&
...
!          if (ps(igrid)%prim(ix1, rho_)/rho_gf .lt. 1.0d4 .and. .gt. 9.95d10) then
          if (ps(igrid)%prim(ix1, rho_)/rho_gf .gt. 1.0d11) then
               pns_radius_local = max(pns_radius_local, ps(igrid)%x(ix1, r_))
...
!!!!!!!!!!!!!!!!!!!!!!!!!code test no need
  ...
  subroutine my_refine(igrid,level,ixImin1,ixImax1,ixOmin1,ixOmax1,qt,w,x,&
     refine,coarsen)
    use mod_global_parameters
    ...
    double precision             :: ratio(ixImin1:ixImax1)
    ...
    phi_grav = minval( w(ixOmin1:ixOmax1, alp_) )
    ...
    ratio(ixOmin1:ixOmax1) = dx(1, level) / x(ixOmin1:ixOmax1,1) 

    if ( any( ratio(ixOmin1:ixOmax1) > 1.0d-2 ) ) then
       ! the resolution is too low
       refine = 1
       coarsen = -1
    else if ( all( ratio(ixOmin1:ixOmax1) < 1.0d-2 ) ) then
...
!           ps(igrid)%prim(ix^D, rho_) = 1.0d12&rho_gf
            ...
            out_munu = 0.0d0
          ! nuchem(i,j,k) = 0.0d0
           if (ps(igrid)%prim(ix1,rho_).gt.1.0d6*rho_gf) then
 !             if((.not.bounce).or.(i.gt.ishock(1))) then
               if((.not.bounce).or.(ps(igrid)%x(ix1,&
                  r_).gt.shock_radius*length_gf)) then
        !code test
         !     if(.not.bounce) then
                 if (do_yeofrhofit) then
                    call fit_ye(ps(igrid)%prim(ix1,rho_)/rho_gf,out_ye)
                 else
                    call interpolate_ye(ps(igrid)%prim(ix1,rho_)/rho_gf,&
                       out_ye)
                 endif
                 delta_ye = min(0.0d0, out_ye-ps(igrid)%prim(ix1,ye_))


                !    call eos(out_munu,rho(i,j,k),temp(i,j,k),ye(i,j,k),eps(i,j,k), &!no need temp ??
                !              keytemp,keyerr,eosflag,eos_rf_prec)
                   ltemp = ps(igrid)%prim(ix1, temp_) !do not want to update prim(temp_)
                   call eos_eps_get_all_one_grid(rho = ps(igrid)%prim(ix1,&
                      rho_), temp=ltemp, ye = ps(igrid)%prim(ix1,ye_),&
                      eps = ps(igrid)%prim(ix1,eps_),munu = out_munu)
...
!                  endif
                    ...
                    if (out_munu.lt.10.0d0) then
                       delta_S = 0.0d0
                    elseif (ps(igrid)%prim(ix1,rho_).lt.2.0d12*rho_gf) then
                       delta_S = -delta_ye*(out_munu-&
                          10.0d0)/ps(igrid)%prim(ix1,temp_)
                    else
                       delta_S = 0.0d0
                    endif
...
!                  endif
                    ...
                    ps(igrid)%prim(ix1,ent_) =  ps(igrid)%prim(ix1,&
                       ent_) + delta_S
                    ps(igrid)%prim(ix1,ye_) = ps(igrid)%prim(ix1,&
                       ye_) + delta_ye
...
!                    write(*,*)  delta_S, delta_ye, "delta"
    !                write(*,*) "pass good way ye)ofrho.f90 !!!!!!!!!!!!!"
                   ...
                   call eos_temp_get_all_one_grid(rho = ps(igrid)%prim(ix1,&
                      rho_),ye = ps(igrid)%prim(ix1,ye_),&
                       temp = ps(igrid)%prim(ix1,temp_),&
                       eps = ps(igrid)%prim(ix1,eps_), prs =ps(igrid)%prim(ix1,&
                      press_),cs2 = ps(igrid)%prim(ix1,cs2_),&
                       ent = ps(igrid)%prim(ix1, ent_))
...
!   old printlog
  subroutine printlog()
    ...
    do iigrid = 1, igridstail
       igrid = igrids(iigrid)
       locate_r = minval(ps(igrid)%x(ixMlo1:ixMhi1,1))
       if (locate_r <= tmp) then
          tmp = locate_r
          igrid_locate_r = igrid
          ilocate_r = minloc(ps(igrid)%x(ixMlo1:ixMhi1,1), dim=1,&
             mask=(ps(igrid)%x(ixMlo1:ixMhi1,1) >0))
       end if
    end do

    call MPI_ALLREDUCE(tmp, locate_r, 1, mpi_double_precision, MPI_MIN, icomm,&
        ierrmpi)
