...
subroutine lohner_grid(igrid)
  ...
  double precision                   :: w(ixGlo1:ixGhi1,1:nprim)
...
subroutine lohner_orig_grid(igrid)
  ...
  do iflag=1,nprim+1
     ...
     do idims=1,ndim
        ...
        if (iflag<=nprim) then
          if (logflag(iflag)) then
            dp(ixmin1:ixmax1)=dlog10(ps(igrid)%prim(jxmin1:jxmax1,&
               iflag))-dlog10(ps(igrid)%prim(ixmin1:ixmax1,iflag))
            dm(ixmin1:ixmax1)=dlog10(ps(igrid)%prim(ixmin1:ixmax1,&
               iflag))-dlog10(ps(igrid)%prim(hxmin1:hxmax1,iflag))
            dref(ixMlo1:ixMhi1)=dabs(dlog10(ps(igrid)%prim(jxmin1:jxmax1,&
               iflag)))+ 2.0d0 * dabs(dlog10(ps(igrid)%prim(ixMlo1:ixMhi1,&
               iflag))) + dabs(dlog10(ps(igrid)%prim(hxmin1:hxmax1,iflag)))
          else
            dp(ixmin1:ixmax1)=ps(igrid)%prim(jxmin1:jxmax1,&
               iflag)-ps(igrid)%prim(ixmin1:ixmax1,iflag)
            dm(ixmin1:ixmax1)=ps(igrid)%prim(ixmin1:ixmax1,&
               iflag)-ps(igrid)%prim(hxmin1:hxmax1,iflag)
            dref(ixMlo1:ixMhi1)=dabs(ps(igrid)%prim(jxmin1:jxmax1,&
               iflag))+2.0d0*dabs(ps(igrid)%prim(ixMlo1:ixMhi1,&
               iflag)) +dabs(ps(igrid)%prim(hxmin1:hxmax1,iflag))
          end if
        else
          if (logflag(iflag)) then
  ...
  do ix1=ixMlo1,ixMhi1
     ...
     if (associated(usr_refine_threshold)) then
        wtol(1:nprim)   = ps(igrid)%prim(ix1,1:nprim)
        xtol(1:ndim) = ps(igrid)%x(ix1,1:ndim)
        call usr_refine_threshold(wtol, xtol, threshold, global_time)
     end if
...
subroutine compare1_grid(igrid,wold,w)
  ...
  integer, intent(in) :: igrid
  double precision, intent(in) :: wold(ixGlo1:ixGhi1,1:nprim), w(ixGlo1:ixGhi1,&
     1:nprim)
...
subroutine forcedrefine_grid(igrid,w)
  ...
  integer, intent(in) :: igrid
  double precision, intent(in) :: w(ixGlo1:ixGhi1,nprim)
