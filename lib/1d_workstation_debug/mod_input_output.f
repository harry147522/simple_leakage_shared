...
contains
  ...
  subroutine read_par_files()
    ...
    namelist /boundlist/ nghostcells,typeboundary,typeghostfill,&
       ...
       typeboundary_max1, save_physical_boundary
    ...
    namelist /meshlist/ refine_max_level,nbufferx1,refine_threshold,&
       ...
       max_blocks,block_nx1,domain_nx1,iprob,xprobmin1,xprobmax1,&
        w_refine_weight, typeprolonglimit, logflag,tfixgrid,itfixgrid,&
       ditregrid
...
'Output type | tsavestart |  dtsave   | ditsave | itsave(1) | tsave(1)'
    ...
    if (any([ domain_nx1/block_nx1 ] == 1) .and. mype == 0) then
...
'will try default value for qstretch_baselevel in dimension', 1
  ...
  subroutine block_shape_io(igrid, n_ghost, ixOmin1,ixOmax1, n_values)
    ...
    n_values = count_ix(ixOmin1,ixOmax1) * nprim
  ...
  subroutine write_snapshot
    ...
    n_values = count_ix(ixGlo1,ixGhi1) * nprim
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGshi1) * nprims
    end if
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if(stagger_grid) then
        w_buffer(1:n_values) = pack(ps(igrid)%prim(ixOmin1:ixOmax1, 1:nprim),&
            .true.)
        ixOsmin1 = ixOmin1 -1
        ixOsmax1 = ixOmax1 
        n_values_stagger= count_ix(ixOsmin1,ixOsmax1)*nprims
        w_buffer(n_values+1:n_values+n_values_stagger) = &
           pack(ps(igrid)%prims(ixOsmin1:ixOsmax1, 1:nprims), .true.)
  ...
  subroutine read_snapshot
    ...
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGslo1,ixGshi1) * nprims
      allocate(ws(ixGslo1:ixGshi1,1:nprims))
    end if
    allocate(w_buffer(n_values))
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          if(stagger_grid) then
            ...
            n_values = n_values + count_ix(ixOsmin1,ixOsmax1) * nprims
          end if
            ...
            if(stagger_grid) then
              w(ixOmin1:ixOmax1, 1:nw_found) = &
                 reshape(w_buffer(1:n_values_stagger), shape(w(ixOmin1:ixOmax1,&
                  1:nw_found)))
              ps(igrid)%prims(ixOsmin1:ixOsmax1,&
                 1:nprims)=reshape(w_buffer(n_values_stagger+1:n_values),&
                  shape(ws(ixOsmin1:ixOsmax1, 1:nprims)))
            else 
            ...
            if (nw_found<nprim) then
              if (associated(usr_transform_w)) then
                call usr_transform_w(ixGlo1,ixGhi1,ixMlo1,ixMhi1,nw_found,w,&
                   ps(igrid)%x,ps(igrid)%prim)
              else
                ps(igrid)%prim(ixOmin1:ixOmax1,1:nw_found)=w(ixOmin1:ixOmax1,&
                   1:nw_found)
              end if
              ...
              if (associated(usr_transform_w)) then
                ...
                ps(igrid)%prim(ixOmin1:ixOmax1,1:nprim)=w(ixOmin1:ixOmax1,&
                   1:nprim)
              end if
            else
              ps(igrid)%prim(ixOmin1:ixOmax1,1:nprim)=w(ixOmin1:ixOmax1,&
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
        ...
        if (nw_found<nprim) then
          ...
          if (associated(usr_transform_w)) then
            ...
            ps(igrid)%prim(ixOmin1:ixOmax1,1:nprim)=w(ixOmin1:ixOmax1,1:nprim)
          end if
        else
          ps(igrid)%prim(ixOmin1:ixOmax1,1:nprim)=w(ixOmin1:ixOmax1,1:nprim)
        end if
  ...
  subroutine read_snapshot_old()
    ...
    double precision              :: wio(ixGlo1:ixGhi1,1:nprim)
    ...
    do iigrid = 1, igridstail
       ...
       do iw = 1, nprim
          wsum(iw) = wsum(iw) + sum(ps(igrid)%dvolume(ixMlo1:ixMhi1)*ps(&
             igrid)%prim(ixMlo1:ixMhi1,iw)**power)
       end do
    end do
  ...
  subroutine get_volume_average_func(func, f_avg, volume)
    ...
    do iigrid = 1, igridstail
       ...
       ! Compute the modes of the cell-centered variables, weighted by volume
       do i1 = ixMlo1, ixMhi1
       wsum(1) = wsum(1) + ps(igrid)%dvolume(i1) * func(ps(igrid)%prim(i1, :),&
           nprim)
       end do
    end do
