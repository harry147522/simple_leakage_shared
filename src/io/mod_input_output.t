...
108    close(unitpar)
  ...
  subroutine write_snapshot
    ...
    if(stagger_grid) then
      n_values = n_values + count_ix(ixGs^LL) * nprims
    end if
    ...
    do Morton_no=Morton_start(mype), Morton_stop(mype)
      ...
      if(stagger_grid) then
        ...
        {ixOsmax^D = ixOmax^D \}
        n_values_stagger= count_ix(ixOs^L)*nprims
        w_buffer(n_values+1:n_values+n_values_stagger) = pack(ps(igrid)%prims(ixOs^S, 1:nprims), .true.)
      ...
      do ipe = 0, npe-1
        do Morton_no=Morton_start(ipe),Morton_stop(ipe)
          ...
          if(stagger_grid) then
            ...
            n_values = n_values + count_ix(ixOs^L) * nprims
          end if
          ...
          if (mype == ipe) then ! Root task
            ...
            if(stagger_grid) then
              ...
              ps(igrid)%prims(ixOs^S,1:nprims)=reshape(w_buffer(n_values_stagger+1:n_values), &
                   shape(ws(ixOs^S, 1:nprims)))
            else 
