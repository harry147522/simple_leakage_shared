...
contains
  ...
  subroutine addsource1_grid(igrid,qdt,qt,w,src_active)
    ...
    double precision, intent(inout) :: w(ixG^T,nprim)
    logical, intent(inout) :: src_active

    double precision :: w1(ixG^T,nprim)
    ...
    w1(ixG^T,1:nprim)=w(ixG^T,1:nprim)

    select case (typesourcesplit)
    case ('sf')
       call addsource2(qdt  ,ixG^LL,ixM^LL,1,nprim,qt,w1,qt,w,&
            ps(igrid)%x,.true.,src_active)
    case ('sfs')
       call addsource2(qdt/2,ixG^LL,ixM^LL,1,nprim,qt,w1,qt,w,&
       ps(igrid)%x,.true.,src_active)
    case ('ssf')
       call addsource2(qdt/2,ixG^LL,ixG^LL,1,nprim,qt,w,qt,w1,&
            ps(igrid)%x,.true.,src_active)
       call addsource2(qdt  ,ixG^LL,ixM^LL,1,nprim,qt,w1,qt,w,&
            ps(igrid)%x,.true.,src_active)
    case ('ssfss')
       call addsource2(qdt/4,ixG^LL,ixG^LL,1,nprim,qt,w,qt,w1,&
            ps(igrid)%x,.true.,src_active)
       call addsource2(qdt/2,ixG^LL,ixM^LL,1,nprim,qt,w1,qt,w,&
            ps(igrid)%x,.true.,src_active)
    case default
  ...
  subroutine addsource2(qdt,ixI^L,ixO^L,iw^LIM,qtC,wCT,qt,&
    ...
    double precision, intent(in)     :: wCT(ixI^S,1:nprim), x(ixI^S,1:ndim)
    double precision, intent(inout)  :: w(ixI^S,1:ncons)
