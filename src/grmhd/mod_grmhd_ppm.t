...
module mod_grmhd_ppm
  ...
  ! Public methods
  public :: grmhd_ppm_init

contains

  !> Initialize the module
  subroutine grmhd_ppm_init()
    use mod_physics_ppm
    phys_ppm_flatcd => grmhd_ppm_flatcd
    phys_ppm_flatsh => grmhd_ppm_flatsh
  end subroutine grmhd_ppm_init

  subroutine grmhd_ppm_flatcd(rec_from, rec_to, ixI^L, ixO^L, ixL^L, ixR^L, w, d2w, drho, dp)
    ...
    call phys_get_csound2(ixI^L, ixO^L,w(ixI^S, 1:nprim), gamma_eos_tmp(ixI^S))
    ...
    dp(ixO^S) = gamma_eos_tmp(ixO^S) * abs(d2w_press(ixO^S)) &
                  / min( w(ixL^S, press_), w(ixR^S, press_) )
  end subroutine grmhd_ppm_flatcd

  subroutine grmhd_ppm_flatsh(ixI^L, ixO^L, ixLL^L, ixL^L, ixR^L, ixRR^L, idims, w, beta, z, dv)
    use mod_global_parameters
    ...
    call phys_get_csound2(ixI^L, ixO^L,w(ixI^S, 1:nprim), cs2(ixI^S))
  ...
  end subroutine grmhd_ppm_flatsh

end module mod_grmhd_ppm
