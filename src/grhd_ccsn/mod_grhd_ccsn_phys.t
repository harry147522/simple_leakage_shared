...
contains
  ...
  subroutine grhd_ccsn_read_params(files)
    ...
    namelist /grhd_ccsn_list/ evolve_hydro, {^NOONED oneDcore, r_core, } &
...
111    close(unitpar)
  ...
  subroutine grhd_ccsn_phys_init()
    ...
    {^NOONED call grhd_ccsn_phys_one_d_core_init() }
  ...
  subroutine grhd_ccsn_update_eos(ixI^L, ixO^L, prim)
    use mod_global_parameters
    use mod_eos
    ...
    {do ix^D = ixO^LIM^D \}
       call grhd_ccsn_update_eos_one_point(prim(ix^D,:))
    {enddo^D&\}
  end subroutine grhd_ccsn_update_eos

  !> Calculate cmax_idim within ixO^L
  subroutine grhd_ccsn_get_cmax(prim, x, ixI^L, ixO^L, idim, cmax)
    use mod_global_parameters
    ...
    call grhd_ccsn_get_lambda(ixI^L, ixO^L, idim, prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lambda(ixI^S,1:2))
  ...
  subroutine grhd_ccsn_get_csound2(ixI^L,ixO^L,prim,csound2)
    use mod_global_parameters
    use mod_eos
    integer, intent(in)             :: ixI^L, ixO^L
  ...
  subroutine grhd_ccsn_get_lfac2(ps_in,ixI^L,ixO^L,lfac2)
    use mod_global_parameters
  ...
  subroutine grhd_ccsn_get_tilde_S(ps_in,ixI^L,ixO^L,tilde_S)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    type(state), intent(in)         :: ps_in
  ...
  subroutine grhd_ccsn_get_dt(w, ixI^L, ixO^L, dtnew, dx^D, x)
    use mod_global_parameters
  ...
  subroutine grhd_ccsn_check_prim(ixI^L, ixO^L, prim, flag)
    use mod_global_parameters
    integer, intent(in)          :: ixI^L, ixO^L
