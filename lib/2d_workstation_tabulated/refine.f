...
subroutine prolong_grid(child_igrid,child_ipe,igrid,ipe)
  ...
  do ic1=1,2
    ...
    if (prolongation_method=="linear") then
       ...
       call prolong_2nd(ps(igrid),ixComin1,ixComin2,ixComax1,ixComax2,&
          ps(ichild), dxCo1,dxCo2,xComin1,xComin2,dxFi1,dxFi2,xFimin1,xFimin2,&
          igrid,ichild)
    else
       call prolong_1st(ps(igrid)%prim,ixComin1,ixComin2,ixComax1,ixComax2,&
          ps(ichild)%prim,ps(ichild)%x)
    end if
  end do
  end do
...
subroutine prolong_2nd(sCo,ixComin1,ixComin2,ixComax1,ixComax2,sFi,dxCo1,dxCo2,&
  ...
  do ixCo1 = ixCgmin1,ixCgmax1
     ...
     do ix1=ixFi1,ixFi1+1 
        ...
        wFi(ix1,ix2,1:nprim) = wCo(ixCo1,ixCo2,1:nprim) + (slope(1:nprim,&
           1)*eta1)+(slope(1:nprim,2)*eta2)
     end do
     end do
  end do
  end do
...
subroutine prolong_1st(wCo,ixComin1,ixComin2,ixComax1,ixComax2,wFi,xFi)
  ...
  double precision, intent(in) :: wCo(ixGlo1:ixGhi1,ixGlo2:ixGhi2,nprim),&
      xFi(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim)
  double precision, intent(out) :: wFi(ixGlo1:ixGhi1,ixGlo2:ixGhi2,nprim)
  ...
  do ixCo1 = ixComin1,ixComax1
     ixFi1=2*(ixCo1-ixComin1)+ixMlo1
     forall(iw=1:nprim) wFi(ixFi1:ixFi1+1,ixFi2:ixFi2+1,iw)=wCo(ixCo1,ixCo2,&
        iw)
  end do
  end do
