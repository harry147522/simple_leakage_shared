...
module mod_fix_conserve
   ...
   subroutine allocateBflux
     ...
     do iigrid=1,igridstail; igrid=igrids(iigrid);
       ...
       do iside=1,2
         ...
         case(neighbor_fine)
           allocate(pflux(iside,1,igrid)%flux(1,1:nx2,1:ncons))
           if(stagger_grid) allocate(pflux(iside,1,igrid)%edge(1,0:nx2,&
              1:ndim-1))
         case(neighbor_coarse)
           allocate(pflux(iside,1,igrid)%flux(1,1:nxCo2,1:ncons))
       ...
       do iside=1,2
         ...
         select case (neighbor_type(i1,i2,igrid))
         case(neighbor_fine)
           allocate(pflux(iside,2,igrid)%flux(1:nx1,1,1:ncons))
           if(stagger_grid) allocate(pflux(iside,2,igrid)%edge(0:nx1,1,&
              1:ndim-1))
         case(neighbor_coarse)
           allocate(pflux(iside,2,igrid)%flux(1:nxCo1,1,1:ncons))
 ...
 !opedit: skip over active/passive interface since flux for passive ones is 
           ...
           do ic1=1+int((1-i1)/2),2-int((1+i1)/2)
             ...
             ! remove coarse flux
             if (slab_uniform) then
               psb(igrid)%prim(ix,ixMlo2:ixMhi2,nw0:nw1) = psb(igrid)%prim(ix,&
                  ixMlo2:ixMhi2,nw0:nw1) -pflux(iside,1,igrid)%flux(1,:,&
                  1:nwfluxin)
             else
               do iw=nw0,nw1
                 psb(igrid)%prim(ix,ixMlo2:ixMhi2,iw)=psb(igrid)%prim(ix,&
           ...
           do ic1=1+int((1-i1)/2),2-int((1+i1)/2)
               ...
               if (ipe_neighbor==mype) then
                 iotherside=3-iside
                 if (slab_uniform) then
                   psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                      nw0:nw1) = psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                      nw0:nw1) + pflux(iotherside,1,ineighbor)%flux(:,:,&
                      1:nwfluxin)* CoFiratio
                 else
                   do iw=nw0,nw1
                     psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                        iw)=psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                 ...
                 if (slab_uniform) then
                   ...
                   psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                      ...
                      *reshape(source=recvbuffer(ibuf:ibufnext-1),&
                       shape=shape(psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                      nw0:nw1)))
                   ibuf=ibuf+isize(1)
                 else
                   ibufnext=ibuf+isize(1)
                   ...
                   do iw=nw0,nw1
                     psb(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
                        ...
                        iw) +reshape(source=recvbuffer(ibuf:ibufnext-1),&
                         shape=shape(psb(igrid)%prim(ixmin1:ixmax1,&
                        ixmin2:ixmax2,iw))) /ps(igrid)%dvolume(ixmin1:ixmax1,&
                        ixmin2:ixmax2)
                     ibuf=ibuf+nbuf
                   end do
                   ibuf=ibufnext
 ...
 !opedit: skip over active/passive interface since flux for passive ones is 
           ...
           do ic1=1+int((1-i1)/2),2-int((1+i1)/2)
             ...
             ! remove coarse flux
             if (slab_uniform) then
               psb(igrid)%prim(ixMlo1:ixMhi1,ix,&
                  nw0:nw1) = psb(igrid)%prim(ixMlo1:ixMhi1,ix,&
                  nw0:nw1) -pflux(iside,2,igrid)%flux(:,1,1:nwfluxin)
             else
               do iw=nw0,nw1
                 psb(igrid)%prim(ixMlo1:ixMhi1,ix,&
                    iw)=psb(igrid)%prim(ixMlo1:ixMhi1,ix,iw)-pflux(iside,2,&
                    igrid)%flux(:,1,iw-nw0+1) /ps(igrid)%dvolume(ixMlo1:ixMhi1,&
                    ix)
               end do
             end if
   ...
   subroutine add_sub_circ(ixFmin1,ixFmin2,ixFmax1,ixFmax2,ixtEmin1,ixtEmin2,&
     ...
     do idim1=1,ndim
        ...
        if (add) then
          s%prims(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             idim1)=s%prims(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             idim1)-circ(ixCmin1:ixCmax1,ixCmin2:ixCmax2,idim1)
        else
          s%prims(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             idim1)=s%prims(ixCmin1:ixCmax1,ixCmin2:ixCmax2,&
             idim1)+circ(ixCmin1:ixCmax1,ixCmin2:ixCmax2,idim1)
