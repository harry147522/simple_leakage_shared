module mod_grhd_ccsn_leakage_mpi_phys_flux
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_mpi_phys_flux_init

contains
  !> Initialize the module
  subroutine grhd_ccsn_leakage_mpi_phys_flux_init()
    phys_get_cbounds         => grhd_ccsn_leakage_mpi_get_cbounds
    phys_get_flux            => grhd_ccsn_leakage_mpi_get_flux
  end subroutine grhd_ccsn_leakage_mpi_phys_flux_init

  !> Calculate cmax_idim = csound + abs(v_idim) within ixO^L
  subroutine grhd_ccsn_leakage_mpi_get_cbounds(consL, consR, primL, primR, x,&
      ixImin1,ixImax1, ixOmin1,ixOmax1, idim, cmax, cmin)
    ...
    call grhd_ccsn_leakage_mpi_get_lambda(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        idim, primL(ixImin1:ixImax1, 1:nprim), x(ixImin1:ixImax1, 1:ndim),&
        lambdaL(ixImin1:ixImax1,1:2))
    call grhd_ccsn_leakage_mpi_get_lambda(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        idim, primR(ixImin1:ixImax1, 1:nprim), x(ixImin1:ixImax1, 1:ndim),&
  ...
  end subroutine grhd_ccsn_leakage_mpi_get_cbounds

  ! Calculate flux f_idim[iw]
  subroutine grhd_ccsn_leakage_mpi_get_flux(cons, prim, x, ixImin1,ixImax1,&
      ixOmin1,ixOmax1, idim, f)
    use mod_global_parameters
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixImin1,ixImax1,&
        ...
        1:ndim), lfac=lfac(ixImin1:ixImax1) )
...
!       f(ixO^S, leak_tau(1:3)) = 0.0d0
       f(ixOmin1:ixOmax1, coolingsource(:)) = 0.0d0
    else
...
!       f(ixO^S, leak_tau(1:3)) = 0.0d0
  ...
  end subroutine grhd_ccsn_leakage_mpi_get_flux

end module mod_grhd_ccsn_leakage_mpi_phys_flux
