module mod_physics_ppm
  ...
  abstract interface
     subroutine sub_ppm_flatcd(rec_from,rec_to,ixI^L,ixO^L,ixL^L,ixR^L,w,d2w,drho,dp)
...
contains
  ...
  subroutine dummy_ppm_flatcd(rec_from,rec_to,ixI^L,ixO^L,ixL^L,ixR^L,w,d2w,drho,dp)
    use mod_global_parameters
    integer, intent(in)             :: rec_from,rec_to,ixI^L,ixO^L,ixL^L,ixR^L
    double precision, intent(in)    :: w(ixI^S,1:nprim),d2w(ixG^T,rec_from:rec_to)
    double precision, intent(inout) :: drho(ixI^S),dp(ixI^S)
