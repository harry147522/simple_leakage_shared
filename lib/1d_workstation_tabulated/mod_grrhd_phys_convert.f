...
contains
  ...
  subroutine grrhd_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    ...
    if ( fix_small_values ) then
       call grrhd_handle_small_values(prim(ixImin1:ixImax1, 1:nprim),&
           x(ixImin1:ixImax1, 1:ndim), ixImin1,ixImax1, ixOmin1,ixOmax1,&
           .True., 'grrhd_to_conserved')
    end if
    ...
    cons(ixOmin1:ixOmax1, nu_Econs) = prim(ixOmin1:ixOmax1, nu_E) 
    ! covariant momentum
    ! F_i/E -> F_i
    do idir = 1, ndir
       cons(ixOmin1:ixOmax1, nu_Fcons(idir)) = prim(ixOmin1:ixOmax1,&
           nu_F_over_E(idir)) * prim(ixOmin1:ixOmax1, nu_E) 
    end do
  end subroutine grrhd_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grrhd_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1, cons, prim,&
      x)
    use mod_global_parameters
    use mod_geometry
    ...
    double precision                :: zeta(ixImin1:ixImax1)
    ...
    ! fixme: conformal factor
    ! Conserved energy density nu_E
    prim(ixOmin1:ixOmax1, nu_E) = cons(ixOmin1:ixOmax1, nu_Econs)
    ! covariant momentum
    ! F_i/E -> F_i
    do idir = 1, ndir
       prim(ixOmin1:ixOmax1, nu_F_over_E(idir)) = cons(ixOmin1:ixOmax1,&
           nu_Fcons(idir)) / prim(ixOmin1:ixOmax1, nu_E) 
    end do
    ...
    if ( use_radiation ) then
      call grrhd_get_nu_zeta(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x)
    end if 
