module mod_grhd_ccsn_ILEAS_phys_flux
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_ILEAS_phys_flux_init

contains
  !> Initialize the module
  subroutine grhd_ccsn_ILEAS_phys_flux_init()
    phys_get_cbounds         => grhd_ccsn_ILEAS_get_cbounds
    phys_get_flux            => grhd_ccsn_ILEAS_get_flux
  end subroutine grhd_ccsn_ILEAS_phys_flux_init

  !> Calculate cmax_idim = csound + abs(v_idim) within ixO^L
  subroutine grhd_ccsn_ILEAS_get_cbounds(consL, consR, primL, primR, x, ixI^L, ixO^L, idim, cmax, cmin)
    ...
    call grhd_ccsn_ILEAS_get_lambda(ixI^L, ixO^L, idim, primL(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lambdaL(ixI^S,1:2))
    call grhd_ccsn_ILEAS_get_lambda(ixI^L, ixO^L, idim, primR(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lambdaR(ixI^S,1:2))
  ...
  end subroutine grhd_ccsn_ILEAS_get_cbounds

  ! Calculate flux f_idim[iw]
  subroutine grhd_ccsn_ILEAS_get_flux(cons, prim, x, ixI^L, ixO^L, idim, f)
    use mod_global_parameters
...
!       f(ixO^S, leak_tau(1:3)) = 0.0d0
  ...
  end subroutine grhd_ccsn_ILEAS_get_flux

end module mod_grhd_ccsn_ILEAS_phys_flux
