...
module mod_grhd_ccsn_leakage_mpi_phys
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_mpi_phys_init

contains

  !> Read this module's parameters from a file
  subroutine grhd_ccsn_leakage_mpi_read_params(files)
    use mod_global_parameters
    character(len=*), intent(in) :: files(:)
    integer                      :: n

    namelist /grhd_ccsn_leakage_mpi_list/ evolve_hydro,  use_GR, tolerance,&
        iter_max, lfac_max

    do n = 1, size(files)
       rewind(unitpar)
       open(unitpar, file=trim(files(n)), status="old")
       read(unitpar, grhd_ccsn_leakage_mpi_list, end=111)
111    close(unitpar)
    end do

  end subroutine grhd_ccsn_leakage_mpi_read_params

  !> Write this module's parameters to a snapsoht
  subroutine grhd_ccsn_leakage_mpi_write_info(fh)
    use mod_global_parameters
    ...
    names(1) = "gamma"
    values(1) = 1.0d0!grhd_ccsn_leakage_mpi_gamma
    call MPI_FILE_WRITE(fh, values, n_par, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, names, n_par * name_len, MPI_CHARACTER, st, er)
  end subroutine grhd_ccsn_leakage_mpi_write_info


  !> Initialize the module
  subroutine grhd_ccsn_leakage_mpi_phys_init()
    use mod_global_parameters
    use mod_grhd_ccsn_leakage_mpi_phys_convert
    use mod_grhd_ccsn_leakage_mpi_phys_flux
    use mod_grhd_ccsn_leakage_mpi_phys_add_source
    use mod_grhd_ccsn_leakage_mpi_phys_one_d_core

    integer :: itr, idir, i, neu

    call grhd_ccsn_leakage_mpi_read_params(par_files)

    physics_type = "grhd_ccsn_leakage_mpi"
    ...
    allocate(leaktau(3))
    do neu = 1, 3
       leaktau(neu) = var_set_primvar('leaktau', neu, var_type=hydro_var,&
           need_bc=.false., need_rec=.false.)
    enddo

    allocate(chi(3))
    do neu = 1,3 
       chi(neu) = var_set_primvar('chi', neu, var_type=hydro_var,&
           need_bc=.false., need_rec=.false.)
    enddo
    allocate(zeta(3))
    do neu = 1,3 
       zeta(neu) = var_set_primvar('zeta', neu, var_type=hydro_var,&
           need_bc=.false., need_rec=.false.)
    enddo
...
!  idir is mom(idir), idir+1 is tau, idir+2 is ye _con
    ...
    do idir = 1, ndir+2
      coolingsource(idir) = var_set_consvar('coolingsource', idir,&
          var_type=hydro_var)
    enddo

    phys_get_dt              => grhd_ccsn_leakage_mpi_get_dt
    phys_get_lfac2           => grhd_ccsn_leakage_mpi_get_lfac2
    phys_get_tilde_S         => grhd_ccsn_leakage_mpi_get_tilde_S
    phys_get_csound2         => grhd_ccsn_leakage_mpi_get_csound2
    phys_get_cmax            => grhd_ccsn_leakage_mpi_get_cmax
    phys_update_eos          => grhd_ccsn_leakage_mpi_update_eos
    phys_check_params        => grhd_ccsn_leakage_mpi_check_params
    phys_check_prim          => grhd_ccsn_leakage_mpi_check_prim
    phys_write_info          => grhd_ccsn_leakage_mpi_write_info
    phys_handle_small_values => grhd_ccsn_leakage_mpi_handle_small_values

    call grhd_ccsn_leakage_mpi_phys_convert_init()
    call grhd_ccsn_leakage_mpi_phys_flux_init()
    call grhd_ccsn_leakage_mpi_phys_add_source_init()
  ...
  end subroutine grhd_ccsn_leakage_mpi_phys_init

  subroutine grhd_ccsn_leakage_mpi_check_params
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_leakage_mpi_check_params

  !> Update the eos (prim) variables, p, temp, entropy, cs2
  subroutine grhd_ccsn_leakage_mpi_update_eos(ixImin1,ixImax1, ixOmin1,ixOmax1,&
      prim)
    use mod_global_parameters
    use mod_eos
    ...
    do ix1 = ixOmin1,ixOmax1 
       call grhd_ccsn_leakage_mpi_update_eos_one_point(prim(ix1,:))
    enddo
  end subroutine grhd_ccsn_leakage_mpi_update_eos

  !> Calculate cmax_idim within ixO^L
  subroutine grhd_ccsn_leakage_mpi_get_cmax(prim, x, ixImin1,ixImax1, ixOmin1,&
     ixOmax1, idim, cmax)
    use mod_global_parameters
    integer, intent(in)                       :: ixImin1,ixImax1, ixOmin1,&
       ixOmax1, idim
    ...
    call grhd_ccsn_leakage_mpi_get_lambda(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        idim, prim(ixImin1:ixImax1, 1:nprim), x(ixImin1:ixImax1, 1:ndim),&
        lambda(ixImin1:ixImax1,1:2))
    cmax(ixOmin1:ixOmax1) = max(abs(lambda(ixOmin1:ixOmax1,1)),&
        abs(lambda(ixOmin1:ixOmax1,2)))

    
  end subroutine grhd_ccsn_leakage_mpi_get_cmax

  subroutine grhd_ccsn_leakage_mpi_get_csound2(ixImin1,ixImax1,ixOmin1,ixOmax1,&
     prim,csound2)
    use mod_global_parameters
    use mod_eos
    ...
    enddo
  end subroutine grhd_ccsn_leakage_mpi_get_csound2

  subroutine grhd_ccsn_leakage_mpi_get_lfac2(ps_in,ixImin1,ixImax1,ixOmin1,&
     ixOmax1,lfac2)
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_leakage_mpi_get_lfac2

  ! get psi^6 (\gamma_{ij}S^{ij}), which is part of the source terms in cfc_alp
  subroutine grhd_ccsn_leakage_mpi_get_tilde_S(ps_in,ixImin1,ixImax1,ixOmin1,&
     ixOmax1,tilde_S)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    type(state), intent(in)         :: ps_in
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixImin1,ixImax1,&
        ...
        1:ndim), gamma=gamma(ixImin1:ixImax1,1:3,1:3), h=h(ixImin1:ixImax1) )
  ...
  end subroutine grhd_ccsn_leakage_mpi_get_tilde_S

  subroutine grhd_ccsn_leakage_mpi_get_dt(w, ixImin1,ixImax1, ixOmin1,ixOmax1,&
      dtnew, dx1, x)
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_leakage_mpi_get_dt

  !> Returns 0 in argument flag where values are ok
  subroutine grhd_ccsn_leakage_mpi_check_prim(ixImin1,ixImax1, ixOmin1,ixOmax1,&
      prim, flag)
    use mod_global_parameters
    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1
  ...
  end subroutine grhd_ccsn_leakage_mpi_check_prim
end module mod_grhd_ccsn_leakage_mpi_phys
