...
contains
  ...
  subroutine timeintegration()
    ...
    time_evol : do
         ...
         if(associated(usr_modify_output)) then
           ...
           do iigrid=1,igridstail; igrid=igrids(iigrid);
             ...
             call usr_modify_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
                ixMhi1,ixMhi2,global_time,ps(igrid)%prim,ps(igrid)%x)
           end do
         end if
         time_write=0.d0
         do ifile=nfile,1,-1
       ...
       if (it>=it_max .or. global_time>=time_max .or. pass_wall_time) exit &
          time_evol
       ...
       if (crashall) then
         ...
         call saveamrfile(2)
         if(mype==0) write(*,*)&
             "Error: small value/ NaN encountered, run crash."
    ...
    if (mype==0) then
       ...
       write(*,'(a,f12.3,a)')' Time spent on Regrid+Update: ',timegr_tot,&
          ' sec'
       ...
       write(*,'(a,f12.3,a)')' Time spent on run          : ',&
          timeloop-timeio_tot,' sec'
       write(*,'(a,es12.3 )')' Cells_updated / cpu / sec  : ',&
