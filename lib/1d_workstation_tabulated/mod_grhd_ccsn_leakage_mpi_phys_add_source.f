...
contains
  ...
  subroutine grhd_ccsn_leakage_mpi_add_source(qdt, ixImin1,ixImax1, ixOmin1,&
...
!    use mod_grhd_ccsn_leakage_mpi_simple_leakage, only : bounce
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixImin1,ixImax1,&
        ixOmin1,ixOmax1, primCT(ixImin1:ixImax1, 1:nprim), x(ixImin1:ixImax1,&
...
!write(*,*) add_source(5,tau_) , x(5,r_), qdt,"normal ource"
  ...
  subroutine grhd_ccsn_leakage_mpi_add_source_geom(qdt, ixImin1,ixImax1,&
      ixOmin1,ixOmax1, consCT, primCT, cons, x)
    use mod_global_parameters
    use mod_geometry
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision, intent(in)    :: qdt, x(ixImin1:ixImax1, 1:ndim)
