...
integer             :: filenr,ncells,ncells1,ncellg,ncellx1,jg1,jig1
...
double precision, dimension(1:1,1:nprim+nwauxio)   :: wval
...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   ...
   ps1(igrid)%prim(ixGlo1:ixGhi1,1:nprim)=ps(igrid)%prim(ixGlo1:ixGhi1,&
      1:nprim)
   ...
   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixGlo1,ixGhi1,ixMlo1-1,ixMhi1+1, ps1(igrid)%prim,&
            ps(igrid)%x,normconv)
      end if
   end if
end do
...
double precision  :: w_recv(ixGlo1:ixGhi1,1:nprim),x_recv(ixGlo1:ixGhi1,&
   1:ndim)
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if (mype/=0)then
   ...
   do ix1=ixMlo1,ixMhi1
      do iw=1,nprim
        if( dabs(ps(igrid)%prim(ix1,iw)) < 1.0d-32 ) ps(igrid)%prim(ix1,&
           iw) = zero
      enddo
       write(qunit,fmt="(100(e14.6))") ps(igrid)%x(ix1,1:ndim),&
          ps(igrid)%prim(ix1,1:nprim)
   end do
  end if
end do   
 ...
 if (mype==0) then
  loop_cpu : do ipe =1, npe-1
   loop_Morton : do Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixMlo1,ixMhi1
            do iw=1,nprim
              if( dabs(ps(igrid)%prim(ix1,&
                 iw)) < smalldouble ) ps(igrid)%prim(ix1,iw) = zero
            enddo
      ...
      call calc_grid(qunit,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,&
         ixCmin1,ixCmax1,ixCCmin1,ixCCmax1,.true.)
          do ix1=ixCCmin1,ixCCmax1
            ...
            w_TEC(1:nprim+nwauxio)=wCC_TMP(ix1,&
               1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
...
do level=levmin,levmax
   ...
   select case(convert_type)
     case('tecplot')
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         do ix1=ixCmin1,ixCmax1
            ...
            w_TEC(1:nprim+nwauxio)=wC_TMP(ix1,&
               1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
            write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         end do
       enddo
     case('tecplotCC')
       ...
       if(ndim+nprim+nwauxio>99) call mpistop(&
       ...
       if(nprim+nwauxio==1)then
        ...
        if(ndim+nprim+nwauxio<10) then
         ...
         write(qunit,"(a,i7,a,a,i7,a,i7,a,f25.16,a,i1,a,i1,a,a)") 'ZONE T="',&
            ...
            ', DATAPACKING=BLOCK, VARLOCATION=([', ndim+1,'-',&
            ndim+nprim+nwauxio,']=CELLCENTERED), ZONETYPE=',  'FELINESEG'
        else
...
do level=levmin,levmax
   ...
   do Morton_no=Morton_start(mype),Morton_stop(mype)
    ...
    if (mype/=0) then
       ...
       siz_ind=4*1
       call MPI_SEND(ind_send,siz_ind,MPI_INTEGER, 0,itag,icomm,ierrmpi)
       call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION, 0,itag,&
          icomm,ierrmpi)
...
character(len=name_len)   :: wnamei(1:nprim+nwauxio),&
   xandwnamei(1:ndim+nprim+nwauxio),outtype
character(len=1024) :: outfilehead
       ...
       do ipe=1,npe-1
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixCCmin1,ixCCmax1
             ...
             w_TEC(1:nprim+nwauxio)=wCC_TMP_recv(ix1,&
                1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
             write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         end do
        end do 
     ...
     case('tecplotmpi')
      ...
      do Morton_no=Morton_start(mype),Morton_stop(mype)
         ...
         if (mype/=0) then
            ...
            siz_ind=2*1
            call MPI_SEND(ind_send,siz_ind, MPI_INTEGER, 0,itag,icomm,ierrmpi)
            call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION, 0,&
               itag,icomm,ierrmpi)
     ...
     case('tecplotmpi')
        ...
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixrvCmin1,ixrvCmax1
              ...
              w_TEC(1:nprim+nwauxio)=wC_TMP_RECV(ix1,&
                 1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
              write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         end do
        end do
