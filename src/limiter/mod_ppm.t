...
contains
  ...
  subroutine PPMlimiter(rec_from,rec_to,ixI^L,ix^L,idims,w,wCT,wLC,wRC,fattening)
    ...
    integer, intent(in)             :: ixI^L, ix^L, idims
    double precision, intent(in)    :: w(ixI^S,1:nprim),wCT(ixI^S,1:nprim)

    double precision, intent(inout) :: wRC(ixG^T,1:nprim),wLC(ixG^T,1:nprim) 
    logical, intent(in)             :: fattening

    double precision,dimension(ixG^T,rec_from:rec_to)  :: dwC,d2wC,ldw
    double precision,dimension(ixG^T,rec_from:rec_to)  :: wMin,wMax,tmp
    ...
    dwC(lxC^S,rec_from:rec_to)=w(lxR^S,rec_from:rec_to)-w(lxC^S,rec_from:rec_to)
    ! Eq. 64,  Miller and Colella 2002, JCP 183, 26 
    d2wC(kxC^S,rec_from:rec_to)=half*(w(kxR^S,rec_from:rec_to)-w(kxL^S,rec_from:rec_to))
    where(dwC(kxC^S,rec_from:rec_to)*dwC(kxL^S,rec_from:rec_to)>zero)
       ! Store the sign of dwC in wMin
       wMin(kxC^S,rec_from:rec_to)= sign(one,d2wC(kxC^S,rec_from:rec_to))
       ! Eq. 65,  Miller and Colella 2002, JCP 183, 26 
       ldw(kxC^S,rec_from:rec_to)= wMin(kxC^S,rec_from:rec_to)*min(dabs(d2wC(kxC^S,rec_from:rec_to)),&
            2.0d0*dabs(dwC(kxL^S,rec_from:rec_to)),&
            2.0d0*dabs(dwC(kxC^S,rec_from:rec_to)))
    elsewhere
       ldw(kxC^S,rec_from:rec_to)=zero
    endwhere

    ! Eq. 66,  Miller and Colella 2002, JCP 183, 26 
    wLC(ixO^S,rec_from:rec_to)=wLC(ixO^S,rec_from:rec_to)+half*dwC(ixO^S,rec_from:rec_to)&
         +(ldw(ixO^S,rec_from:rec_to)-ldw(ixR^S,rec_from:rec_to))/6.0d0

    wRC(ixL^S,rec_from:rec_to)=wRC(ixL^S,rec_from:rec_to)-(half*dwC(ixL^S,rec_from:rec_to)&
         -(ldw(ixL^S,rec_from:rec_to)-ldw(ixO^S,rec_from:rec_to))/6.0d0)

    ! make sure that min wCT(i)<wLC(i)<wCT(i+1) same for wRC(i)
    call extremaw(rec_from,rec_to,ixI^L,kxC^L,wCT,1,wMax,wMin)

    ! Eq. B8, page 217, Mignone et al 2005, ApJS
    wRC(ixL^S,rec_from:rec_to)=max(wMin(ixO^S,rec_from:rec_to)&
         ,min(wMax(ixO^S,rec_from:rec_to),wRC(ixL^S,rec_from:rec_to))) 
    wLC(ixO^S,rec_from:rec_to)=max(wMin(ixO^S,rec_from:rec_to)&
         ,min(wMax(ixO^S,rec_from:rec_to),wLC(ixO^S,rec_from:rec_to)))


    ! Eq. B9, page 217, Mignone et al 2005, ApJS
    where((wRC(ixL^S,rec_from:rec_to)-wCT(ixO^S,rec_from:rec_to))&
         *(wCT(ixO^S,rec_from:rec_to)-wLC(ixO^S,rec_from:rec_to))<=zero)
       wRC(ixL^S,rec_from:rec_to)=wCT(ixO^S,rec_from:rec_to)
       wLC(ixO^S,rec_from:rec_to)=wCT(ixO^S,rec_from:rec_to)
    end where

    wMax(ixO^S,rec_from:rec_to)=(wLC(ixO^S,rec_from:rec_to)-wRC(ixL^S,rec_from:rec_to))*&
         (wCT(ixO^S,rec_from:rec_to)-&
         (wLC(ixO^S,rec_from:rec_to)+wRC(ixL^S,rec_from:rec_to))/2.0d0)
    wMin(ixO^S,rec_from:rec_to)=(wLC(ixO^S,rec_from:rec_to)-wRC(ixL^S,rec_from:rec_to))**2.0d0/6.0d0
    tmp(ixL^S,rec_from:rec_to)=wRC(ixL^S,rec_from:rec_to)
    ! Eq. B10, page 218, Mignone et al 2005, ApJS
    where(wMax(hxR^S,rec_from:rec_to)>wMin(hxR^S,rec_from:rec_to))
       wRC(hxC^S,rec_from:rec_to)= 3.0d0*wCT(hxR^S,rec_from:rec_to)&
            -2.0d0*wLC(hxR^S,rec_from:rec_to)
    endwhere
    ! Eq. B11, page 218, Mignone et al 2005, ApJS
    where(wMax(hxC^S,rec_from:rec_to)<-wMin(hxC^S,rec_from:rec_to))
       wLC(hxC^S,rec_from:rec_to)= 3.0d0*wCT(hxC^S,rec_from:rec_to)&
            -2.0d0*tmp(hxL^S,rec_from:rec_to)
    endwhere

    ! flattening at the contact discontinuities
    if(flatcd.and.fattening)then
       ! Note that aa here is drho, ab is dpress
       call phys_ppm_flatcd(rec_from,rec_to,ixI^L,kxC^L,kxL^L,kxR^L,wCT,d2wC,aa,ab)
  ...
  subroutine extremaw(rec_from,rec_to,ixI^L,ixO^L,w,nshift,wMax,wMin)
    use mod_global_parameters
    ...
    double precision, intent(out) :: wMax(ixI^S,rec_from:rec_to),wMin(ixI^S,rec_from:rec_to)
    ...
    do ishift=1,nshift
      ...
      ixsL^L=ixO^L-ishift*kr(idims,^D);
      if (ishift==1) then
        wMax(ixO^S,rec_from:rec_to)= &
             max(w(ixO^S,rec_from:rec_to),w(ixsR^S,rec_from:rec_to),w(ixsL^S,rec_from:rec_to))
        wMin(ixO^S,rec_from:rec_to)= &
             min(w(ixO^S,rec_from:rec_to),w(ixsR^S,rec_from:rec_to),w(ixsL^S,rec_from:rec_to))
      else
        wMax(ixO^S,rec_from:rec_to)= &
             max(wMax(ixO^S,rec_from:rec_to),w(ixsR^S,rec_from:rec_to),w(ixsL^S,rec_from:rec_to))
        wMin(ixO^S,rec_from:rec_to)= &
             min(wMin(ixO^S,rec_from:rec_to),w(ixsR^S,rec_from:rec_to),w(ixsL^S,rec_from:rec_to))
      end if
      {^NOONED
      idims=1
      jdims=idims+1
