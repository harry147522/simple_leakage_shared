...
if (mype == 0) then 
   ...
   allocate(woneblock(nc1,1:nprim))
   
   do ix1=1,nc1  
   
   read(unit,*) xoneblock(ix1,1:1), woneblock(ix1,1:nprim)
...
if (npe>1) then 
   ...
   call MPI_BCAST(woneblock,nc1*nprim,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
end if! npe>1
