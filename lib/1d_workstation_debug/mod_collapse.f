...
collapsedData(1:nprim+nwauxio) = collapsedData(1:nprim+nwauxio) + &
   ps_sub(jgrid)%prim(1:nprim+nwauxio)
...
if(slab_uniform) then
  do ix=ixMlo1,ixMhi1
     ps_sub(jgrid)%prim(1:nprim+nwauxio) = ps_sub(jgrid)%prim(1:nprim+nwauxio) &
        + wCC_TMP(ix,1:nprim+nwauxio) * dx1
  end do
else
  do iw=1,nprim+nwauxio
    do ix=ixMlo1,ixMhi1
      ps_sub(jgrid)%prim(iw) = ps_sub(jgrid)%prim(iw) + wCC_TMP(ix,&
         iw) * ps(igrid)%dx(ix,1)
    end do
  end do
end if
