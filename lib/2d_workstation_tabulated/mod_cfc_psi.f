...
("Error: The boundary conditions of psi/alp are not set correctly.")
    ...
    do iigrid = 1, igridstail
       ...
       mg%boxes(id)%cc(0:nc+1,0:nc+1, mg_iphi) = (ps(igrid)%prim(ixmin1:ixmax1,&
          ixmin2:ixmax2, psi_) - 1.0d0)

       ! source term 1
       src1(ixMlo1:ixMhi1,ixMlo2:ixMhi2) = 2.0d0 * dpi * ( &
          ps(igrid)%cons(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
           tau_) + ps(igrid)%cons(ixMlo1:ixMhi1,ixMlo2:ixMhi2, D_) ) 
       mg%boxes(id)%cc(1:nc,1:nc, mg_itmp1) = src1(ixMlo1:ixMhi1,&
          ixMlo2:ixMhi2)

       ! source term 2, A2
       mg%boxes(id)%cc(1:nc,1:nc, mg_itmp2) = 1.25d-1 * A2(ixMlo1:ixMhi1,&
          ixMlo2:ixMhi2, iigrid)
    ...
    do iigrid = 1, igridstail
       ...
       ps(igrid)%prim(ixmin1:ixmax1,ixmin2:ixmax2,&
           psi_) = mg%boxes(id)%cc(0:nc+1,0:nc+1, mg_iphi) + 1.0d0
    end do
