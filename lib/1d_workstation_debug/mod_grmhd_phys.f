...
111    close(unitpar)
  ...
  subroutine grmhd_update_eos(ixImin1,ixImax1, ixOmin1,ixOmax1, prim)
    use mod_global_parameters
    ...
    do ix1 = ixOmin1,ixOmax1 
       call grmhd_update_eos_one_point(prim(ix1, :))
    enddo
  end subroutine grmhd_update_eos

  !> Calculate cmax_idim within ixO^L
  subroutine grmhd_get_cmax(prim, x, ixImin1,ixImax1, ixOmin1,ixOmax1, idim,&
      cmax)
    use mod_global_parameters
    integer, intent(in)                       :: ixImin1,ixImax1, ixOmin1,&
       ixOmax1, idim
    ...
    integer                                   :: idir, ix1
    double precision                          :: lambda(ixImin1:ixImax1,1:3)
  ...
  subroutine grmhd_get_csound2(ixImin1,ixImax1,ixOmin1,ixOmax1,prim,csound2)
    use mod_global_parameters
    use mod_eos
  ...
  subroutine grmhd_get_lfac2(ps_in,ixImin1,ixImax1,ixOmin1,ixOmax1,lfac2)
    use mod_global_parameters
  ...
  subroutine grmhd_get_tilde_S(ps_in,ixImin1,ixImax1,ixOmin1,ixOmax1,tilde_S)
    use mod_global_parameters
    use mod_geometry, only: get_gamma_ij_hat
    ...
    call grmhd_get_intermediate_variables(ixImin1,ixImax1, ixOmin1,ixOmax1,&
        ...
        B_dot_v=B_dot_v(ixImin1:ixImax1), b2=b2(ixImin1:ixImax1),&
        Ptot=Ptot(ixImin1:ixImax1), htot=htot(ixImin1:ixImax1) )

    ! nota that S = S_i v^i + 3 p* - b^2
    tilde_S(ixOmin1:ixOmax1) = 3.0d0 * Ptot(ixOmin1:ixOmax1) - &
       b2(ixOmin1:ixOmax1)
    do idir = 1, ndir
       tilde_S(ixOmin1:ixOmax1) = tilde_S(ixOmin1:ixOmax1) + ( ( &
          prim(ixOmin1:ixOmax1, rho_) * htot(ixOmin1:ixOmax1) - &
          B_dot_v(ixOmin1:ixOmax1)**2 ) * prim(ixOmin1:ixOmax1,&
           W_vel(idir)) - B_dot_v(ixOmin1:ixOmax1) * prim(ixOmin1:ixOmax1,&
           Bvec(idir)) / lfac(ixOmin1:ixOmax1) ) * gamma(ixOmin1:ixOmax1,idir,&
  ...
  subroutine grmhd_get_dt(w, ixImin1,ixImax1, ixOmin1,ixOmax1, dtnew, dx1, x)
    use mod_global_parameters
  ...
  subroutine grmhd_check_prim(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, flag)
    use mod_global_parameters
    integer, intent(in)          :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision, intent(in) :: prim(ixImin1:ixImax1, 1:nprim)
  ...
  subroutine grmhd_handle_small_values(prim, x, ixImin1,ixImax1, ixOmin1,&
    ...
    case ("replace")
       ...
       do ix1 = ixOmin1,ixOmax1 
          ...
          if ( need_to_fix_eos .and. update_eos ) then
             call grmhd_update_eos_one_point(prim(ix1,1:nprim))
          end if
       enddo
    case default
       ! nothing to do here
  ...
  subroutine grmhd_modify_wLR(ixImin1,ixImax1,ixOmin1,ixOmax1,qt,wLC,wRC,wLp,&
     wRp,s,idir)
    use mod_global_parameters
    use mod_usr_methods
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, idir
    double precision, intent(in)    :: qt
    double precision, intent(inout) :: wLC(ixImin1:ixImax1,1:nprim),&
        wRC(ixImin1:ixImax1,1:nprim)
    double precision, intent(inout) :: wLp(ixImin1:ixImax1,1:nprim),&
        wRp(ixImin1:ixImax1,1:nprim)
    type(state)                     :: s
    double precision                :: dB(ixImin1:ixImax1),&
        dPsi(ixImin1:ixImax1)

    if(stagger_grid) then
      wLC(ixOmin1:ixOmax1,Bvec(idir))=s%prims(ixOmin1:ixOmax1,idir)
      wRC(ixOmin1:ixOmax1,Bvec(idir))=s%prims(ixOmin1:ixOmax1,idir)
      wLp(ixOmin1:ixOmax1,Bvec(idir))=s%prims(ixOmin1:ixOmax1,idir)
      wRp(ixOmin1:ixOmax1,Bvec(idir))=s%prims(ixOmin1:ixOmax1,idir)
    end if
