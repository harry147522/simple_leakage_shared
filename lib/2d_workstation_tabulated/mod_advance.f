...
contains
  ...
  subroutine advance(iit)
    ...
    use mod_physics, only: phys_req_diagonal , phys_to_primitive,&
        phys_to_conserved , phys_step_adv_global 
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
       call phys_to_conserved(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixmin1,ixmin2,ixmax1,&
          ixmax2,ps(igrid)%cons(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ncons),&
          ps(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim),&
          ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim))
       pso(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
          1:nprim)=ps(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim)
       pso(igrid)%cons(ixmin1:ixmax1,ixmin2:ixmax2,&
          1:ncons)=ps(igrid)%cons(ixmin1:ixmax1,ixmin2:ixmax2,1:ncons)
       if(stagger_grid) pso(igrid)%prims=ps(igrid)%prims
    end do
    !$OMP END PARALLEL DO

! single core do
    if (associated(usr_nompi)) then
    ! leakage add source
    write(*,*) "read usr_nompi"
    call usr_nompi(iit, global_time)
    end if
...
!why still here
    ...
    if (cfc_update_flag()) then
       ...
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,ps(igrid)%cons(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ncons),&
             ps(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim),&
             ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim))
       end do
       !$OMP END PARALLEL DO
  ...
  subroutine advect(idimmin,idimmax)
    ...
    call init_comm_fix_conserve(idimmin,idimmax,ncons)
    ...
    case ("onestep")
       ...
       case ("Forward_Euler")
          call advect1(flux_scheme,1.0d0,idimmin,idimmax,global_time,ps1,&
             global_time,ps)

       case ("IMEX_Euler")
          call advect1(flux_scheme,1.0d0,idimmin,idimmax,global_time,ps,&
             global_time,ps1)
    ...
    case ("twostep")
       ...
       case ("Predictor_Corrector")
          ...
          call advect1(typepred1,0.5d0,idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          ! corrector step
          fix_conserve_at_step = time_advance .and. levmax>levmin
          call advect1(flux_scheme,1.0d0,idimmin,idimmax,global_time+0.5d0*dt,&
             ps1,global_time,ps)

       case ("RK2_alfa")
          ! RK2 with alfa parameter, where rk_a21=alfa
          call advect1(flux_scheme,rk_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons = ps(igrid)%cons+&
                rk_b1*(ps1(igrid)%cons-ps(igrid)%cons)/rk_a21
             if(stagger_grid) ps(igrid)%prims = &
                ps(igrid)%prims+(one-rk_b2)*(ps1(igrid)%prims-&
                ps(igrid)%prims)/rk_a21
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_b2,idimmin,idimmax,global_time+rk_a21*dt,&
       ...
       case ("ssprk2")
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%prims = &
                0.5d0*ps(igrid)%prims+0.5d0*ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,0.5d0,idimmin,idimmax,global_time+dt,ps1,&
             global_time+0.5d0*dt,ps)
       ...
       case ("IMEX_Midpoint")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          call global_implicit_update(0.5d0,dt,global_time+0.5d0*dt,ps2,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons = ps(igrid)%cons+&
                2.0d0*(ps2(igrid)%cons-ps1(igrid)%cons)
             if(stagger_grid) ps(igrid)%prims = &
                ps(igrid)%prims+2.0d0*(ps2(igrid)%prims-ps1(igrid)%prims)
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,1.0d0, idimmin,idimmax,global_time+0.5d0*dt,&
             ps2,global_time,ps)
       ...
       case ("IMEX_Trapezoidal")
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps2(igrid)%prims = &
                0.5d0*(ps(igrid)%prims+ps1(igrid)%prims)
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%prims = &
                ps1(igrid)%prims+0.5d0*dt*ps(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%prims = &
                ps2(igrid)%prims+0.5d0*dt*ps(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%prims = &
                ps(igrid)%prims+ps2(igrid)%prims-ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time+dt,ps2,&
             global_time+0.5d0*dt,ps)
    ...
    case ("threestep")
       ...
       case ("ssprk3")
          ...
          call advect1(flux_scheme,rk_beta11, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%cons=rk_alfa21*ps(igrid)%cons+&
                rk_alfa22*ps1(igrid)%cons
             if(stagger_grid) ps2(igrid)%prims=rk_alfa21*ps(igrid)%prims+&
                rk_alfa22*ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta22, idimmin,idimmax,&
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%prims=rk_alfa31*ps(igrid)%prims+&
                rk_alfa33*ps2(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta33, idimmin,idimmax,&
             global_time+rk_c3*dt,ps2,global_time+(1.0d0-rk_beta33)*dt,ps)

       case ("RK3_BT")
          ! this is a general threestep RK according to its Butcher Table
          call advect1(flux_scheme,rk3_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%cons=(ps1(igrid)%cons-ps(igrid)%cons)/rk3_a21
             if(stagger_grid) ps3(igrid)%prims=(ps1(igrid)%prims-&
                ps(igrid)%prims)/rk3_a21
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps2(igrid)%prims=ps(igrid)%prims+&
                rk3_a31*ps3(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk3_a32, idimmin,idimmax,&
             global_time+rk3_c2*dt,ps1,global_time+rk3_a31*dt,ps2)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons=ps(igrid)%cons+rk3_b1*ps3(igrid)%cons &
                +rk3_b2*(ps2(igrid)%cons-(ps(igrid)%cons+&
                rk3_a31*ps3(igrid)%cons))/rk3_a32
             if(stagger_grid)then
                 ps(igrid)%prims=ps(igrid)%prims+rk3_b1*ps3(igrid)%prims &
                    +rk3_b2*(ps2(igrid)%prims-(ps(igrid)%prims+&
                    rk3_a31*ps3(igrid)%prims))/rk3_a32
             endif
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk3_b3, idimmin,idimmax,&
             global_time+rk3_c3*dt,ps2,global_time+(1.0d0-rk3_b3)*dt,ps)

       case ("IMEX_ARS3")
          ! this is IMEX scheme ARS3
          call advect1(flux_scheme,ars_gamma, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps4(igrid)%cons=(ps1(igrid)%cons-ps(igrid)%cons)/ars_gamma
             if(stagger_grid) ps4(igrid)%prims=(ps1(igrid)%prims-&
                ps(igrid)%prims)/ars_gamma
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%prims=(ps2(igrid)%prims-&
                ps1(igrid)%prims)/ars_gamma
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%cons=ps(igrid)%cons+(ars_gamma-1.0d0)*ps4(igrid)%cons+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%cons
             if(stagger_grid) then
                ps3(igrid)%prims=ps(igrid)%prims+&
                   (ars_gamma-1.0d0)*ps4(igrid)%prims+&
                   (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%prims
             endif
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,2.0d0*(1.0d0-ars_gamma), idimmin,idimmax,&
             global_time+ars_gamma*dt,ps2,global_time+(ars_gamma-1.0d0)*dt,&
             ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%cons=ps1(igrid)%cons+(ps3(igrid)%cons-(ps(igrid)%cons+ &
                (ars_gamma-1.0d0)*ps4(igrid)%cons+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%cons))/(2.0d0*(1.0d0-&
                ars_gamma))
             if(stagger_grid) then
             ps2(igrid)%prims=ps1(igrid)%prims+&
                (ps3(igrid)%prims-(ps(igrid)%prims+ &
                (ars_gamma-1.0d0)*ps4(igrid)%prims+&
                (1.0d0-2.0d0*ars_gamma)*ps1(igrid)%prims))/(2.0d0*(1.0d0-&
                ars_gamma))
             endif
          end do
          !$OMP END PARALLEL DO
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,&
             global_time+(1.0d0-ars_gamma)*dt,ps4,global_time+0.5d0*dt,ps)

       case ("IMEX_232")
          ! this is IMEX_ARK(2,3,2) or IMEX_SSP(2,3,2)
          call advect1(flux_scheme,imex_a21, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps1(igrid)%prims=ps1(igrid)%prims+&
                imex_ha21*dt*ps3(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons=ps(igrid)%cons+imex_a31*ps4(igrid)%cons &
                +imex_b1*dt*ps3(igrid)%cons+&
                imex_b2*(ps2(igrid)%cons-ps1(igrid)%cons)/imex_ha22
             if(stagger_grid) then
             ps(igrid)%prims=ps(igrid)%prims+imex_a31*ps4(igrid)%prims &
                +imex_b1*dt*ps3(igrid)%prims+&
                imex_b2*(ps2(igrid)%prims-ps1(igrid)%prims)/imex_ha22
             endif
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,imex_a32, idimmin,idimmax,&
             global_time+imex_c2*dt,ps2,global_time+imex_a31*dt,ps)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%cons=(ps(igrid)%cons-ps3(igrid)%cons-&
                imex_a31*ps4(igrid)%cons)/imex_a32 &
                +(1.0d0-imex_b2/imex_a32)*(ps2(igrid)%cons-&
                ps1(igrid)%cons)/imex_ha22
             if(stagger_grid) then
             ps2(igrid)%prims=(ps(igrid)%prims-ps3(igrid)%prims-&
                imex_a31*ps4(igrid)%prims)/imex_a32 &
                +(1.0d0-imex_b2/imex_a32)*(ps2(igrid)%prims-&
                ps1(igrid)%prims)/imex_ha22
             endif
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%cons=ps3(igrid)%cons+imex_b1*ps4(igrid)%cons+&
                imex_b2*ps2(igrid)%cons
             if(stagger_grid) then
             ps1(igrid)%prims=ps3(igrid)%prims+imex_b1*ps4(igrid)%prims+&
                imex_b2*ps2(igrid)%prims
             endif
          end do
          !$OMP END PARALLEL DO
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,imex_b3, idimmin,idimmax,&
    ...
    case ("fourstep")
       ...
       case ("ssprk4")
          ...
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%cons=rk_alfa31*ps(igrid)%cons+&
                rk_alfa33*ps2(igrid)%cons
             if(stagger_grid) ps1(igrid)%prims=rk_alfa31*ps(igrid)%prims+&
                rk_alfa33*ps2(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta33, idimmin,idimmax,&
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ...
             if(stagger_grid) ps(igrid)%prims=rk_alfa41*ps(igrid)%prims+&
                rk_alfa44*ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta44, idimmin,idimmax,&
       ...
       case ("rk4")
          ...
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time+0.5d0*dt,&
             ps1,global_time,ps2)
          call advect1(flux_scheme,1.0d0, idimmin,idimmax,global_time+0.5d0*dt,&
             ps2,global_time,ps3)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons=(1.0d0/3.0d0)*(-ps(igrid)%cons+ps1(igrid)%cons+&
                2.0d0*ps2(igrid)%cons+ps3(igrid)%cons)
             if(stagger_grid) ps(igrid)%prims=(1.0d0/3.0d0) &
                *(-ps(igrid)%prims+ps1(igrid)%prims+2.0d0*ps2(igrid)%prims+&
                ps3(igrid)%prims)
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,1.0d0/6.0d0, idimmin,idimmax,global_time+dt,&
       ...
       case ("jameson")
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,1.0d0/4.0d0, idimmin,idimmax,global_time,ps,&
             global_time,ps1)
          call advect1(flux_scheme,1.0d0/3.0d0, idimmin,idimmax,&
             global_time+dt/4.0d0,ps1,global_time,ps2)
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps1(igrid)%cons=ps(igrid)%cons
             if(stagger_grid) ps1(igrid)%prims=ps(igrid)%prims
          ...
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,0.5d0, idimmin,idimmax,global_time+dt/3.0d0,&
             ps2,global_time,ps1)
          call advect1(flux_scheme,1.0d0, idimmin,idimmax,global_time+0.5d0*dt,&
             ps1,global_time,ps)
    ...
    case ("fivestep")
       ...
       case ("ssprk5")
          ...
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps3(igrid)%cons=rk_alfa53*ps2(igrid)%cons+&
                rk_alfa54*ps1(igrid)%cons
             if(stagger_grid) ps3(igrid)%prims=rk_alfa53*ps2(igrid)%prims+&
                rk_alfa54*ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          !$OMP PARALLEL DO PRIVATE(igrid)
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps2(igrid)%cons=rk_alfa41*ps(igrid)%cons+&
                rk_alfa44*ps1(igrid)%cons
             if(stagger_grid) ps2(igrid)%prims=rk_alfa41*ps(igrid)%prims+&
                rk_alfa44*ps1(igrid)%prims
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta44, idimmin,idimmax,&
          ...
          do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
             ps(igrid)%cons=ps3(igrid)%cons+rk_alfa55*ps2(igrid)%cons &
                +(rk_beta54/rk_beta44)*(ps2(igrid)%cons-&
                (rk_alfa41*ps(igrid)%cons+rk_alfa44*ps1(igrid)%cons))
             if(stagger_grid) then
             ps(igrid)%prims=ps3(igrid)%prims+rk_alfa55*ps2(igrid)%prims &
                +(rk_beta54/rk_beta44)*(ps2(igrid)%prims-&
                (rk_alfa41*ps(igrid)%prims+rk_alfa44*ps1(igrid)%prims))
             endif
          end do
          !$OMP END PARALLEL DO
          call advect1(flux_scheme,rk_beta55, idimmin,idimmax,&
  ...
  subroutine advect1(method,dtfactor,idimmin,idimmax,qtC,psa,qt,psb)
    ...
    if (istep > 0) then
       ...
       do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
          call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,psa(igrid)%cons(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
             1:ncons),psa(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim),&
             psa(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim))
       end do
       !$OMP END PARALLEL DO
       ! fixme
    ...
    do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
       ...
       call process1_grid(method(level),igrid,qdt,ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
          idimmin,idimmax,qtC,psa(igrid),qt,psb(igrid),pso(igrid))
    ...
    if (fix_conserve_at_step) then
      ...
      call fix_conserve(psb,idimmin,idimmax,1,ncons)
      if(stagger_grid) call fix_edges(psb,idimmin,idimmax)
    end if
    ...
    if(stagger_grid) then
      ...
      !$OMP PARALLEL DO PRIVATE(igrid)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
        call phys_face_to_center(ixGlo1,ixGlo2,ixGhi1,ixGhi2,psb(igrid))
      end do
      !$OMP END PARALLEL DO
    end if
  ...
  subroutine process1_grid(method,igrid,qdt,ixImin1,ixImin2,ixImax1,ixImax2,&
    ...
    ! cell face flux
    double precision :: fC(ixImin1:ixImax1,ixImin2:ixImax2,1:ncons,1:ndim)
    ! cell edge flux
    ...
    call advect1_grid(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,idimmin,&
       idimmax,qtC,sCT,qt,s,sold,fC,fE,dx1,dx2, ps(igrid)%x)
    ...
    if (fix_conserve_at_step) then
      call store_flux(igrid,fC,idimmin,idimmax,ncons)
  ...
  subroutine advect1_grid(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,idimmin,&
     idimmax,qtC,sCT,qt,s,sold,fC,fE,dx1,dx2,x)
    ...
    case ('tvdmu','tvdlf','hll')
       call finite_volume(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,qtC,sCT,qt,s,sold,fC,fE,dx1,&
          dx2,x)
    case ('tvd')
       call tvdlimit(method,qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,&
          ixOmin2,ixOmax1,ixOmax2,idimmin,idimmax,sCT,qt+qdt,s,fC,dx1,dx2,x)
    case ('source')
       call addsource2(qdt*dble(idimmax-idimmin+1)/dble(ndim),ixImin1,ixImin2,&
          ixImax1,ixImax2,ixOmin1,ixOmin2,ixOmax1,ixOmax2,1,nprim,qtC,sCT%prim,&
          qt,s%prim,x,.false.)
    case ('nul')
  ...
  subroutine process(iit,qt)
    ...
    use mod_physics, only: phys_process_grid, phys_process_global
    use mod_global_parameters
    use mod_ghostcells_update
    ! .. scalars ..
    integer,intent(in)          :: iit
    ...
    if (associated(phys_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call phys_process_grid(igrid,level,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,&
            ixMlo2,ixMhi1,ixMhi2, qt,ps(igrid)%prim,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      update_bc = .True.
    end if
  ...
  subroutine process_advanced(iit,qt)
    ...
    use mod_physics, only: phys_handle_small_values, phys_process_adv_grid,&
    ...
    if (associated(phys_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call phys_process_adv_grid(igrid,level,ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
            ixMlo1,ixMlo2,ixMhi1,ixMhi2, qt,ps(igrid)%prim,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      update_bc = .True.
    end if
    ...
    if ( fix_small_values ) then
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call phys_handle_small_values(ps(igrid)%prim(ixGlo1:ixGhi1,&
             ixGlo2:ixGhi2,1:nprim),ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
             1:ndim),ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,ixMhi1,ixMhi2,&
              .True., 'process_advanced')
       end do
       !$OMP END PARALLEL DO
    end if
    ...
    if (check_NaN_values) then
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call prim_NaN_checker(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,ps(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim),&
             ps(igrid)%x(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:ndim))
       end do
       !$OMP END PARALLEL DO
    end if
