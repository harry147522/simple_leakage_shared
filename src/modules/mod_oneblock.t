...
if (mype == 0) write(*,*) 'mod_oneblock: reading ',nprim,' variables on unit:', unit
...
if (mype == 0) then 
   ...
   allocate(woneblock(nc^D,1:nprim))
   
   {do ix^DB=1,nc^DB\}  
   
   read(unit,*) xoneblock(ix^D,1:^ND), woneblock(ix^D,1:nprim)
...
if (npe>1) then 
   ...
   call MPI_BCAST(woneblock,{nc^D|*}*nprim,MPI_DOUBLE_PRECISION,0,icomm,ierrmpi)
end if! npe>1
