...
module mod_fix_conserve
   ...
   subroutine allocateBflux
     ...
     do iigrid=1,igridstail; igrid=igrids(iigrid);
       ...
       do iside=1,2
         ...
         case(neighbor_fine)
           allocate(pflux(iside,1,igrid)%flux(1,1:ncons))
 ...
 !opedit: skip over active/passive interface since flux for passive ones is 
             ...
             ! remove coarse flux
             if (slab_uniform) then
               psb(igrid)%prim(ix,nw0:nw1) = psb(igrid)%prim(ix,&
                  nw0:nw1) -pflux(iside,1,igrid)%flux(1,1:nwfluxin)
             else
               do iw=nw0,nw1
                 psb(igrid)%prim(ix,iw)=psb(igrid)%prim(ix,iw)-pflux(iside,1,&
            ...
            do ic1=1+int((1-i1)/2),2-int((1+i1)/2)
               ...
               if (ipe_neighbor==mype) then
                 iotherside=3-iside
                 if (slab_uniform) then
                   psb(igrid)%prim(ixmin1:ixmax1,&
                      nw0:nw1) = psb(igrid)%prim(ixmin1:ixmax1,&
                      nw0:nw1) + pflux(iotherside,1,ineighbor)%flux(:,&
                      1:nwfluxin)* CoFiratio
                 else
                   do iw=nw0,nw1
                     psb(igrid)%prim(ixmin1:ixmax1,&
                        iw)=psb(igrid)%prim(ixmin1:ixmax1,&
                 ...
                 if (slab_uniform) then
                   ...
                   psb(igrid)%prim(ixmin1:ixmax1,&
                      ...
                      *reshape(source=recvbuffer(ibuf:ibufnext-1),&
                       shape=shape(psb(igrid)%prim(ixmin1:ixmax1,nw0:nw1)))
                   ibuf=ibuf+isize(1)
                 else
                   ibufnext=ibuf+isize(1)
                   if(stagger_grid) then
   ...
   subroutine add_sub_circ(ixFmin1,ixFmax1,ixtEmin1,ixtEmax1,ixEmin1,ixEmax1,&
     ...
     do idim1=1,ndim
        ...
        if (add) then
          s%prims(ixCmin1:ixCmax1,idim1)=s%prims(ixCmin1:ixCmax1,&
