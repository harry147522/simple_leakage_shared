...
contains
  ...
  subroutine tvdlimit2(method,qdt,ixI^L,ixIC^L,ixO^L,idims,wL,wR,wnew,x,fC,dx^D)
    ...
    double precision, dimension(ixG^T,nprim) :: wL, wR
    double precision, intent(in) :: x(ixI^S,1:ndim)
    double precision :: wnew(ixI^S,1:nprim)
    double precision :: fC(ixI^S,1:ncons,1:ndim)

    double precision:: workroe(ixG^T,1:nworkroe)
    double precision, dimension(ixG^T,nprim) :: wroeC
