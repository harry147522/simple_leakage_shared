module mod_grhd_ccsn_leakage_mpi_phys_convert
  use mod_physics
...
!  use mod_grhd_ccsn_leakage_mpi_simple_leakage
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_mpi_phys_convert_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_leakage_mpi_phys_convert_init()
    ! initialize the k_max and v_max
    v_max = dsqrt(1.0d0 - 1.0d0 / lfac_max**2)
    k_max = 2.0d0 * v_max / (1.0d0 + v_max**2) ! k_max < 1
    phys_to_conserved        => grhd_ccsn_leakage_mpi_to_conserved
    phys_to_primitive        => grhd_ccsn_leakage_mpi_to_primitive
  end subroutine grhd_ccsn_leakage_mpi_phys_convert_init

  !> Transform primitive variables into conservative ones
  subroutine grhd_ccsn_leakage_mpi_to_conserved(ixImin1,ixImax1, ixOmin1,&
     ixOmax1, cons, prim, x)
    ...
    if ( fix_small_values ) then
       call grhd_ccsn_leakage_mpi_handle_small_values(prim(ixImin1:ixImax1,&
           1:nprim), x(ixImin1:ixImax1, 1:ndim), ixImin1,ixImax1, ixOmin1,&
          ixOmax1, .True., 'grhd_ccsn_leakage_mpi_to_conserved')
    end if
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixImin1,ixImax1,&
        ...
        1:ndim), gamma=gamma(ixImin1:ixImax1,1:3,1:3),&
        lfac=lfac(ixImin1:ixImax1), h=h(ixImin1:ixImax1) )
    ...
    cons(ixOmin1:ixOmax1, ye_con_) =  prim(ixOmin1:ixOmax1,&
        ye_) * cons(ixOmin1:ixOmax1, D_) 
...
!    ! leak_tau
!    if (.not. bounce) then
!        cons(ixO^S, leak_tau(:)) = 0.0d0
!    else if (first_iteration .and. bounce) then
!        cons(ixO^S, leak_tau(:)) = 0.0d0
!    else
!          
!    endif
...
!     positive_preserving  will also pass this sub for R and L
      cons(ixOmin1:ixOmax1, coolingsource(:)) = 0.0d0
!    endif
...
!  write(*,*) cons(5, coolingsource(:))
    ...
    cons(ixOmin1:ixOmax1, ye_con_) = cons(ixOmin1:ixOmax1,&
        ye_con_) * psi6(ixOmin1:ixOmax1)
    do idir = 1, ndir
       cons(ixOmin1:ixOmax1, mom(idir)) = cons(ixOmin1:ixOmax1,&
           mom(idir)) * psi6(ixOmin1:ixOmax1)
    end do
  end subroutine grhd_ccsn_leakage_mpi_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grhd_ccsn_leakage_mpi_to_primitive(ixImin1,ixImax1, ixOmin1,&
     ixOmax1, cons, prim, x)
    use mod_global_parameters
    use mod_geometry
...
!write(*,*) cons(5, leak_tau(:)), cons(10, coolingsource(:)), "leak_tau"
!   write(*,*) "to  prim"
    ...
    cons_tmp(ixOmin1:ixOmax1, tau_) = cons(ixOmin1:ixOmax1,&
        tau_) / psi6(ixOmin1:ixOmax1)
    cons_tmp(ixOmin1:ixOmax1, ye_con_) = cons(ixOmin1:ixOmax1,&
        ye_con_) / psi6(ixOmin1:ixOmax1)
    do idir = 1, ndir
...
! only ye no need rootfinding
       prim(ix1, ye_) = cons_tmp(ix1, ye_con_)/cons_tmp(ix1, D_)
         if (prim(ix1, ye_) .lt. eos_yemin) then
         if (mype == 0) then
         write(*,*) "con2prim   ye < yemin"
         endif
         prim(ix1, ye_) = eos_yemin
         endif

         if (prim(ix1, ye_) .gt. eos_yemax) then
         if (mype == 0) then
         write(*,*) "con2prim   ye > yemax"
         endif
         prim(ix1, ye_) = eos_yemax
         endif
...
!            eps_bar = max(small_eps, eos_epsmin)
         ...
         if ( adjustment ) then
            ...
            cons(ix1, ye_con_) = cons(ix1, D_) * prim(ix1, ye_) 
            ! Convert velocity to covariant momentum
            do idir = 1, ndir
            ...
            cons(ix1, ye_con_) = cons(ix1, ye_con_) * psi6(ix1)
    ...
    if (any(flag(ixOmin1:ixOmax1) < 0)) then
       ...
       call small_values_error(prim, x, ixImin1,ixImax1, ixOmin1,ixOmax1, flag,&
           'grhd_ccsn_leakage_mpi_to_primitive: (NaN z)')
    end if
...
!          if (present(ye_in)) ye_bar = ye_in / D
          ...
          if (present(h_out)) then
              !  write(*,*) rho_bar, eps_bar, ye_bar, temp_bar
  ...
  end subroutine grhd_ccsn_leakage_mpi_to_primitive
  
end module mod_grhd_ccsn_leakage_mpi_phys_convert
