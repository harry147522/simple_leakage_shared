...
double precision, dimension(1:1,1:1,1:nprim+nwauxio)   :: wval
...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   ...
   ps1(igrid)%prim(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
   ...
   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1-1,ixMlo2-1,&
            ixMhi1+1,ixMhi2+1, ps1(igrid)%prim,ps(igrid)%x,normconv)
      end if
   end if
end do
...
double precision  :: w_recv(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nprim),&
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if (mype/=0)then
   ...
   do ix1=ixMlo1,ixMhi1
      do iw=1,nprim
        if( dabs(ps(igrid)%prim(ix1,ix2,iw)) < 1.0d-32 ) ps(igrid)%prim(ix1,&
           ix2,iw) = zero
      enddo
       write(qunit,fmt="(100(e14.6))") ps(igrid)%x(ix1,ix2,1:ndim),&
          ps(igrid)%prim(ix1,ix2,1:nprim)
   end do
   end do
  end if
end do   
 ...
 if (mype==0) then
  loop_cpu : do ipe =1, npe-1
   loop_Morton : do Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixMlo1,ixMhi1
            do iw=1,nprim
              if( dabs(ps(igrid)%prim(ix1,ix2,&
                 iw)) < smalldouble ) ps(igrid)%prim(ix1,ix2,iw) = zero
            enddo
     ...
     case('tecplot')
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         do ix1=ixCmin1,ixCmax1
            ...
            w_TEC(1:nprim+nwauxio)=wC_TMP(ix1,ix2,&
       ...
       if(nprim+nwauxio==1)then
        ...
        if(ndim+nprim+nwauxio<10) then
         ...
         write(qunit,"(a,i7,a,a,i7,a,i7,a,f25.16,a,i1,a,i1,a,a)") 'ZONE T="',&
            ...
            ', DATAPACKING=BLOCK, VARLOCATION=([', ndim+1,'-',&
            ndim+nprim+nwauxio,']=CELLCENTERED), ZONETYPE=',&
              'FEQUADRILATERAL'
        else
...
double precision, dimension(ixMlo1-1:ixMhi1,ixMlo2-1:ixMhi2,&
   nprim+nwauxio)   :: wC_TMP,wC_TMP_recv
double precision, dimension(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
   nprim+nwauxio)     :: wCC_TMP,wCC_TMP_recv
double precision, dimension(0:nprim+nwauxio)                   :: normconv
     ...
     case('tecplotmpi')
        ...
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         do ix1=ixrvCmin1,ixrvCmax1
              ...
              w_TEC(1:nprim+nwauxio)=wC_TMP_RECV(ix1,ix2,&
                 1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
              write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         end do
         end do
