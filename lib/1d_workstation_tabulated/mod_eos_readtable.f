...
contains
   ...
   ! all variables in the module. 
     ...
     use mod_eos, only: eos_rhomin,  eos_rhomax, eos_yemin,   eos_yemax,&
          eos_tempmin, eos_tempmax, eos_epsmin, eos_epsmax
   ...
   ! HDF5 vars
     ...
     call h5fopen_f (trim(adjustl(eos_filename)), H5F_ACC_RDONLY_F, file_id,&
         error)
   ...
   ! read scalars
     ...
     if(dataset_there) then
        ...
        call h5dread_f(dset_id, H5T_NATIVE_INTEGER, have_rel_cs2, dims1,&
            error)
        call h5dclose_f(dset_id,error)
        if(error.ne.0) then
           stop "Could not read EOS table file"
     ...
     if (mype==0) write(message,"(a25,i5,i5,i5)") "We have nrho ntemp nye: ",&
         nrho,ntemp,nye
     ...
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_logpress),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "logenergy", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_logenergy),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "entropy", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_entropy),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "munu", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_munu),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "cs2", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_cs2), dims3,&
         error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "dedt", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_dedt),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "dpdrhoe", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_dpdrhoe),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     call h5dopen_f(file_id, "dpderho", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_dpderho),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   ...
   ! chemical potentials
     ...
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_muhat),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "mu_e", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_mu_e),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "mu_p", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_mu_p),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "mu_n", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_mu_n),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   ...
   ! compositions
     ...
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_xa), dims3,&
         error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "Xh", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_xh), dims3,&
         error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "Xn", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_xn), dims3,&
         error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "Xp", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_xp), dims3,&
         error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   ...
   ! average nucleus
     ...
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_abar),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   
     call h5dopen_f(file_id, "Zbar", dset_id, error)
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_zbar),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
   ...
   ! Gamma
     ...
     call h5dread_f(dset_id, H5T_NATIVE_DOUBLE, eos_tables(:,:,:,i_gamma),&
         dims3, error)
     call h5dclose_f(dset_id,error)
     accerr=accerr+error
     ...
     eos_tables(1:nrho,1:ntemp,1:nye,i_logpress) = eos_tables(1:nrho,1:ntemp,&
        1:nye,i_logpress) + log10(press_gf)
     eos_tables(1:nrho,1:ntemp,1:nye,i_logenergy) = eos_tables(1:nrho,1:ntemp,&
        1:nye,i_logenergy) + log10(eps_gf)
!     eos_tables(1:nrho,1:ntemp,1:nye,i_cs2) = eos_tables(1:nrho,1:ntemp,1:nye,i_cs2)*(clight)**2
     eos_tables(1:nrho,1:ntemp,1:nye,i_cs2) = eos_tables(1:nrho,1:ntemp,1:nye,&
        i_cs2)/(clight)**2
     eos_tables(1:nrho,1:ntemp,1:nye,i_dpderho) = eos_tables(1:nrho,1:ntemp,&
        1:nye,i_dpderho) + log10(press_gf) - log10(eps_gf)
     eos_tables(1:nrho,1:ntemp,1:nye,i_dpdrhoe) = eos_tables(1:nrho,1:ntemp,&
        1:nye,i_dpdrhoe) + log10(press_gf) - log10(rho_gf)
...
!                      (1.0d0 + 10.0d0**eos_tables(i,j,k,i_logenergy) - energy_shift + &
                 eos_tables(i,j,k,i_cs2) = eos_tables(i,j,k,&
                    i_cs2) / (1.0d0 + 10.0d0**eos_tables(i,j,k,&
                    i_logenergy) + 10.0d0**eos_tables(i,j,k,&
                    i_logpress)/10.0d0**logrho_table(i))
              enddo
           enddo
        enddo
