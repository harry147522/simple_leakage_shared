...
contains
  ...
  subroutine prolong_2nd_stg(sCo,sFi,ixComin1in,ixComax1in,ixFimin1in,&
    ...
    double precision :: bfluxCo(sCo%ixGsmin1:sCo%ixGsmax1,nprims),&
       bfluxFi(sFi%ixGsmin1:sFi%ixGsmax1,nprims)
        ...
        if (neighbor_type(i1,igrid)==neighbor_coarse) then
          ...
          if (refine(ineighbor,ipe_neighbor)) then
            ...
            if (iside==1) then !! left side
              pface(iside,1,igrid)%face(1)=ps(igrid)%prims(ixMlo1-1,1)
            else !! right side
              pface(iside,1,igrid)%face(1)=ps(igrid)%prims(ixMhi1,1)
            end if
