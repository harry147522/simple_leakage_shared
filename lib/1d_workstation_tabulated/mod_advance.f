...
contains
  ...
  subroutine advance(iit)
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
       call phys_to_conserved(ixGlo1,ixGhi1,ixmin1,ixmax1,&
       ...
       pso(igrid)%prim(ixGlo1:ixGhi1,1:nprim)=ps(igrid)%prim(ixGlo1:ixGhi1,&
          1:nprim)
       pso(igrid)%cons(ixmin1:ixmax1,1:ncons)=ps(igrid)%cons(ixmin1:ixmax1,&
          1:ncons)
...
!        write(*,*)"passed phys_step_adv_global"
  ...
  subroutine advect1(method,dtfactor,idimmin,idimmax,qtC,psa,qt,psb)
    ...
    if(stagger_grid) then
      ...
      !$OMP PARALLEL DO PRIVATE(igrid)
      do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
        call phys_face_to_center(ixGlo1,ixGhi1,psb(igrid))
      end do
      !$OMP END PARALLEL DO
    end if
  ...
  subroutine process1_grid(method,igrid,qdt,ixImin1,ixImax1,idimmin,idimmax,&
    ...
    ! cell face flux
    double precision :: fC(ixImin1:ixImax1,1:ncons,1:ndim)
    ! cell edge flux
  ...
  subroutine process(iit,qt)
    ...
    if (associated(phys_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call phys_process_grid(igrid,level,ixGlo1,ixGhi1,ixMlo1,ixMhi1, qt,&
    ...
    if (associated(usr_process_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         typelimiter=type_limiter(node(plevel_,igrid))
         typegradlimiter=type_gradient_limiter(node(plevel_,igrid))
         call usr_process_grid(igrid,level,ixGlo1,ixGhi1,ixMlo1,ixMhi1,qt,&
            ps(igrid)%prim,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
       update_bc = .True.
  ...
  subroutine process_advanced(iit,qt)
    ...
    if (associated(phys_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call phys_process_adv_grid(igrid,level,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
    ...
    if (associated(usr_process_adv_grid)) then
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         call usr_process_adv_grid(igrid,level,ixGlo1,ixGhi1,ixMlo1,ixMhi1, qt,&
            ps(igrid)%prim,ps(igrid)%x)
      end do
      !$OMP END PARALLEL DO
      update_bc = .True.
    ...
    if ( fix_small_values ) then
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call phys_handle_small_values(ps(igrid)%prim(ixGlo1:ixGhi1,1:nprim),&
             ps(igrid)%x(ixGlo1:ixGhi1,1:ndim),ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
    ...
    if (check_NaN_values) then
       !$OMP PARALLEL DO PRIVATE(igrid)
       do iigrid=1,igridstail; igrid=igrids(iigrid);
          call prim_NaN_checker(ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
             ps(igrid)%prim(ixGlo1:ixGhi1,1:nprim),ps(igrid)%x(ixGlo1:ixGhi1,&
             1:ndim))
       end do
       !$OMP END PARALLEL DO
