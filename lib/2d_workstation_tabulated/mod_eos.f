...
module mod_eos
  ...
  procedure(sub_get_eps_range), pointer                :: eos_get_eps_range    &
                 => null()
  procedure(sub_get_temp_one_grid), pointer            :: &
     eos_get_temp_one_grid            => null()
  procedure(sub_eps_get_all_one_grid), pointer         :: &
     eos_eps_get_all_one_grid         => null()
  procedure(sub_temp_get_all_one_grid), pointer        :: &
     eos_temp_get_all_one_grid        => null()
  procedure(sub_tabulated_logeps_to_logtemp),&
      pointer  :: tabulated_logeps_to_logtemp      => null()
  ...
  abstract interface
     ...
     subroutine sub_eps_get_all_one_grid(rho,temp,ye,eps,prs,ent,cs2,dedt,&
        dpderho,dpdrhoe,xa,xh,xn,xp,abar,zbar,mu_e,mu_n,mu_p,muhat,munu)
       double precision, intent(inout) :: rho
       double precision, intent(in) :: ye
       double precision, intent(inout) :: eps

       double precision, intent(inout), optional :: prs,temp,ent,cs2,dedt,&
          dpderho,dpdrhoe,xa,xh,xn,xp,abar,zbar,mu_e,mu_n,mu_p,muhat,munu
     ...
     subroutine sub_temp_get_all_one_grid(rho,temp,ye,eps,prs,ent,cs2,dedt,&
       ...
       double precision, intent(inout), optional :: prs,ent,cs2,dedt,dpderho,&
          dpdrhoe,xa,xh,xn,xp,abar,zbar,mu_e,mu_n,mu_p,muhat,munu

     end subroutine sub_temp_get_all_one_grid


     subroutine sub_tabulated_logeps_to_logtemp(lr,epsin,lt0,y)
       double precision :: lr,epsin,lt0,y
     end subroutine sub_tabulated_logeps_to_logtemp
...
!   code test
    ! based on idealgas (oldgmunu)
    small_press = idealK1*((small_rho/rho_gf)**(idealgamma))*press_gf
    small_eps = small_press / small_rho / ( idealgamma - 1.0d0 )


    write(*,*) small_press, small_eps, small_rho, "small prs, eps, rho"
!stop
