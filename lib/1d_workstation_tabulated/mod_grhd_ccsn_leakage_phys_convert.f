...
contains
  ...
  subroutine grhd_ccsn_leakage_to_conserved(ixImin1,ixImax1, ixOmin1,ixOmax1,&
      cons, prim, x)
    ...
    if ( fix_small_values ) then
       call grhd_ccsn_leakage_handle_small_values(prim(ixImin1:ixImax1,&
           1:nprim), x(ixImin1:ixImax1, 1:ndim), ixImin1,ixImax1, ixOmin1,&
          ixOmax1, .True., 'grhd_ccsn_leakage_to_conserved')
    end if
...
!  write(*,*) cons(5, coolingsource(:))
  ...
  subroutine grhd_ccsn_leakage_to_primitive(ixImin1,ixImax1, ixOmin1,ixOmax1,&
      cons, prim, x)
    use mod_global_parameters
    use mod_geometry
...
!            eps_bar = max(small_eps, eos_epsmin)
         ...
         enddo
         call grhd_ccsn_leakage_update_eos_one_point(prim(ix1,&
             1:nprim))       
    ...
    if (any(flag(ixOmin1:ixOmax1) < 0)) then
       ...
       call small_values_error(prim, x, ixImin1,ixImax1, ixOmin1,ixOmax1, flag,&
           'grhd_ccsn_leakage_to_primitive: (NaN z)')
    end if
