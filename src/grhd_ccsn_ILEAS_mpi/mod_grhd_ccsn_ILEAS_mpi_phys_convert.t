module mod_grhd_ccsn_ILEAS_phys_convert
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_ILEAS_phys_convert_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_ILEAS_phys_convert_init()
    ! initialize the k_max and v_max
    v_max = dsqrt(1.0d0 - 1.0d0 / lfac_max**2)
    k_max = 2.0d0 * v_max / (1.0d0 + v_max**2) ! k_max < 1
    phys_to_conserved        => grhd_ccsn_ILEAS_to_conserved
    phys_to_primitive        => grhd_ccsn_ILEAS_to_primitive
  end subroutine grhd_ccsn_ILEAS_phys_convert_init

  !> Transform primitive variables into conservative ones
  subroutine grhd_ccsn_ILEAS_to_conserved(ixI^L, ixO^L, cons, prim, x)
    ...
    if ( fix_small_values ) then
       call grhd_ccsn_ILEAS_handle_small_values(prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), ixI^L, ixO^L, .True., 'grhd_ccsn_ILEAS_to_conserved')
    end if
    ...
    cons(ixO^S, ye_con_) =  prim(ixO^S, ye_) * cons(ixO^S, D_) 
...
!     positive_preserving  will also pass this sub for R and L
      cons(ixO^S, coolingsource(:)) = 0.0d0
!    endif
...
!  write(*,*) cons(5, coolingsource(:))
    ...
    cons(ixO^S, ye_con_) = cons(ixO^S, ye_con_) * psi6(ixO^S)
    do idir = 1, ndir
       cons(ixO^S, mom(idir)) = cons(ixO^S, mom(idir)) * psi6(ixO^S)
    end do
  end subroutine grhd_ccsn_ILEAS_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grhd_ccsn_ILEAS_to_primitive(ixI^L, ixO^L, cons, prim, x)
    use mod_global_parameters
    use mod_geometry
...
!   write(*,*) "to  prim"
    ...
    cons_tmp(ixO^S, ye_con_) = cons(ixO^S, ye_con_) / psi6(ixO^S)
    do idir = 1, ndir
...
! only ye no need rootfinding
       prim(ix^D, ye_) = cons_tmp(ix^D, ye_con_)/cons_tmp(ix^D, D_)
         if (prim(ix^D, ye_) .lt. eos_yemin) then
         if (mype == 0) then
         write(*,*) "con2prim   ye < yemin"
         endif
         prim(ix^D, ye_) = eos_yemin
         endif

         if (prim(ix^D, ye_) .gt. eos_yemax) then
         if (mype == 0) then
         write(*,*) "con2prim   ye > yemax"
         endif
         prim(ix^D, ye_) = eos_yemax
         endif
...
!            eps_bar = max(small_eps, eos_epsmin)
         ...
         enddo
         call grhd_ccsn_ILEAS_update_eos_one_point(prim(ix^D, 1:nprim))       
         ...
         if ( adjustment ) then
            ...
            cons(ix^D, ye_con_) = cons(ix^D, D_) * prim(ix^D, ye_) 
            ! Convert velocity to covariant momentum
            do idir = 1, ndir
            ...
            cons(ix^D, ye_con_) = cons(ix^D, ye_con_) * psi6(ix^D)
    ...
    if (any(flag(ixO^S) < 0)) then
       flag(ixO^S) = rho_
       call small_values_error(prim, x, ixI^L, ixO^L, flag, 'grhd_ccsn_ILEAS_to_primitive: (NaN z)')
    end if
    ...
    contains
       subroutine get_vars(z, W_out, rho_out, eps_out, h_out)
          ...
          double precision                :: W_bar, eps_bar, a_bar, rho_bar, prs_bar, ye_bar, temp_bar
...
!          if (present(ye_in)) ye_bar = ye_in / D
          ...
          if (present(h_out)) then
             ...
             call eos_get_pressure_one_grid(prs_bar, rho_bar, eps_bar, ye=ye_bar, temp=temp_bar)
  ...
  end subroutine grhd_ccsn_ILEAS_to_primitive
  
end module mod_grhd_ccsn_ILEAS_phys_convert
