    ...
    do ic1=1,2
       ...
       if (ipeFi==mype) then
          ...
          if (ipe==mype) then
             ...
             if(stagger_grid) then
               do idir=1,ndim
                 ...
                 call MPI_ISEND(psc(igridFi)%prims,1,&
                    type_coarse_block_stg(idir,ic1,ic2),ipe,itag_stg, icomm,&
                    sendrequest_stg(isend),ierrmpi)
               end do
             end if
          end if
       else
          if (ipe==mype) then
             itag=ipeFi*max_blocks+igridFi
             irecv=irecv+1
             call MPI_IRECV(ps(igrid)%prim,1,type_sub_block(ic1,ic2),ipeFi,&
                itag, icomm,recvrequest(irecv),ierrmpi)
             if(stagger_grid) then
               do idir=1,ndim
                 itag_stg=(npe+ipeFi+1)*max_blocks+igridFi*(ndir-1+idir)
                 call MPI_IRECV(ps(igrid)%prims,1,type_sub_block_stg(idir,ic1,&
