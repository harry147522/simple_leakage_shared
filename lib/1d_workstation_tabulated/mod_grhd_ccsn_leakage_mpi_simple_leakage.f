!                                 heat_rad (absorption
module mod_grhd_ccsn_leakage_mpi_simple_leakage

  use mod_eos
  use mod_global_parameters
  use mod_physics
  use mod_grhd_ccsn_leakage_mpi_phys_parameters
  use mod_grhd_ccsn_leakage_mpi_simple_leakage_parameters
...
!  this subroutine is called after the getbc
  subroutine grhd_simple_leakage_mpi(iit,qt)
    ...
    double precision :: outputvar(ixGlo1:ixGhi1,1:3,igridstail) !1:3  lum1:3
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      do ix1 = ixGlo1,ixGhi1 
         ...
         enddo
         ps(igrid)%prim(ix1, leaktau(:)) = 0.0d0 
      enddo
    enddo
...
!      do iigrid=1,igridstail; igrid=igrids(iigrid);
!        write(*,*) ps(2)%prim(:,leaktau(1)), 'after getbc'
!        write(*,*) '###########' 
!        write(*,*) ps(2)%prim(:,rho_), 'after getbc  rho' 
!      enddo

!stop 
      do iigrid=1,igridstail; igrid=igrids(iigrid);
        call grhd_simple_leakage_mpi_main(qt,ixGlo1,ixGhi1,ixMlo1,ixMhi1,&
            ps(igrid)%cons(ixGlo1:ixGhi1,1:ncons),ps(igrid)%prim(ixGlo1:ixGhi1,&
           1:nprim),ps(igrid)%x(ixGlo1:ixGhi1,1:ndim), outputvar(ixGlo1:ixGhi1,&
           1:3,iigrid)) !inside cons(leak_tau(:)) changed
      enddo
        ...
        lum_total = 0.0d0
         do iigrid=1,igridstail; igrid=igrids(iigrid);
             do ix1 = ixMlo1,ixMhi1 
                lum_local(1:3) = lum_local(1:3) + outputvar(ix1,1:3,iigrid) 
!                lum_local(1:3) = max(outputvar(ix^D,lum(1:3),iigrid), lum_local(1:3))
             enddo
         enddo
        
                call MPI_ALLREDUCE(lum_local(1:3), lum_total(1:3), 3,&
...
!          filename = trim(/users/ho-yin.ng/patrick_gmunu/gmunu/tests/collapse/1D_collapse_leakage/output)//"/lum_nu.dat"
    if (mype == 0) then
          ...
          write(667,*) global_time,lum_total(1:3), shock_radius/1.0d5,&
              pns_radius/1.0d5
          close(667) 
    endif
...
!######################
  ...
  subroutine grhd_simple_leakage_mpi_main(qt,ixImin1,ixImax1,ixOmin1,ixOmax1,&
     cons,prim,x,outputvar)
    use mod_global_parameters
    use mod_geometry
    !use mod_eos
    implicit none
    ...
    double precision, intent(inout) :: outputvar(ixImin1:ixImax1, 1:3)
    double precision :: lprim(ixImin1:ixImax1, 1:nprim) 
    double precision :: lx(ixImin1:ixImax1, 1:nprim) 
    double precision :: lxi(ixImin1:ixImax1, 1:nprim)
    double precision :: leakvar(ixImin1:ixImax1, 1:nleak)  
    double precision :: eta_nue_eq(ixImin1:ixImax1)
...
!######################################################################
 ...
 !                         in     in    in   in  inout inout inout inout
   call primitive_mapping(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim, lx,&
       lxi, leakvar)
...
!   update  degeneracy parameters
         do ix1 = ixImin1,ixImax1 
          leakvar(ix1,eta_nu(3)) = 0.0d0   ! (A2)
          ! (A5) equilibrium eta, we have rest masses in our chemical potentials
          eta_nue_eq(ix1) = leakvar(ix1,eta_nucleons(1)) - leakvar(ix1,&
             eta_nucleons(3)) + leakvar(ix1,eta_nucleons(2))
          ! (A3); note that the ^0 etas are set to 0.0d0
          leakvar(ix1,eta_nu(1)) =  eta_nue_eq(ix1) * (1.0d0-exp(-prim(ix1,&
             leaktau(1)))) 
          ! (A4)
          leakvar(ix1,eta_nu(2)) = -eta_nue_eq(ix1) * (1.0d0-exp(-prim(ix1,&
             leaktau(2)))) 
         ...
         do ix1 = ixImin1,ixImax1 
           leakvar(ix1,eta_pn) = avo*lprim(ix1,rho_)*(leakvar(ix1,&
              mass_fraction(1))-leakvar(ix1,&
              mass_fraction(2)))/(exp(leakvar(ix1,eta_hat))-1.0d0)
           leakvar(ix1,eta_pn) = max(leakvar(ix1,eta_pn),0.0d0)
           leakvar(ix1,eta_np) = avo*lprim(ix1,rho_)*(leakvar(ix1,&
              mass_fraction(2))-leakvar(ix1,&
              mass_fraction(1)))/(exp(-leakvar(ix1,eta_hat))-1.0d0)
           leakvar(ix1,eta_np) = max(leakvar(ix1,eta_np),0.0d0)
...
! Rdiff Qdiff
        do ix1 = ixImin1,ixImax1 
            ...
            rate_const = 4.0d0*pi*clight*prim(ix1,&
               zeta(1))/(hc_mevcm**3*6.0d0*prim(ix1,chi(1))**2)
            leakvar(ix1,R_diff(1)) = rate_const*lprim(ix1,&
                temp_)*get_fermi_integral(0,leakvar(ix1,eta_nu(1)))
            leakvar(ix1,Q_diff(1)) = rate_const*lprim(ix1,&
                temp_)**2*get_fermi_integral(1,leakvar(ix1,eta_nu(1)))
            
            rate_const = 4.0d0*pi*clight*prim(ix1,&
               zeta(2))/(hc_mevcm**3*6.0d0*prim(ix1,chi(2))**2)
            leakvar(ix1,R_diff(2)) = rate_const*lprim(ix1,&
                temp_)*get_fermi_integral(0,leakvar(ix1,eta_nu(2)))
            leakvar(ix1,Q_diff(2)) = rate_const*lprim(ix1,&
                temp_)**2*get_fermi_integral(1,leakvar(ix1,eta_nu(2)))
            
            rate_const = 16.0d0*pi*clight*prim(ix1,&
               zeta(3))/(hc_mevcm**3*6.0d0*prim(ix1,chi(3))**2)
            leakvar(ix1,R_diff(3)) = rate_const*lprim(ix1,&
                temp_)*get_fermi_integral(0,leakvar(ix1,eta_nu(3)))
            leakvar(ix1,Q_diff(3)) = rate_const*lprim(ix1,&
                temp_)**2*get_fermi_integral(1,leakvar(ix1,eta_nu(3)))
     
         enddo ! end loop for every points
!write(*,*) leakvar(:,eta_nu(1)), 'etanu1'
!stop
    !find Q_loc and R_loc
    call compute_emission(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim, lx,&
        lxi, leakvar)

    !compute the rate of energy and ye loss/gain in each cell for RK
    call compute_update(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim, lx,&
        lxi, leakvar,outputvar)

! includes ghostzone
    do ix1 = ixImin1,ixImax1 
       do idir = 1, ndir+2
          cons(ix1, coolingsource(idir)) = leakvar(ix1,lcoolingsource(idir))
       enddo
    enddo ! end loop for every points
...
! single core
  subroutine integration_singlecore
    ...
    integer jm1,ix1, ix_next1, i
   integer :: igrid, iigrid, idir
!  hydro  for EOS 

    double precision, dimension(ixGlo1:ixGhi1, nprim, igridstail) :: lprim 
    double precision, dimension(ixGlo1:ixGhi1, ndim, igridstail) :: lx
    double precision, dimension(ixGlo1:ixGhi1, ndim, igridstail) :: lxi
    double precision, dimension(ixGlo1:ixGhi1, nleak, igridstail) :: leakvar
...
!   leak_tau  and eta_nu
    ...
    double precision, dimension(ixGlo1:ixGhi1,&
        igridstail) :: kappa_const_scat_n
    double precision, dimension(ixGlo1:ixGhi1,&
        igridstail) :: kappa_const_scat_p
!    double precision, dimension(ixI^S) :: kappa_const_scat_h
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: kappa_const_abs
    double precision :: csn_0,csp_0,t1,t2
    double precision :: xerr, xerr_local
    double precision, parameter :: xerr_out = 1.0d-12
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: kappa_tot   
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: kappa_tot_p 
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: kappa_scat_n !1/cm
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: kappa_scat_p !1/cm
!    double precision, dimension(ixG^T) :: kappa_scat_h ! 1/cm
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: kappa_abs_n !1/cm
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: kappa_abs_p !1/cm
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: local_eta_nue
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: local_eta_nua
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: local_eta_nux
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: eta_nue_eq
...
!   diffusion 
    ...
    double precision, dimension(ixGlo1:ixGhi1,3,3,&
       igridstail) :: kappa_tilde_nu_scat
    double precision, dimension(ixGlo1:ixGhi1,3,3,&
       igridstail) :: kappa_tilde_nu_abs
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: eta_pn
    double precision, dimension(ixGlo1:ixGhi1, igridstail) :: eta_np
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: R_diff
    double precision, dimension(ixGlo1:ixGhi1,3,igridstail) :: Q_diff
    double precision, dimension(1:3) :: chi_local, chi_mpi
    double precision :: rho_max_local, rho_max, ye_max_local, ye_max,&
        testvalue_local(1:3), testvalue

    do iigrid=1,igridstail; igrid=igrids(iigrid);
        
         call primitive_mapping(ixGlo1,ixGhi1, ixMlo1,ixMhi1,&
             ps(igrid)%prim(ixGlo1:ixGhi1,1:nprim),ps(igrid)%x(ixGlo1:ixGhi1,&
            1:ndim),lprim(ixGlo1:ixGhi1,1:nprim,iigrid), lx(ixGlo1:ixGhi1,&
            1:ndim,iigrid), lxi(ixGlo1:ixGhi1,1:ndim,iigrid),&
            leakvar(ixGlo1:ixGhi1,1:ndim,iigrid))
    enddo
...
!stop 
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      do ix1 = ixGlo1,ixGhi1 
       ...
       t1 = sigma_0 * avo * lprim(ix1, rho_, iigrid) * (lprim(ix1, temp_,&
           iigrid)/me_mev)**2
       kappa_const_scat_n(ix1,iigrid) = csn_0 * t1
       kappa_const_scat_p(ix1,iigrid) = csp_0 * t1
       ! (A11) constant part
       kappa_const_abs(ix1,iigrid) = (1.0d0+3.0d0*alpha**2)/4.0d0 * t1
      enddo ! end loop for every points
    enddo
    ...
    do while(xerr.gt.xerr_out .and. icount.lt.icount_max)
       ...
       ! contributions
       ! nu_e; (A17)
       kappa_tot(ixGlo1:ixGhi1,1,:) = kappa_scat_p(ixGlo1:ixGhi1,1,&
          :) + kappa_scat_n(ixGlo1:ixGhi1,1,:) + kappa_abs_n(ixGlo1:ixGhi1,:)
       ! antis; (A18)
       kappa_tot(ixGlo1:ixGhi1,2,:) = kappa_scat_p(ixGlo1:ixGhi1,2,&
          :) + kappa_scat_n(ixGlo1:ixGhi1,2,:) + kappa_abs_p(ixGlo1:ixGhi1,:)

       ! nu_xl (A19)
       kappa_tot(ixGlo1:ixGhi1,3,:) = + kappa_scat_p(ixGlo1:ixGhi1,3,&
          :) + kappa_scat_n(ixGlo1:ixGhi1,3,:)
           ...
           !  find the r_min 
              do iigrid=1,igridstail; igrid=igrids(iigrid);
                   coor_local(1) = min(minval(ps(igrid)%x(ixMlo1:ixMhi1, r_)),&
                       coor_local(1))
              enddo  
                  call MPI_ALLREDUCE(coor_local(1), coor(1), 1,&
        ...
        do i = 1, n1    
           ...
           do iigrid=1,igridstail; igrid=igrids(iigrid);
              do ix1 = ixMhi1, ixMlo1, -1
                if (ps(igrid)%x(ix1,r_) .ge. coor(1)) then 

                 dr = lxi(ix1+1,r_,iigrid) - lxi(ix1,r_,iigrid)
                 leak_tau_local(1:3) = leak_tau_local(1:3) + kappa_tot(ix1,1:3,&
                    iigrid) * dr * lprim(ix1, psi_,iigrid)**2
                endif
               
                if (ps(igrid)%x(ix1,r_) .gt. coor(1)) then 
                 x_min_local = min(ps(igrid)%x(ix1,r_), x_min_local)
                endif 
              enddo
           enddo 

                call MPI_ALLREDUCE(leak_tau_local(1:3), leak_tau(1:3), 3,&
                    mpi_double_precision, MPI_SUM, icomm, ierrmpi)
                ...
                do iigrid=1,igridstail; igrid=igrids(iigrid);
                  do ix1 = ixMlo1,ixMhi1 
                   if (ps(igrid)%x(ix1,r_) .eq. coor(1)) then
                        ps(igrid)%prim(ix1, leaktau(1:3)) = leak_tau(1:3)
...
!                stop
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         do ix1 = ixMlo1,ixMhi1 
          local_eta_nux(ix1, iigrid) = 0.0d0   ! (A2)
          ! (A5) equilibrium eta, we have rest masses in our chemical potentials
          ! no need to include mass difference in eta
          eta_nue_eq(ix1,iigrid) = leakvar(ix1,eta_nu(1),iigrid)
          ! (A3); note that the ^0 etas are set to 0.0d0
          local_eta_nue(ix1,iigrid) =  eta_nue_eq(ix1,&
             iigrid) * (1.0d0-exp(-ps(igrid)%prim(ix1,leaktau(1)))) 
          ! (A4)
          local_eta_nua(ix1,iigrid) = -eta_nue_eq(ix1,&
             iigrid) * (1.0d0-exp(-ps(igrid)%prim(ix1,leaktau(2)))) 
         enddo ! end loop for every points
       enddo
!        write(*,*)  local_eta_nue(5), local_eta_nua(5), "lcoal_eta_Nue, and nua"
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         do ix1 = ixMlo1,ixMhi1 
          ...
          xlye = lprim(ix1, ye_,iigrid)
          ! (A8)
          xyn = (1.0d0-xlye) / (1.0d0 + 2.0d0/3.0d0 * max(leakvar(ix1,&
             eta_nucleons(3),iigrid),0.0d0))
          xyp = xlye / (1.0d0 + 2.0d0/3.0d0*max(leakvar(ix1,eta_nucleons(2),&
             iigrid),0.0d0))
          t1 = exp(-leakvar(ix1,eta_hat,iigrid))
          ! (A13)
          ...
          t1 = get_fermi_integral(4+jm1,local_eta_nue(ix1,&
             iigrid)) / get_fermi_integral(2+jm1,local_eta_nue(ix1,iigrid))
          ! (A15)
          t2 = 1.0d0 + exp(leakvar(ix1,eta_nucleons(1),&
             iigrid)-get_fermi_integral(5,local_eta_nue(ix1,&
             iigrid)) / get_fermi_integral(4,local_eta_nue(ix1,iigrid)))
          ! (A6)

          kappa_scat_n(ix1,1,iigrid) = kappa_const_scat_n(ix1,&
             iigrid) * xyn  * t1
          kappa_scat_p(ix1,1,iigrid) = kappa_const_scat_p(ix1,&
             iigrid) * xyp  * t1

          ! (A11)
          kappa_abs_n(ix1,iigrid) = kappa_const_abs(ix1,&
             iigrid) * xynp * t1 / t2 

          ! anti-electron neutrinos
          t1 = get_fermi_integral(4+jm1,local_eta_nua(ix1,&
             iigrid)) / get_fermi_integral(2+jm1,local_eta_nua(ix1,iigrid))
          ! (A16)
          t2 = 1.0d0 + exp(-leakvar(ix1,eta_nucleons(1),&
             iigrid)-get_fermi_integral(5,local_eta_nua(ix1,&
             iigrid)) / get_fermi_integral(4,local_eta_nua(ix1,iigrid)))
          ! (A6)
          kappa_scat_n(ix1,2,iigrid) = kappa_const_scat_n(ix1,&
             iigrid) * xyn  * t1
          kappa_scat_p(ix1,2,iigrid) = kappa_const_scat_p(ix1,&
             iigrid) * xyp  * t1

          ! (A12)
          kappa_abs_p(ix1,iigrid) = kappa_const_abs(ix1,&
             iigrid) * xypn * t1 / t2 
          ! nux neutrinos
          t1 = get_fermi_integral(4+jm1,local_eta_nux(ix1,&
             iigrid)) / get_fermi_integral(2+jm1,local_eta_nux(ix1,iigrid))
          ! (A6)
          kappa_scat_n(ix1,3,iigrid) = kappa_const_scat_n(ix1,&
             iigrid) * xyn * t1
          kappa_scat_p(ix1,3,iigrid) = kappa_const_scat_p(ix1,&
             iigrid) * xyp * t1
...
!       write(*,*)  xynp*avo*lprim(5,rho_),  "eta_np"
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
       do ix1 = ixMlo1,ixMhi1 
          xerr_local = max(xerr_local,abs(kappa_tot(ix1,1,&
             iigrid)/kappa_tot_p(ix1,1,iigrid)-1.0d0))
          xerr_local = max(xerr_local,abs(kappa_tot(ix1,2,&
             iigrid)/kappa_tot_p(ix1,2,iigrid)-1.0d0))
          xerr_local = max(xerr_local,abs(kappa_tot(ix1,3,&
             iigrid)/kappa_tot_p(ix1,3,iigrid)-1.0d0))
       enddo ! end loop for every points
       enddo
...
!write(*,*) xerr, "icount+1"
    ...
    !pass    
    !   do iigrid=1,igridstail; igrid=igrids(iigrid);
    !    write(*,*) ps(igrid)%prim(ixM^T,leaktau(1))
    !   enddo 
...
!    !set degeneracy factors to interpolated values
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         do ix1 = ixMlo1,ixMhi1 
          leakvar(ix1,eta_nu(3),iigrid) = 0.0d0   ! (A2)
          ! (A5) equilibrium eta, we have rest masses in our chemical potentials
          eta_nue_eq(ix1,iigrid) = leakvar(ix1,eta_nucleons(1),&
             iigrid) - leakvar(ix1,eta_nucleons(3),iigrid) + leakvar(ix1,&
             eta_nucleons(2),iigrid)
          ! (A3); note that the ^0 etas are set to 0.0d0
          leakvar(ix1,eta_nu(1),iigrid) =  eta_nue_eq(ix1,&
             iigrid) * (1.0d0-exp(-ps(igrid)%prim(ix1,leaktau(1)))) 
          ! (A4)
          leakvar(ix1,eta_nu(2),iigrid) = -eta_nue_eq(ix1,&
             iigrid) * (1.0d0-exp(-ps(igrid)%prim(ix1,leaktau(2)))) 
         enddo ! end loop for every points
       enddo
!        write(*,*)  local_eta_nue(5), local_eta_nua(5), "lcoal_eta_Nue, and nua"
...
!######compute_diffusion############################# 
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      do ix1 = ixMlo1,ixMhi1 
        ps(igrid)%prim(ix1,zeta(1:3)) = 0.0d0
        ps(igrid)%prim(ix1,chi(1:3))  = 0.0d0
      enddo ! end loop for every points
    enddo
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      do ix1 = ixMlo1,ixMhi1 
       !scattering
       scattering_kappa = lprim(ix1, rho_,iigrid)*avo*0.25d0*sigma_0/me_mev**2
       kappa_tilde_nu_scat(ix1,1,1,iigrid) = leakvar(ix1,mass_fraction(1),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,1,2,iigrid) = leakvar(ix1,mass_fraction(2),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,1,iigrid) = leakvar(ix1,mass_fraction(1),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,2,iigrid) = leakvar(ix1,mass_fraction(2),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,1,iigrid) = leakvar(ix1,mass_fraction(1),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,2,iigrid) = leakvar(ix1,mass_fraction(2),&
          iigrid)*scattering_kappa
       
       scattering_kappa = lprim(ix1, rho_,&
          iigrid)*avo*0.0625d0*sigma_0/me_mev**2* leakvar(ix1,mass_fraction(5),&
          iigrid)*(1.0d0-leakvar(ix1,mass_fraction(6),iigrid)/leakvar(ix1,&
          mass_fraction(5),iigrid))**2 !only have 1 factor of A because kappa multiples the number fraction, not mass fractions
       kappa_tilde_nu_scat(ix1,1,3,iigrid) = leakvar(ix1,mass_fraction(4),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,3,iigrid) = leakvar(ix1,mass_fraction(4),&
          iigrid)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,3,iigrid) = leakvar(ix1,mass_fraction(4),&
          iigrid)*scattering_kappa

       eta_pn(ix1,iigrid) = avo*lprim(ix1,rho_,iigrid)*(leakvar(ix1,&
          mass_fraction(1),iigrid)-leakvar(ix1,mass_fraction(2),&
          iigrid))/(exp(leakvar(ix1,eta_hat,iigrid))-1.0d0)
       eta_pn(ix1,iigrid) = max(eta_pn(ix1,iigrid),0.0d0)
       eta_np(ix1,iigrid) = avo*lprim(ix1,rho_,iigrid)*(leakvar(ix1,&
          mass_fraction(2),iigrid)-leakvar(ix1,mass_fraction(1),&
          iigrid))/(exp(-leakvar(ix1,eta_hat,iigrid))-1.0d0)
       eta_np(ix1,iigrid) = max(eta_np(ix1,iigrid),0.0d0)
      !  write(*,*) eta_np(ix^D), lx(ix^D,r_),"eta_np"
!       if (eta_pn(ix^D) .le. 0.0d0  .or.  eta_np(ix^D) .le. 0.0d0) then 
!           stop "eta_pn, np   -ve"
!       endif
       ...
       if (lprim(ix1, rho_,iigrid).lt.1.0d11) then
          !non degenerate here, use mass fractions as chemical potentials fail at low densities
          eta_pn(ix1,iigrid) = avo*lprim(ix1, rho_,iigrid)*leakvar(ix1,&
             mass_fraction(2),iigrid)
          eta_np(ix1,iigrid) = avo*lprim(ix1, rho_,iigrid)*leakvar(ix1,&
             mass_fraction(1),iigrid)
       endif

       !absorption
!       abs_kappa = lrho(i)*avo*
       abs_kappa = (1.0d0+3.0d0*alpha**2)*0.25d0*sigma_0/me_mev**2
       block_factor = 1.0d0 + exp(leakvar(ix1,eta_nucleons(1),&
          iigrid)-get_fermi_integral(5,leakvar(ix1,eta_nu(1),&
          iigrid))/ get_fermi_integral(4,leakvar(ix1,eta_nu(1),iigrid)))
       kappa_tilde_nu_abs(ix1,1,1,iigrid) = eta_np(ix1,&
          iigrid)*abs_kappa/block_factor
       kappa_tilde_nu_abs(ix1,2,1,iigrid) = 0.0d0 !no absorption of a-type on neutrons
       kappa_tilde_nu_abs(ix1,3,1,iigrid) = 0.0d0 !no absorption of x-type neutrinos
       kappa_tilde_nu_abs(ix1,1,2,iigrid) = 0.0d0 !no absorption of e-type on protons
       block_factor = 1.0d0 + exp(-leakvar(ix1,eta_nucleons(1),&
          iigrid)-get_fermi_integral(5,leakvar(ix1,eta_nu(2),&
          iigrid))/ get_fermi_integral(4,leakvar(ix1,eta_nu(2),iigrid)))
       kappa_tilde_nu_abs(ix1,2,2,iigrid) = eta_pn(ix1,&
          iigrid)*abs_kappa/block_factor
       kappa_tilde_nu_abs(ix1,3,2,iigrid) = 0.0d0 !no absorption of x-type neutrinos
       kappa_tilde_nu_abs(ix1,1,3,iigrid) = 0.0d0 !no absorption on nuclei
       kappa_tilde_nu_abs(ix1,2,3,iigrid) = 0.0d0 !no absorption on nuclei
       kappa_tilde_nu_abs(ix1,3,3,iigrid) = 0.0d0 !no absorption on nuclei

       !sum up opacities to get zeta (again, factoring out energy dependence)
       ps(igrid)%prim(ix1,zeta(1)) = kappa_tilde_nu_scat(ix1,1,1,&
          iigrid) + kappa_tilde_nu_scat(ix1,1,2,&
          iigrid) + kappa_tilde_nu_scat(ix1,1,3,&
          iigrid) + kappa_tilde_nu_abs(ix1,1,1,&
          iigrid) + kappa_tilde_nu_abs(ix1,1,2,&
          iigrid) + kappa_tilde_nu_abs(ix1,1,3,iigrid)

       ps(igrid)%prim(ix1,zeta(2)) = kappa_tilde_nu_scat(ix1,2,1,&
          iigrid) + kappa_tilde_nu_scat(ix1,2,2,&
          iigrid) + kappa_tilde_nu_scat(ix1,2,3,&
          iigrid) + kappa_tilde_nu_abs(ix1,2,1,&
          iigrid) + kappa_tilde_nu_abs(ix1,2,2,&
          iigrid) + kappa_tilde_nu_abs(ix1,2,3,iigrid)

       ps(igrid)%prim(ix1,zeta(3)) = kappa_tilde_nu_scat(ix1,3,1,&
          iigrid) + kappa_tilde_nu_scat(ix1,3,2,&
          iigrid) + kappa_tilde_nu_scat(ix1,3,3,&
          iigrid) + kappa_tilde_nu_abs(ix1,3,1,&
          iigrid) + kappa_tilde_nu_abs(ix1,3,2,&
          iigrid) + kappa_tilde_nu_abs(ix1,3,3,iigrid)
    enddo ! end loop for every points
   enddo
           ...
           do iigrid=1,igridstail; igrid=igrids(iigrid);
              do ix1 = ixMhi1, ixMlo1, -1
                if (ps(igrid)%x(ix1,r_) .ge. coor(1)) then 
                 ...
                 dr = lxi(ix1+1,r_,iigrid) - lxi(ix1,r_,iigrid)
                 chi_local(1:3) = chi_local(1:3)  + ps(igrid)%prim(ix1,&
                    zeta(1:3))* dr
              ...
              enddo
           enddo 
                call MPI_ALLREDUCE(chi_local(1:3), chi_mpi(1:3), 3,&
                    mpi_double_precision, MPI_SUM, icomm, ierrmpi)
                call MPI_ALLREDUCE(x_min_local, x_min, 1, mpi_double_precision,&
                    MPI_MIN, icomm, ierrmpi)
                ...
                do iigrid=1,igridstail; igrid=igrids(iigrid);
                  do ix1 = ixMlo1,ixMhi1 
                   if (ps(igrid)%x(ix1,r_) .eq. coor(1)) then
                        ps(igrid)%prim(ix1, chi(1:3)) = chi_mpi(1:3)
                   endif
                  enddo ! end loop for every points
                enddo
                coor(1) = x_min  
...
!                do iigrid=1,igridstail; igrid=igrids(iigrid);
!                       write(*,*)  ps(igrid)%prim(ixM^T, chi(1)), ps(igrid)%prim(ixM^T,zeta(1))
!                enddo
!stop
...
! end subroutine finding_intermediate_variables
 ...
 subroutine primitive_mapping(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim,&
     lx, lxi, leakvar)
  implicit none
    ...
    double precision, intent(inout)    :: lprim(ixImin1:ixImax1, 1:nprim)
    double precision, intent(inout)    :: lx(ixImin1:ixImax1, 1:ndim)
    double precision, intent(inout)    :: lxi(ixImin1:ixImax1, 1:ndim)
    double precision, intent(inout)    :: leakvar(ixImin1:ixImax1, 1:nleak)
 

   lprim(ixImin1:ixImax1,:) = prim(ixImin1:ixImax1,:)
   lx(ixImin1:ixImax1,:) = x(ixImin1:ixImax1,:)
...
!write(*,*)   lx(ixO^S,r_), "x"
  ...
  !   one more ghostzone  to be buffer 
    do ix1 = ixOmin1-1,  ixOmax1+1
      lxi(ix1,:) = (x(ix1-1,:) + x(ix1,:))/2.0d0
    enddo ! end loop for every points


   ! only need change radius unit
    lxi(ixImin1:ixImax1, r_) = lxi(ixImin1:ixImax1, r_)/length_gf
    lx(ixImin1:ixImax1, r_) = lx(ixImin1:ixImax1, r_)/length_gf
...
!fixme
    do ix1 = ixOmin1,ixOmax1 
       leakvar(ix1,lvol) = 4.0d0*pi/3.0d0* (lxi(ix1, r_)**3 - lxi(ix1-1,&
           r_)**3 ) !using inner surface x1    !approxed for 1D 2D 3D 
    enddo ! end loop for every points


    do ix1 = ixOmin1,ixOmax1 
       leakvar(ix1,lIarea) = 1.0d0/(4.0d0*pi*lx(ix1, r_)**2) !using cell center   1d 2d approximately the same
    enddo ! end loop for every points
    ...
    do ix1 = ixImin1,ixImax1 
       ...
       !call eos to get quantities we need
        call eos_temp_get_all_one_grid(rho = lprim(ix1, rho_), eps = lprim(ix1,&
            ...
            xa = leakvar(ix1,mass_fraction(3)), xh = leakvar(ix1,&
           mass_fraction(4)),xn = leakvar(ix1,mass_fraction(1)),&
           xp = leakvar(ix1,mass_fraction(2)),abar = leakvar(ix1,&
           mass_fraction(5)), zbar = leakvar(ix1,mass_fraction(6)),&
            mu_e = xmu_e, mu_n = xmu_n, mu_p = xmu_p)

       ! in our EOS the rest mass difference is in the chemical potentials of
       ! the neucleons

       leakvar(ix1,eta_nucleons(1)) = xmu_e/lprim(ix1, temp_)
       leakvar(ix1,eta_nucleons(2)) = xmu_p/lprim(ix1, temp_)
       leakvar(ix1,eta_nucleons(3)) = xmu_n/lprim(ix1, temp_)

       leakvar(ix1,eta_hat) = leakvar(ix1,eta_nucleons(3))-leakvar(ix1,&
          eta_nucleons(2)) - Qnp/lprim(ix1, temp_)

       leakvar(ix1,eta_nu(1)) = leakvar(ix1,eta_nucleons(1)) - leakvar(ix1,&
          eta_nucleons(3)) + leakvar(ix1,eta_nucleons(2)) !fully includes effects of rest masses
       leakvar(ix1,eta_nu(2)) = -leakvar(ix1,eta_nu(1))
       leakvar(ix1,eta_nu(3)) = 0.0d0
...
! neutrino phys related subs
  ...
  subroutine compute_update(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim,&
      lx, lxi, leakvar, outputvar)
    use mod_global_parameters
    implicit none
    ...
    double precision, intent(inout) :: lx(ixImin1:ixImax1, 1:ndim)
    double precision, intent(inout) :: lxi(ixImin1:ixImax1, 1:ndim)
    double precision, intent(inout) :: outputvar(ixImin1:ixImax1, 1:3) !1. is lum
    double precision, intent(in) :: x(ixImin1:ixImax1, 1:ndim)
    double precision, intent(in) :: prim(ixImin1:ixImax1, 1:nprim)
    integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
...
!output
    ...
    double precision, dimension(ixImin1:ixImax1,3,3)       :: gamma_ij 
    double precision, dimension(ixImin1:ixImax1)           :: lfac
...
!  gamma_ij is code unit
   call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixImin1,ixImax1,&
       ixOmin1,ixOmax1, prim(ixImin1:ixImax1,1:nprim), x(ixImin1:ixImax1,&
      1:ndim), gamma=gamma_ij(ixImin1:ixImax1,1:3,1:3),&
...
!   write(*,*)  ixOmax1,  "ixOmax1" 
   do ix1 = ixOmin1,ixOmax1 
       ...
       leakvar(ix1,R_eff(:)) = leakvar(ix1,R_loc(:))/(1.0d0+leakvar(ix1,&
          R_loc(:))/leakvar(ix1,R_diff(:)))
       leakvar(ix1,Q_eff(:)) = leakvar(ix1,Q_loc(:))/(1.0d0+leakvar(ix1,&
          Q_loc(:))/leakvar(ix1,Q_diff(:)))

       gamma_eff(ix1,:,1) = 1.d0/(1.0d0+leakvar(ix1,R_loc(:))/leakvar(ix1,&
          R_diff(:)))
       gamma_eff(ix1,:,2) = 1.d0/(1.0d0+leakvar(ix1,Q_loc(:))/leakvar(ix1,&
          Q_diff(:)))
 
       do neu_num=1,3 
          if (leakvar(ix1,Q_diff(neu_num)).eq.0.0d0) then
             leakvar(ix1, Q_eff(neu_num)) = 0.0d0
             gamma_eff(ix1,neu_num,2) = 0.0d0
             ave_enr_nu(ix1,neu_num) = 0.0d0
          else
             ave_enr_nu(ix1,neu_num) = leakvar(ix1,Q_eff(neu_num))/leakvar(ix1,&
                R_eff(neu_num))
          endif

          if (leakvar(ix1,R_diff(neu_num)).eq.0.0d0) then
             leakvar(ix1, R_eff(neu_num)) = 0.0d0
       ...
       enddo
        ...
        leakvar(ix1,Q_tot) =  (sum(-leakvar(ix1,Q_eff(:))))
        leakvar(ix1,R_tot) =  -(leakvar(ix1,R_eff(1))-leakvar(ix1,R_eff(2)))
    enddo ! end loop for every points
...
!          !just cooling
              outputvar(ix1,:) =  leakvar(ix1,&
                 Q_eff(:))* mev_to_erg*leakvar(ix1,lvol)*lprim(ix1,&
                  alp_)**2*lprim(ix1, psi_)**6

              depsdt(ix1) = -sum(leakvar(ix1,Q_eff(1:3)))*mev_to_erg/lprim(ix1,&
                  rho_)
              dyedt(ix1) = -(leakvar(ix1,R_eff(1))-leakvar(ix1,&
                 R_eff(2)))*massn_cgs/lprim(ix1, rho_)
          else
...
!       coolingsource for S_r, S_theta, S_phi
          leakvar(ix1,lcoolingsource(idir)) = lprim(ix1, alp_)*lprim(ix1,&
         ...
         enddo
!                      for tau
          leakvar(ix1,lcoolingsource(ndir+1)) = lprim(ix1,&
              psi_)**6*lfac(ix1)*lprim(ix1, alp_)*lprim(ix1,&
              rho_) * rho_gf *depsdt(ix1)*eps_gf/time_gf
!                      for ye_con
          leakvar(ix1,lcoolingsource(ndir+2)) = lprim(ix1, psi_)**6*lprim(ix1,&
...
!   write(*,*) outputvar(:,1),'lum1'
!   write(*,*) outputvar(:,2),'lum2'
!stop
...
!######################################################################
  subroutine compute_emission(ixImin1,ixImax1, ixOmin1,ixOmax1, prim, x, lprim,&
      lx, lxi, leakvar)
    ...
    !electron & positron capture
       leakvar(ix1,R_loc(1)) = beta*leakvar(ix1,eta_pn)*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,leakvar(ix1,eta_nucleons(1)))
       leakvar(ix1,Q_loc(1)) = beta*leakvar(ix1,eta_pn)*lprim(ix1,&
           temp_)**6*get_fermi_integral(5,leakvar(ix1,eta_nucleons(1)))
       leakvar(ix1,R_loc(2)) = beta*leakvar(ix1,eta_np)*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,-leakvar(ix1,eta_nucleons(1)))
       leakvar(ix1,Q_loc(2)) = beta*leakvar(ix1,eta_np)*lprim(ix1,&
           temp_)**6*get_fermi_integral(5,-leakvar(ix1,eta_nucleons(1)))

       !e-e+ pair processes from Ruffert et al.
       block_factor_e = 1.0d0+exp(leakvar(ix1,&
          eta_nu(1))-0.5d0*( get_fermi_integral(4,leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,leakvar(ix1,&
          eta_nucleons(1))) + get_fermi_integral(4,-leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,-leakvar(ix1,&
          eta_nucleons(1))) ))
       block_factor_a = 1.0d0+exp(leakvar(ix1,&
          eta_nu(2))-0.5d0*( get_fermi_integral(4,leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,leakvar(ix1,&
          eta_nucleons(1))) + get_fermi_integral(4,-leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,-leakvar(ix1,&
          eta_nucleons(1))) ))
       block_factor_x = 1.0d0+exp(leakvar(ix1,&
          eta_nu(3))-0.5d0*( get_fermi_integral(4,leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,leakvar(ix1,&
          eta_nucleons(1))) + get_fermi_integral(4,-leakvar(ix1,&
          eta_nucleons(1)))/get_fermi_integral(3,-leakvar(ix1,&
          eta_nucleons(1))) ))
...
!       leakvar(ix^D,Q_loc(2)) = leakvar(ix^D,Q_loc(2)) + R_pair*0.5d0*(enr_tilde_m*enr_p+enr_m*enr_tilde_p)/(enr_m*enr_p)
       ...
       leakvar(ix1,R_loc(3)) = leakvar(ix1,R_loc(3)) + R_pair
       leakvar(ix1,Q_loc(3)) = leakvar(ix1,&
          Q_loc(3)) + R_pair*0.5d0*(enr_tilde_m*enr_p+&
          enr_m*enr_tilde_p)/(enr_m*enr_p)

       !plasmon decay from Ruffert et al.
       gamma = gamma_0*sqrt((pi**2+3.0d0*leakvar(ix1,&
          eta_nucleons(1))**2)/3.0d0)
       block_factor_e = 1.0d0 + exp(leakvar(ix1,&
          eta_nu(1))-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))
       block_factor_a = 1.0d0 + exp(leakvar(ix1,&
          eta_nu(2))-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))
       block_factor_x = 1.0d0 + exp(leakvar(ix1,&
          eta_nu(3))-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))
...
!       leakvar(ix^D,Q_loc(3)) = leakvar(ix^D,Q_loc(3)) + R_gamma*0.5d0*lprim(ix^D, temp_)*(2.0d0+gamma**2/(1.0d0+gamma))
       ...
       if (do_NNBrem) then
          R_pair = 0.3333d0*const_brems * ( leakvar(ix1,&
             mass_fraction(1))**2 + leakvar(ix1,&
             mass_fraction(2))**2 +(28.d0/3.d0)*leakvar(ix1,&
             mass_fraction(1))*leakvar(ix1,mass_fraction(2)))*lprim(ix1,&
              rho_)**2*lprim(ix1, temp_)**4.5d0
          Q_pair = R_pair * lprim(ix1, temp_)*3.0d0
  
          Q_pair = Q_pair *4.0d0
          R_pair = R_pair *4.0d0
          
          leakvar(ix1,Q_loc(3)) =  leakvar(ix1,Q_loc(3)) + Q_pair 
          leakvar(ix1,R_loc(3)) =  leakvar(ix1,R_loc(3)) + R_pair 
        endif        
...
end module mod_grhd_ccsn_leakage_mpi_simple_leakage
