    ...
    contains
      ...
      subroutine lb_recv
        ...
        if(stagger_grid) then
          ...
          call MPI_IRECV(ps(recv_igrid)%prims,1,type_block_io_stg,send_ipe,&
             itag, icomm,recvrequest_stg(irecv),ierrmpi)
        end if
      ...
      subroutine lb_send
        ...
        if(stagger_grid) then
          itag=recv_igrid+max_blocks
          call MPI_ISEND(ps(send_igrid)%prims,1,type_block_io_stg,recv_ipe,&
             itag, icomm,sendrequest_stg(isend),ierrmpi)
        end if
