       ...
       if (ipeFi==mype) then
          ...
          if (ipe==mype) then
             ...
             if(stagger_grid) then
               do idir=1,ndim
                 ...
                 call MPI_ISEND(psc(igridFi)%prims,1,type_coarse_block_stg(idir,ic^D),ipe,itag_stg, &
          ...
          if (ipe==mype) then
             ...
             if(stagger_grid) then
               do idir=1,ndim
                 itag_stg=(npe+ipeFi+1)*max_blocks+igridFi*(ndir-1+idir)
                 call MPI_IRECV(ps(igrid)%prims,1,type_sub_block_stg(idir,ic^D),ipeFi,itag_stg, &
