...
double precision, dimension({^D&1:1},1:nprim+nwauxio)   :: wval
double precision, dimension({^D&1:1},1:ndim)         :: xval
double precision:: normconv(0:nprim+nwauxio)

integer           :: iw,iiw,writenw,iwrite(1:nprim+nwauxio),iigrid,idim
...
normconv(0:nprim+nwauxio)=one
normconv(0) = length_convert_factor
normconv(1:nprim) = w_convert_factor
writenw=count(w_write(1:nprim))+nwauxio
iiw=0
do iw =1,nprim
...
do iigrid=1,igridstail; igrid=igrids(iigrid)
   ...
   ps1(igrid)%prim(ixG^T,1:nprim)=ps(igrid)%prim(ixG^T,1:nprim)
   ...
   if (nwauxio > 0) then
      if (.not. associated(usr_aux_output)) then
         ...
         call usr_aux_output(ixG^LL,ixM^LL^LADD1, &
              ps1(igrid)%prim,ps(igrid)%x,normconv)
      end if
   end if
end do
             ...
             select case(convert_type)
               case("oneblock")
                 write(qunit,fmt="(100(e14.6))") &
                  ...
                  (ps1(igrid)%prim(ix^D,iwrite(iw))*normconv(iwrite(iw)),iw=1,writenw)
               case("oneblockB")
                 write(qunit) real(ps(igrid)%x(ix^D,1:ndim)*normconv(0)),&
                  (real(ps1(igrid)%prim(ix^D,iwrite(iw))*normconv(iwrite(iw))),iw=1,writenw)
             end select
           end if Master_write
         end do
       end do
...
! only writes all 1:nprim variables, no nwauxio
...
double precision  :: w_recv(ixG^T,1:nprim),x_recv(ixG^T,1:ndim)
...
do Morton_no=Morton_start(mype),Morton_stop(mype)
  ...
  if (mype/=0)then
      ...
      itag=igrid
      call MPI_SEND(ps(igrid)%prim,1,type_block_io, 0,itag,icomm,ierrmpi)
  else
   {do ix^DB=ixMlo^DB,ixMhi^DB\}
      do iw=1,nprim
        if( dabs(ps(igrid)%prim(ix^D,iw)) < 1.0d-32 ) ps(igrid)%prim(ix^D,iw) = zero
      enddo
       write(qunit,fmt="(100(e14.6))") ps(igrid)%x(ix^D,1:ndim)&
                                     ,ps(igrid)%prim(ix^D,1:nprim)
   {end do\}
  end if
end do   
 ...
 if (mype==0) then
  loop_cpu : do ipe =1, npe-1
   loop_Morton : do Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         {do ix^DB=ixMlo^DB,ixMhi^DB\}
            do iw=1,nprim
              if( dabs(ps(igrid)%prim(ix^D,iw)) < smalldouble ) ps(igrid)%prim(ix^D,iw) = zero
            enddo
...
if(nprim/=count(w_write(1:nprim)))then
   ...
   do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      call calc_grid(qunit,igrid,xC,xCC,xC_TMP,xCC_TMP,wC_TMP,wCC_TMP,normconv,ixC^L,ixCC^L,.true.)
          {do ix^DB=ixCCmin^DB,ixCCmax^DB\}
            ...
            w_TEC(1:nprim+nwauxio)=wCC_TMP(ix^D,1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
     ...
     case('tecplot')
       ...
       do iigrid=1,igridstail; igrid=igrids(iigrid);
         ...
         {do ix^DB=ixCmin^DB,ixCmax^DB\}
            ...
            w_TEC(1:nprim+nwauxio)=wC_TMP(ix^D,1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
            write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         {end do\}
       enddo
     case('tecplotCC')
       ...
       if(ndim+nprim+nwauxio>99) call mpistop("adjust format specification in writeout")
       if(nprim+nwauxio==1)then
        ...
        if(ndim+nprim+nwauxio<10) then
         ! difference only in length of integer format specification for ndim+nprim+nwauxio
         write(qunit,"(a,i7,a,a,i7,a,i7,a,f25.16,a,i1,a,i1,a,a)") &
            'ZONE T="',level,'"',', N=',nodesonlevel,', E=',elemsonlevel, &
            ', SOLUTIONTIME=',global_time*time_convert_factor,', DATAPACKING=BLOCK, VARLOCATION=([', &
            ndim+1,'-',ndim+nprim+nwauxio,']=CELLCENTERED), ZONETYPE=', &
         {^IFONED 'FELINESEG'}{^IFTWOD 'FEQUADRILATERAL'}{^IFTHREED 'FEBRICK'}
        else
...
double precision, dimension(ixMlo^D-1:ixMhi^D,nprim+nwauxio)   :: wC_TMP,wC_TMP_recv
double precision, dimension(ixMlo^D:ixMhi^D,nprim+nwauxio)     :: wCC_TMP,wCC_TMP_recv
double precision, dimension(0:nprim+nwauxio)                   :: normconv
  ...
  do ipe=1,npe-1
   ...
   do level=levmin_recv,levmax_recv
    ...
    do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
     ...
     ixrvCmin^D=ind_recv(^D);ixrvCmax^D=ind_recv(^ND+^D);
     ixrvCCmin^D=ind_recv(2*^ND+^D);ixrvCCmax^D=ind_recv(3*^ND+^D);
     call MPI_RECV(normconv,nprim+nwauxio+1, MPI_DOUBLE_PRECISION,ipe,itag&
...
double precision, intent(in) :: normconv(0:nprim+nwauxio) 
...
character(len=name_len)   :: wnamei(1:nprim+nwauxio),xandwnamei(1:ndim+nprim+nwauxio),outtype
character(len=1024) :: outfilehead
       ...
       do ipe=1,npe-1
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         {do ix^DB=ixCCmin^DB,ixCCmax^DB\}
             ...
             w_TEC(1:nprim+nwauxio)=wCC_TMP_recv(ix^D,1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
             write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         {end do\}
        end do 
       end do
     ...
     case('tecplotmpi')
        ...
        do  Morton_no=Morton_start(ipe),Morton_stop(ipe)
         ...
         {do ix^DB=ixrvCmin^DB,ixrvCmax^DB\}
              ...
              w_TEC(1:nprim+nwauxio)=wC_TMP_RECV(ix^D,1:nprim+nwauxio)*normconv(1:nprim+nwauxio)
              write(qunit,fmt="(100(e14.6))") x_TEC, w_TEC
         {end do\}
        end do
     case('tecplotCCmpi')
