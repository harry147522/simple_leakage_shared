module mod_physics_hllc
  ...
  abstract interface
     subroutine sub_diffuse_hllcd(ixImin1,ixImax1,ixOmin1,ixOmax1,idims,wLC,&
       ...
       double precision, dimension(ixImin1:ixImax1,1:nprim),&
           intent(in)    :: wRC,wLC
       double precision, dimension(ixImin1:ixImax1,1:ncons),intent(in) :: fLC,&
           fRC
     ...
     subroutine sub_get_lCD(wLC,wRC,fLC,fRC,cmin,cmax,idims,ixImin1,ixImax1,&
       ...
       double precision, dimension(ixImin1:ixImax1,1:nprim),&
           intent(in)      :: wLC,wRC
       double precision, dimension(ixImin1:ixImax1,1:ncons),&
           intent(in)  :: fLC,fRC
     ...
     subroutine sub_get_wCD(wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,cmin,&
       ...
       double precision, dimension(ixImin1:ixImax1),&
           intent(in)          :: cmax,cmin
       double precision, dimension(ixImin1:ixImax1,1:ncons), intent(in) :: fRC,&
          fLC
       integer, dimension(ixImin1:ixImax1),&
           intent(in)                   :: patchf
       double precision, dimension(ixImin1:ixImax1,1:ncons),intent(out) :: f
...
contains
  ...
  subroutine dummy_diffuse_hllcd(ixImin1,ixImax1,ixOmin1,ixOmax1,idims,wLC,wRC,&
    ...
    double precision, dimension(ixImin1:ixImax1,1:nprim), intent(in)    :: wRC,&
       wLC
    double precision, dimension(ixImin1:ixImax1,1:ncons),intent(in) :: fLC,&
        fRC
  ...
  subroutine dummy_get_lCD(wLC,wRC,fLC,fRC,cmin,cmax,idims,ixImin1,ixImax1,&
    ...
    integer, dimension(ixImin1:ixImax1), intent(inout)                 :: &
       patchf
    double precision, dimension(ixImin1:ixImax1,1:ncons), intent(out) :: Fhll,&
       whll
    double precision, dimension(ixImin1:ixImax1),&
        intent(out)          :: lambdaCD

    ! Next must normally be computed
    Fhll(ixOmin1:ixOmax1,1:ncons) = zero
    whll(ixOmin1:ixOmax1,1:ncons) = zero
  ...
  subroutine dummy_get_wCD(wLC,wRC,whll,fRC,fLC,Fhll,patchf,lambdaCD,cmin,cmax,&
    ...
    double precision, dimension(ixImin1:ixImax1,1:nprim),&
        intent(in)     :: wRC,wLC
    double precision, dimension(ixImin1:ixImax1,1:ncons), intent(in) :: whll,&
        Fhll
    double precision, dimension(ixImin1:ixImax1),&
        intent(in)          :: lambdaCD
    double precision, dimension(ixImin1:ixImax1), intent(in)          :: cmax,&
    ...
    ! Next must normally be computed
    f(ixOmin1:ixOmax1,1:ncons)  = zero
