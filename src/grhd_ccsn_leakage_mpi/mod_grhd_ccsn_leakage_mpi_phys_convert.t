...
contains
  ...
  subroutine grhd_ccsn_leakage_mpi_to_conserved(ixI^L, ixO^L, cons, prim, x)
    ...
    if ( fix_small_values ) then
       call grhd_ccsn_leakage_mpi_handle_small_values(prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), ixI^L, ixO^L, .True., 'grhd_ccsn_leakage_mpi_to_conserved')
    end if
...
!  write(*,*) cons(5, coolingsource(:))
  ...
  subroutine grhd_ccsn_leakage_mpi_to_primitive(ixI^L, ixO^L, cons, prim, x)
    use mod_global_parameters
    use mod_geometry
...
!            eps_bar = max(small_eps, eos_epsmin)
    ...
    if (any(flag(ixO^S) < 0)) then
       flag(ixO^S) = rho_
       call small_values_error(prim, x, ixI^L, ixO^L, flag, 'grhd_ccsn_leakage_mpi_to_primitive: (NaN z)')
    end if
