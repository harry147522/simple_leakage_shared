...
contains
  ...
  subroutine finite_volume(method,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,idimsmin,&
     idimsmax,qtC,sCT,qt,snew,sold,fC,fE,dx1,x)
    use mod_physics
    ...
    double precision, dimension(ixImin1:ixImax1,1:ncons,1:ndim)     :: fC
    double precision, dimension(ixImin1:ixImax1,7-2*ndim:3)         :: fE

    ! left and right constructed status in conservative form
    double precision, dimension(ixImin1:ixImax1,1:ncons) :: consL, consR
    ! left and right constructed status in primitive form, needed for better performance
    double precision, dimension(ixImin1:ixImax1,1:nprim) :: primL, primR
    ! High order flux
    double precision, dimension(ixImin1:ixImax1,1:ncons) :: fLC, fRC
    ! Low order flux
    ...
    double precision, dimension(ixImin1:ixImax1,1:ncons,1:ndim) :: fClow
    ...
    do idims= idimsmin,idimsmax
       ...
       xi=x
       xi(ixImin1:ixImax1,idims)=xi(ixImin1:ixImax1,&
          idims)+0.5d0*sCT%dx(ixImin1:ixImax1,idims)
       ...
       primR(kxCmin1:kxCmax1,1:nprim)=primCT(kxRmin1:kxRmax1,1:nprim)
       primL(kxCmin1:kxCmax1,1:nprim)=primCT(kxCmin1:kxCmax1,1:nprim)

       ! low order TVD-Lax-Friedrich flux first.
       if (positivity_preserving) then
         ! fixme: maybe this part can be faster
         call phys_to_conserved(ixImin1,ixImax1,ixCRmin1,ixCRmax1,consR,primR,&
            xi)
         call phys_to_conserved(ixImin1,ixImax1,ixCRmin1,ixCRmax1,consL,primL,&
            xi)
         call phys_get_flux(consL,primL,xi,ixImin1,ixImax1,ixCmin1,ixCmax1,&
            idims,fLC)
         call phys_get_flux(consR,primR,xi,ixImin1,ixImax1,ixCmin1,ixCmax1,&
            idims,fRC)
         call phys_get_cbounds(consL,consR,primL,primR,xi,ixImin1,ixImax1,&
            ixCmin1,ixCmax1,idims,cmaxC)
         call get_Riemann_flux_tvdlf()
         if ( slab_uniform ) then
           do iw=nc_hydro_lo,nc_hydro_hi
             fClow(ixImin1:ixImax1,iw,idims)=fC(ixImin1:ixImax1,iw,idims)
           end do
         else 
           do iw=nc_hydro_lo,nc_hydro_hi
             fClow(ixImin1:ixImax1,iw,idims)=fC(ixImin1:ixImax1,iw,&
       ...
       if ( nmetric > 0 ) then
          ...
          if ( reconstruct_cfc ) then
             primR(kxCmin1:kxCmax1,nmetric_lo:nmetric_hi)=primCT(&
                kxRmin1:kxRmax1,nmetric_lo:nmetric_hi)
             primL(kxCmin1:kxCmax1,nmetric_lo:nmetric_hi)=primCT(&
                kxCmin1:kxCmax1,nmetric_lo:nmetric_hi)
             call reconstruct_LR(limiter_wenozp5, nmetric_lo, nmetric_hi,&
                 ixImin1,ixImax1,ixCRmin1,ixCRmax1,ixCRmin1,ixCRmax1,idims,&
                primCT,consL,consR,primL,primR,xi,dxdim(idims))
          else
             primL(kxCmin1:kxCmax1,nmetric_lo:nmetric_hi)=primCT(&
                kxCmin1:kxCmax1,nmetric_lo:nmetric_hi)
             call cfc_metric_interpolation(ixImin1,ixImax1,ixCRmin1,ixCRmax1,&
                idims,primCT,x,primL,xi)
             primR(ixCRmin1:ixCRmax1,nmetric_lo:nmetric_hi)=primL(&
                ixCRmin1:ixCRmax1,nmetric_lo:nmetric_hi)
          end if
       end if
       ! apply limited reconstruction for left and right status at cell interfaces
       call reconstruct_LR(typelimiter, nhydro_lo, nhydro_lo+nreconstruct-1,&
           ixImin1,ixImax1,ixCRmin1,ixCRmax1,ixCRmin1,ixCRmax1,idims,primCT,&
       ...
       call phys_get_flux(consL,primL,xi,ixImin1,ixImax1,ixCmin1,ixCmax1,idims,&
          fLC)
       call phys_get_flux(consR,primR,xi,ixImin1,ixImax1,ixCmin1,ixCmax1,idims,&
          fRC)
       ...
       if(method=='tvdlf'.or.method=='tvdmu') then
         ...
         call phys_get_cbounds(consL,consR,primL,primR,xi,ixImin1,ixImax1,&
            ixCmin1,ixCmax1,idims,cmaxC,cminC)
       end if
       ...
       if (.not.slab_uniform) then
          do iw=nc_hydro_lo,nc_hydro_hi
            fC(ixImin1:ixImax1,iw,idims)=fC(ixImin1:ixImax1,iw,&
               idims)*block%surfaceC(ixImin1:ixImax1,idims)
          end do
       end if
    ...
    if(associated(usr_set_flux)) then
       do idims= idimsmin,idimsmax
          call usr_set_flux(ixImin1,ixImax1,ixCmin1,ixCmax1,idims,fC,xi)
       end do ! Next idims
    end if

    ! fixme: this might not right as I modified the sturcture
    if(stagger_grid) call phys_update_faces(ixImin1,ixImax1,ixOmin1,ixOmax1,&
       qdt,primCT,fC,fE,sCT,snew)
    ...
    do idims= idimsmin,idimsmax
       ...
       if (slab_uniform) then
          do iw = nc_hydro_lo,nc_hydro_hi
            if ( flux_type(idims, iw) /= flux_nul ) then
               if (associated(phys_iw_methods(iw)%inv_capacity)) then
                 call phys_iw_methods(iw)%inv_capacity(ixImin1,ixImax1,&
                     ixOmin1,ixOmax1, cons_new, inv_volume)
                 cons_new(ixOmin1:ixOmax1,iw)=cons_new(ixOmin1:ixOmax1,&
                    iw) + inv_volume * dxinv(idims)*(fC(ixOmin1:ixOmax1,iw,&
                    idims)-fC(hxOmin1:hxOmax1,iw,idims))
               else
                 cons_new(ixOmin1:ixOmax1,iw)=cons_new(ixOmin1:ixOmax1,&
                    iw) + dxinv(idims)*(fC(ixOmin1:ixOmax1,iw,&
                    idims)-fC(hxOmin1:hxOmax1,iw,idims))
               end if
            end if
          end do
          ...
          if (associated(phys_iw_methods(iw)%inv_capacity)) then
            call phys_iw_methods(iw)%inv_capacity(ixImin1,ixImax1, ixOmin1,&
               ixOmax1, cons_new, inv_volume)
          else
            inv_volume(ixOmin1:ixOmax1) = 1.0d0
          end if
          inv_volume(ixOmin1:ixOmax1) = inv_volume(&
       ...
       if (method=='tvdmu') call tvdlimit2(method,qdt,ixImin1,ixImax1,ixCmin1,&
          ixCmax1,ixOmin1,ixOmax1,idims,consL,consR,cons_new,x,fC,dx1)

    end do ! Next idims

    if (.not.slab.and.idimsmin==1) call phys_add_source_geom(qdt,ixImin1,&
       ixImax1,ixOmin1,ixOmax1,consCT,primCT,cons_new,x)
    ...
    call addsource2(qdt*dble(idimsmax-idimsmin+1)/dble(ndim), ixImin1,ixImax1,&
       ixOmin1,ixOmax1,1,nprim,qtC,primCT,qt,cons_new,x,.false.)
  ...
  contains
      ...
      do iw=nc_hydro_lo,nc_hydro_hi
         ...
         if (flux_type(idims, iw) /= flux_no_dissipation) then
            fLC(ixCmin1:ixCmax1, iw)=fLC(ixCmin1:ixCmax1,&
                iw) + fac*(consR(ixCmin1:ixCmax1,iw)-consL(ixCmin1:ixCmax1,&
               iw))
         end if
    ...
    subroutine get_Riemann_flux_hll()
      ...
      elsewhere
        ...
        fac(ixCmin1:ixCmax1) = tvdlfeps*cminC(ixCmin1:ixCmax1)*cmaxC(&
           ixCmin1:ixCmax1)
        div(ixCmin1:ixCmax1) = 1/(cmaxC(ixCmin1:ixCmax1)-&
           cminC(ixCmin1:ixCmax1))
      endwhere
      ...
      do iw=nc_hydro_lo,nc_hydro_hi
         if (flux_type(idims, iw) == flux_tvdlf) then
            fLC(ixCmin1:ixCmax1, iw) = 0.5d0*(fLC(ixCmin1:ixCmax1,&
                ...
                dabs(cminC(ixCmin1:ixCmax1))) * (consR(ixCmin1:ixCmax1,&
               iw)-consL(ixCmin1:ixCmax1,iw)))
         else
            where(patchf(ixCmin1:ixCmax1)==1)
               ...
               fLC(ixCmin1:ixCmax1, iw) = &
                  (cmaxC(ixCmin1:ixCmax1)*fLC(ixCmin1:ixCmax1,&
                   ...
                   iw) +fac(ixCmin1:ixCmax1)*(consR(ixCmin1:ixCmax1,&
                  iw)-consL(ixCmin1:ixCmax1,iw))) * div(ixCmin1:ixCmax1)
    ...
    subroutine get_Riemann_flux_hllc()
      implicit none
      double precision, dimension(ixImin1:ixImax1,1:ncons)     :: whll, Fhll,&
          fCD
      ...
      if(method=='hllcd') call phys_diffuse_hllcd(ixImin1,ixImax1,ixCmin1,&
         ixCmax1,idims,consL,consR,fLC,fRC,patchf)

      !---- calculate speed lambda at CD ----!
      if(any(patchf(ixCmin1:ixCmax1)==1)) call phys_get_lCD(consL,consR,fLC,&
         fRC,cminC,cmaxC,idims,ixImin1,ixImax1,ixCmin1,ixCmax1, whll,Fhll,&
         lambdaCD,patchf)
      ...
      if(any(abs(patchf(ixCmin1:ixCmax1))== 1))then
         ...
         call phys_get_wCD(consL,consR,whll,fRC,fLC,Fhll,patchf,lambdaCD,cminC,&
            cmaxC,ixImin1,ixImax1,ixCmin1,ixCmax1,idims,fCD)
      ...
      do iw=nc_hydro_lo,nc_hydro_hi
         if (flux_type(idims, iw) == flux_tvdlf) then
            fLC(ixCmin1:ixCmax1,iw) = 0.5d0 * (fLC(ixCmin1:ixCmax1,&
               iw) + fRC(ixCmin1:ixCmax1,&
               iw) - tvdlfeps * max(cmaxC(ixCmin1:ixCmax1),&
                abs(cminC(ixCmin1:ixCmax1))) * (consR(ixCmin1:ixCmax1,&
               iw) - consL(ixCmin1:ixCmax1,iw)))
         else
    ...
    subroutine positivity_preserving_limiter()
      ...
      implicit none
      double precision, dimension(ixImin1:ixImax1)   :: inv_lambda
      double precision, dimension(ixImin1:ixImax1)   :: theta, theta_tau

       do idims= idimsmin,idimsmax
         if (slab_uniform) then
            !inv_lambda(ixI^S) = -dxinv(idims)
            inv_lambda(ixImin1:ixImax1) = dxdim(idims)/(qdt)
         else
            inv_lambda(ixImin1:ixImax1) = block%dvolume(ixImin1:ixImax1)/(qdt)
         end if

         call get_theta(ixImin1,ixImax1,ixCmin1,ixCmax1,idims,epsD,&
            inv_lambda(ixImin1:ixImax1),sCT%cons(ixImin1:ixImax1,D_),&
            fClow(ixImin1:ixImax1,D_,idims),fC(ixImin1:ixImax1,D_,idims),&
            theta)
         call get_theta(ixImin1,ixImax1,ixCmin1,ixCmax1,idims,epstau,&
            inv_lambda(ixImin1:ixImax1),sCT%cons(ixImin1:ixImax1,tau_),&
            fClow(ixImin1:ixImax1,tau_,idims),fC(ixImin1:ixImax1,tau_,idims),&
            theta_tau)

         theta(ixCmin1:ixCmax1) = min(theta(ixCmin1:ixCmax1),&
            theta_tau(ixCmin1:ixCmax1))

         do iw = nc_hydro_lo,nc_hydro_hi
            fC(ixCmin1:ixCmax1,iw,idims) = &
               theta(ixCmin1:ixCmax1)*fC(ixCmin1:ixCmax1,iw,&
               idims) + (1.0d0-theta(ixCmin1:ixCmax1))*fClow(ixCmin1:ixCmax1,&
               iw,idims)
         end do
       end do ! Next idims
  ...
  subroutine reconstruct_LR(typelimiter_in,rec_from,rec_to,ixImin1,ixImax1,&
     ixLmin1,ixLmax1,ixRmin1,ixRmax1,idims,w,consL,consR,primL,primR,x,dxdim)
    use mod_physics
    use mod_eos
    use mod_global_parameters
    use mod_limiter
    ...
    double precision, dimension(ixImin1:ixImax1,1:nprim) :: w
    ! left and right constructed status in conservative form
    double precision, dimension(ixImin1:ixImax1,1:ncons) :: consL, consR
    ...
    case (limiter_venk)
       call venklimiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          dxdim,w,primL,primR) 
    case (limiter_mp5)
       call MP5limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,w,&
          primL,primR)
    case (limiter_weno3)
       call WENO3limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          w,primL,primR)
    case (limiter_weno5)
       call WENO5limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          dxdim,w,primL,primR,1)
    case (limiter_wenoz5)
       call WENO5limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          dxdim,w,primL,primR,2)
    case (limiter_wenozp5)
       call WENO5limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          dxdim,w,primL,primR,3)
    case (limiter_weno7)
       call WENO7limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          w,primL,primR,1)
    case (limiter_mpweno7)
       call WENO7limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,idims,&
          w,primL,primR,2)
    case (limiter_exeno7)
       call exENO7limiter(rec_from,rec_to,ixImin1,ixImax1,ixLmin1,ixLmax1,&
          idims,w,primL,primR)
    case (limiter_ppm)
       if ( (rec_from >= nhydro_lo) .and. (rec_to <= nhydro_hi) ) fattening = &
          .True.
       ! our fattening is only available for hydro
       call PPMlimiter(rec_from,rec_to,ixImin1,ixImax1,ixMlo1,ixMhi1,idims,w,w,&
          primL,primR,fattening)
    case default
       ...
       do iw=rec_from, rec_to
          if (loglimit(iw)) then
             ...
             primL(ixLmin1:ixLmax1,iw)=dlog10(primL(ixLmin1:ixLmax1,iw))
             primR(ixRmin1:ixRmax1,iw)=dlog10(primR(ixRmin1:ixRmax1,iw))
          end if
          ...
          call dwlimiter2(dwC,ixImin1,ixImax1,ixCmin1,ixCmax1,idims,&
             typelimiter_in,ldw,rdw)
          primL(ixLmin1:ixLmax1,iw)=primL(ixLmin1:ixLmax1,&
             iw)+0.5d0*ldw(ixLmin1:ixLmax1)
          primR(ixRmin1:ixRmax1,iw)=primR(ixRmin1:ixRmax1,&
             iw)-0.5d0*rdw(jxRmin1:jxRmax1)

          if (loglimit(iw)) then
             w(ixCmin1:jxCmax1,iw)=10.0d0**w(ixCmin1:jxCmax1,iw)
             primL(ixLmin1:ixLmax1,iw)=10.0d0**primL(ixLmin1:ixLmax1,iw)
             primR(ixRmin1:ixRmax1,iw)=10.0d0**primR(ixRmin1:ixRmax1,iw)
          end if
       end do
    end select
    ...
    if ( (rec_from >= nhydro_lo) .and. (rec_to <= nhydro_hi) ) then
       ...
       call phys_handle_small_values(primL,x,ixImin1,ixImax1,ixLmin1,ixLmax1,&
           .False., 'reconstruction')
       call phys_handle_small_values(primR,x,ixImin1,ixImax1,ixRmin1,ixRmax1,&
           .False., 'reconstruction')

       ! we need to update rest of the prim variables
       call phys_update_eos(ixImin1,ixImax1,ixLmin1,ixLmax1,primL)
       call phys_update_eos(ixImin1,ixImax1,ixRmin1,ixRmax1,primR)
   
       call phys_to_conserved(ixImin1,ixImax1,ixLmin1,ixLmax1,consL,primL,x)
       call phys_to_conserved(ixImin1,ixImax1,ixRmin1,ixRmax1,consR,primR,x)
    end if
  end subroutine reconstruct_LR

  subroutine get_theta(ixImin1,ixImax1,ixCmin1,ixCmax1,idims,eps,inv_lambda,u,&
     flow,fhigh,theta)
    use mod_global_parameters, only: kr
    integer, intent(in)                             :: ixImin1,ixImax1,&
        ixCmin1,ixCmax1, idims
    double precision, intent(in)                    :: eps
    double precision, dimension(ixImin1:ixImax1), intent(in)  :: inv_lambda, u
    double precision, dimension(ixImin1:ixImax1), intent(in)  :: flow, fhigh
    double precision, dimension(ixImin1:ixImax1), intent(out) :: theta

    integer                                         :: ixCpmin1,ixCpmax1,&
        ixOpmin1,ixOpmax1
    double precision, dimension(ixImin1:ixImax1)              :: tmp, thp, thm
    double precision, dimension(ixImin1:ixImax1)              :: diff_fdA

    ! Note: here we assume that u( i=0 ) is given
    ixCpmin1=ixCmin1+kr(idims,1);ixCpmax1=ixCmax1+kr(idims,1);
    ixOpmin1=ixCmin1; ixOpmax1=ixCpmax1;
    
    thm(ixCmin1:ixCmax1) = 1.0d0
    thp(ixCmin1:ixCmax1) = 1.0d0
    
    tmp(ixOpmin1:ixOpmax1) = 0.5d0*inv_lambda(ixOpmin1:ixOpmax1)*(u(&
       ixOpmin1:ixOpmax1)-eps)

    diff_fdA(ixCmin1:ixCmax1) = -flow(ixCmin1:ixCmax1) + &
       fhigh(ixCmin1:ixCmax1)
    where (diff_fdA(ixCmin1:ixCmax1) == 0.0d0)
       diff_fdA(ixCmin1:ixCmax1) = epsilon(0.0d0) ! avoid flow = fhight case
    end where
    
    where (tmp(ixCmin1:ixCmax1) < fhigh(ixCmin1:ixCmax1))
       thm(ixCmin1:ixCmax1) = tmp(ixCmin1:ixCmax1) - flow(ixCmin1:ixCmax1)
       thm(ixCmin1:ixCmax1) = thm(ixCmin1:ixCmax1) / &
          (diff_fdA(ixCmin1:ixCmax1))
    end where
    
    where (tmp(ixCpmin1:ixCpmax1) < -fhigh(ixCmin1:ixCmax1))
       !thp(ixCp^S) = - tmp(ixCp^S) - flow(ixC^S)
       !thp(ixCp^S) = thp(ixCp^S) / (diff_fdA(ixC^S))
       thp(ixCmin1:ixCmax1) = - tmp(ixCpmin1:ixCpmax1) - flow(ixCmin1:ixCmax1)
       thp(ixCmin1:ixCmax1) = thp(ixCmin1:ixCmax1) / &
          (diff_fdA(ixCmin1:ixCmax1))
    end where

    theta(ixCmin1:ixCmax1) = min(thm(ixCmin1:ixCmax1),thp(ixCmin1:ixCmax1))
    theta(ixCmin1:ixCmax1) = min(max(theta(ixCmin1:ixCmax1),0.0d0),1.0d0)
