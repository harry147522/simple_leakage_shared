...
! leakage
      ...
      integer, allocatable  :: R_eff(:) !effective number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3
      integer, allocatable  :: R_loc(:) !local number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3
      integer, allocatable  :: R_diff(:) !diffusive number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3
      integer, allocatable  :: R_tot(:)

      integer, allocatable  :: Q_eff(:) !effective energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3
      !energy emission rate used in determining change in energy
      integer, allocatable  :: Q_loc(:) !local energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3
      integer, allocatable  :: Q_diff(:) !diffusive energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3
...
!      integer  :: zeta !Eq. A21, mean free path with E^2 factored out, indices <radial zones:neutrino species> - 1 / MeV^2 / cm
      ...
      integer  :: lvol !cell volume in cm3, indices <radial position>
      ...
      !degeneracy parameters
         ! 1: electrons  2: p  3: n
      integer, allocatable  :: eta_nucleons(:) !e degeneracy, indices <radial position>, including rest mass - dimension(:)less
