module mod_grhd_ccsn_leakage_mpi_phys_add_source
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_leakage_mpi_phys_add_source_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_leakage_mpi_phys_add_source_init()
    ...
    if (coordinate /= cartesian) then
       ...
       phys_add_source_geom     => grhd_ccsn_leakage_mpi_add_source_geom
    end if
    phys_add_source          => grhd_ccsn_leakage_mpi_add_source
  end subroutine grhd_ccsn_leakage_mpi_phys_add_source_init
  ...
  subroutine grhd_ccsn_leakage_mpi_add_source(qdt, ixI^L, ixO^L, primCT, cons, x, qsourcesplit, active)
    use mod_global_parameters
    use mod_geometry
!    use mod_grhd_ccsn_leakage_mpi_simple_leakage, only : bounce
    use mod_grhd_ccsn_leakage_mpi_simple_leakage, only : bounce
    integer, intent(in)             :: ixI^L, ixO^L
    ...
    double precision                :: add_source_leakage(ixI^S,ndir+2)
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixI^L, ixO^L, primCT(ixI^S, 1:nprim), x(ixI^S, 1:ndim), &
...
!stop  "you arrived add_source"
 ...
 ! write(*,*) cons(5, coolingsource(:))
       do idir = 1, ndir+2
         add_source_leakage(ixO^S, idir) = cons(ixO^S, coolingsource(idir))
       enddo
 
!  !     write(*,*) "wrong path"
       cons(ixO^S, tau_) = cons(ixO^S, tau_) + qdt * add_source_leakage(ixO^S, ndir+1)
       cons(ixO^S, ye_con_) = cons(ixO^S, ye_con_) + qdt * add_source_leakage(ixO^S, ndir+2)
       do idir =1, ndir
         cons(ixO^S, mom(idir)) = cons(ixO^S, mom(idir)) + qdt * add_source_leakage(ixO^S, idir)
       end do
   endif
...
!write(*,*) add_source(5,tau_) , x(5,r_), qdt,"normal ource"
  end subroutine grhd_ccsn_leakage_mpi_add_source

  !> Add geometrical source terms to w
  subroutine grhd_ccsn_leakage_mpi_add_source_geom(qdt, ixI^L, ixO^L, consCT, primCT, cons, x)
    use mod_global_parameters
    use mod_geometry
    integer, intent(in)             :: ixI^L, ixO^L
    double precision, intent(in)    :: qdt, x(ixI^S, 1:ndim)
    ...
    call grhd_ccsn_leakage_mpi_get_intermediate_variables(ixI^L, ixO^L, primCT(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lfac=lfac(ixI^S) )
  ...
  end subroutine grhd_ccsn_leakage_mpi_add_source_geom

end module mod_grhd_ccsn_leakage_mpi_phys_add_source
