      ...
      enddo
       ...
       !  find total_lum at infinity
        ...
        lum_total = 0.0d0
         do iigrid=1,igridstail; igrid=igrids(iigrid);
             {do ix^D = ixM^LL \}
                lum_local(1:3) = lum_local(1:3) + outputvar(ix^D,1:3,iigrid) 
...
!   update  degeneracy parameters
         ...
         {do ix^D = ixI^LIM^D \}
           leakvar(ix^D,eta_pn) = avo*lprim(ix^D,rho_)*&
                                 (leakvar(ix^D,mass_fraction(1))-leakvar(ix^D,mass_fraction(2)))/(exp(leakvar(ix^D,eta_hat))-1.0d0)
           leakvar(ix^D,eta_pn) = max(leakvar(ix^D,eta_pn),0.0d0)
           leakvar(ix^D,eta_np) = avo*lprim(ix^D,rho_)*&
                                 (leakvar(ix^D,mass_fraction(2))-leakvar(ix^D,mass_fraction(1)))/(exp(-leakvar(ix^D,eta_hat))-1.0d0)
           leakvar(ix^D,eta_np) = max(leakvar(ix^D,eta_np),0.0d0)

           if (lprim(ix^D, rho_).lt.1.0d11) then
              !non degenerate here, use mass fractions as chemical potentials fail at low densities
              leakvar(ix^D,eta_pn) = avo*lprim(ix^D, rho_)*leakvar(ix^D,mass_fraction(2))
              leakvar(ix^D,eta_np) = avo*lprim(ix^D, rho_)*leakvar(ix^D,mass_fraction(1))
           endif
         {enddo^D&\} ! end loop for every points
...
!stop 
    ...
    do while(xerr.gt.xerr_out .and. icount.lt.icount_max)
       ...
       ! nu_e; (A17)
       kappa_tot(ixG^T,1,:) = &
              kappa_scat_p(ixG^T,1,:) &
            + kappa_scat_n(ixG^T,1,:) &
            + kappa_abs_n(ixG^T,:)
       ! antis; (A18)
       kappa_tot(ixG^T,2,:) = &
              kappa_scat_p(ixG^T,2,:) &
            + kappa_scat_n(ixG^T,2,:) &
            + kappa_abs_p(ixG^T,:)

       ! nu_xl (A19)
       kappa_tot(ixG^T,3,:) = &
            + kappa_scat_p(ixG^T,3,:) &
            + kappa_scat_n(ixG^T,3,:)
