...
!  use mod_usr, only: bounce, t_bounce, shock_radius, pns_radius 
  ...
  private
      ...
      double precision, parameter :: sigma_0 = 1.76d-44 ! in units of cm2
      double precision, parameter :: alpha = 1.25d0 !dimension(ixImin1:ixImax1)less
      ...
      double precision, parameter :: rho_min_leak = 1.0d4 !in g/cm3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      double precision,allocatable  :: lprim(:, :)   ! cgs primvarialbes
      double precision,allocatable  :: lleak_tau(:, :) !read from cons(:,leak_tau(:)) 

!for mapping
      double precision,allocatable  :: lprim_local(:, :)   ! cgs primvarialbes
      double precision,allocatable  :: lleak_tau_local(:, :) !read from cons(:,leak_tau(:)) 
      ...
      double precision,allocatable  :: lfac(:)    ! Lorentz factor
      double precision,allocatable  :: gamma_ij(:,:,:)
      double precision,allocatable  :: lcoolingsource(:,:) !ndir are mom, ndir + 1 : tau, ndir +2 : Dye

      !number emission rate used in determining change in ye
      double precision,allocatable  :: R_eff(:,:) !effective number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3
      double precision,allocatable  :: R_loc(:,:) !local number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3 
      double precision,allocatable  :: R_diff(:,:) !diffusive number emission rate per volume, indices <radial zones:neutrino species> - number / sec / cm3 
      double precision,allocatable  :: R_tot(:)

      double precision,allocatable  :: Q_eff(:,:) !effective energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3
      !energy emission rate used in determining change in energy
      double precision,allocatable  :: Q_loc(:,:) !local energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3 
      double precision,allocatable  :: Q_diff(:,:) !diffusive energy emission rate per volume, indices <radial zones:neutrino species> - MeV / sec / cm3 
      double precision,allocatable  :: Q_tot(:)

      double precision,allocatable  :: chi_single(:,:) !optical depth with E2 factored out,indices <radial zones:neutrino species> - 1 / MeV2
      double precision,allocatable  :: zeta_single(:,:) !Eq. A21, mean free path with E2 factored out, indices <radial zones:neutrino species> - 1 / MeV2 / cm

      double precision,allocatable  :: kappa_tilde_nu_scat(:,:,:) !opacity without energy dependence for neutrino-matter scattering
                                                        !indices <radial zones:neutrino species:matter species> - 1 / MeV^2 / cm
      double precision,allocatable  :: kappa_tilde_nu_abs(:,:,:) !opacity without energy dependence for neutrino absorption
                                                        !indices <radial zones:neutrino species:matter species> - 1 / MeV^2 / cm
   
      double precision,allocatable  :: lvol(:) !cell volume in cm3, indices <radial position>
      double precision,allocatable  :: lIarea(:) !inverse cell area in cm-2, indices <radial position>
      double precision,allocatable  :: lmass(:) !cell mass in g, indices <radial position>
    
      !degeneracy parameters
         ! 1: electrons  2: p  3: n
      double precision,allocatable  :: eta_nucleons(:,:) !e degeneracy, indices <radial position>, including rest mass - dimension(:)less

      double precision,allocatable  :: eta_hat(:) !n degeneracy - p dengeneracy - Qnp/temp, removes mass difference, indices <radial position> - dimension(:)less
         ! 1: nue  2: nua  3: nux
      double precision,allocatable  :: eta_nu(:,:) !nu_e degeneracy, from eta_e - eta_n + eta_p, indices <radial position> - dimension(:)less

      double precision,allocatable  :: lepton_blocking(:,:) !blocking terms for electrons and positrons, indices <radial position,lepton type> - dimensionless
      
      !mass fractions
      ! 1. xxn   2. xxp   3.  xxa   4. xxh   5. xabar   6. xzbar
      double precision,allocatable  :: mass_fraction(:,:) 
    
      double precision,allocatable  :: eta_pn(:) !proton number density corrected for neutron final state blocking, indices <radial position> - dimension(:)less
      double precision,allocatable  :: eta_np(:) !neutron number density corrected for proton final state blocking, indices <radial position> - dimension(:)less


      !neutrino sphere
      integer          :: ns_location1(3) !neutrino sphere location, indices <neutrino species> - radial index
...
!    saveigrid=igrid
    ...
    do ix1 = ixMlo1,ixMhi1 
       ...
       do idir = 1, ndir+2
       ps(igrid)%cons(ix1, coolingsource(idir)) = 0.0d0
       enddo
    enddo
...
!######################
  ...
  subroutine grhd_simple_leakage_add_source(qdt,ixImin1,ixImax1,ixOmin1,&
     ixOmax1,iwmin,iwmax,qtC,primCT,qt,cons,x)
    use mod_global_parameters
    use mod_geometry
    implicit none
    ...
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1, iwmin,&
       iwmax
    double precision, intent(in)    :: qtC, qdt, x(ixImin1:ixImax1, 1:ndim)
    double precision, intent(in)    :: primCT(ixImin1:ixImax1, 1:nprim), qt
...
!       write(*,*) "wrong path" 
       cons(ixOmin1:ixOmax1, tau_) = cons(ixOmin1:ixOmax1,&
           tau_) + qdt * cons(ixOmin1:ixOmax1, coolingsource(ndir+1))
       cons(ixOmin1:ixOmax1, ye_con_) = cons(ixOmin1:ixOmax1,&
           ye_con_) + qdt * cons(ixOmin1:ixOmax1, coolingsource(ndir+2))
       do idir =1, ndir
         cons(ixOmin1:ixOmax1, mom(idir)) = cons(ixOmin1:ixOmax1,&
             mom(idir)) + qdt * cons(ixOmin1:ixOmax1, coolingsource(idir))
       end do
    endif
...
!  subroutine grhd_simple_leakage(ixO^L, ixI^L, prim, cons, x, dx^D, coolingsource)
  subroutine grhd_simple_leakage
    ...
    !use mod_eos
      ...
      integer :: idir, ix1, ix_next1
      integer :: n1,  ixImin1,ixImax1, ixOmin1,ixOmax1
...
!     write(*,*) ps(5)%cons(:,tau_), "b4 inverse mamp"
    ...
    call finding_array_size(n1)

    call initialize_arrays(n1)

    call mapping_to_single_core(n1)
   ...
   ixImin1 = 1
   ixImax1 = n1
   ixOmin1 = 1+ghostzones1
   ixOmax1 = n1-ghostzones1
!write(*,*) ixImin1, ixImax1, ixOmin1, ixOmax1
   call finding_intermediate_variables(ixImin1,ixImax1, ixOmin1,ixOmax1)
...
!  NOT calculate ghostzone
    do ix1 = ixOmin1,ixOmax1 
       lvol(ix1) = 4.0d0*pi/3.0d0* (lxi(ix1+1, r_)**3 - lxi(ix1, r_)**3 ) !using inner surface x1    !approxed for 1D 2D 3D 
    enddo ! end loop for every points

    do ix1 = ixOmin1,ixOmax1 
       lIarea(ix1) = 1.0d0/(4.0d0*pi*lx(ix1, r_)**2) !using cell center   1d 2d approximately the same
    enddo ! end loop for every points
...
! input is in code unit EOS
 ...
 !  {do ix^D = ixO^LIM^D \}
       ...
       !call eos to get quantities we need
        call eos_temp_get_all_one_grid(rho = lprim(ix1, rho_), eps = lprim(ix1,&
            eps_),  temp = lprim(ix1, temp_),ye = lprim(ix1, ye_),&
            xa = mass_fraction(ix1,3), xh = mass_fraction(ix1,4),&
           xn = mass_fraction(ix1,1),xp = mass_fraction(ix1,2),&
           abar = mass_fraction(ix1,5), zbar = mass_fraction(ix1,6),&
            mu_e = xmu_e, mu_n = xmu_n, mu_p = xmu_p)

       ! in our EOS the rest mass difference is in the chemical potentials of
       ! the neucleons

       eta_nucleons(ix1,1) = xmu_e/lprim(ix1, temp_)
       eta_nucleons(ix1,2) = xmu_p/lprim(ix1, temp_)
       eta_nucleons(ix1,3) = xmu_n/lprim(ix1, temp_)

       eta_hat(ix1) = eta_nucleons(ix1,3)-eta_nucleons(ix1,2) - Qnp/lprim(ix1,&
           temp_)

       eta_nu(ix1,1) = eta_nucleons(ix1,1) - eta_nucleons(ix1,&
          3) + eta_nucleons(ix1,2) !fully includes effects of rest masses
       eta_nu(ix1,2) = -eta_nu(ix1,1)
       eta_nu(ix1,3) = 0.0d0
...
!need map ghostzones as well
    lprim(ix1,rho_)  = lprim(ix1, rho_)/rho_gf
    lprim(ix1,eps_)  = lprim(ix1, eps_)/eps_gf
    lprim(ix1,cs2_)  = lprim(ix1, cs2_) * clight**2
    lprim(ix1,press_)  = lprim(ix1, press_)/press_gf
    enddo ! end loop for every points
...
!#############s
    ...
    call find_ruf_tau(ixImin1,ixImax1, ixOmin1,ixOmax1) 

!write(*,*) lleak_tau(:,1), shape(lleak_tau(:,1)) 
    !find Q_diff and R_diff
    call compute_diffusion(ixImin1,ixImax1, ixOmin1,ixOmax1)

    !find Q_loc and R_loc
    call compute_emission(ixImin1,ixImax1, ixOmin1,ixOmax1)

    !compute the rate of energy and ye loss/gain in each cell for RK
    call compute_update(ixImin1,ixImax1, ixOmin1,ixOmax1)

!stop "b4 inverse_mapping"
    call inverse_mapping(n1)
...
!######################################################################
  subroutine compute_update(ixImin1,ixImax1, ixOmin1,ixOmax1)
    use mod_global_parameters
    implicit none

    integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision :: heat_rad(ixImin1:ixImax1,3),heat_net(ixImin1:ixImax1)
...
!    character(len=100) filename
    ...
    integer :: ix1, ix_next1, ix_prev1, neu_num, theta, phi
!output
    double precision, dimension(ixImin1:ixImax1)           :: depsdt !energy change in cell due to neutrinos, indices <radial position> in erg/g/s
    double precision, dimension(ixImin1:ixImax1)           :: dyedt !change in electron number fraction, indices <radial position> in number fraction / s
    double precision, dimension(ixImin1:ixImax1,3)         :: ave_enr_nu !average neutrino energy, indices <radial position:neutrino species> in MeV
    double precision, dimension(ixImin1:ixImax1,3,2)       :: gamma_eff 
    double precision, dimension(ixImin1:ixImax1,3)         :: lum
    ...
    heat_const = (1.0d0+3.0d0*alpha**2) * sigma_0 / (me_mev**2 * massn_cgs) &
       *0.25d0
...
!   write(*,*)  ixOmax1,  "ixOmax1" 
   ...
   do ix1 = ixOmin1,ixOmax1 
       ...
       R_eff(ix1,:) = R_loc(ix1,:)/(1.0d0+R_loc(ix1,:)/R_diff(ix1,:))
       Q_eff(ix1,:) = Q_loc(ix1,:)/(1.0d0+Q_loc(ix1,:)/Q_diff(ix1,:))

       gamma_eff(ix1,:,1) = 1.d0/(1.0d0+R_loc(ix1,:)/R_diff(ix1,:))
       gamma_eff(ix1,:,2) = 1.d0/(1.0d0+Q_loc(ix1,:)/Q_diff(ix1,:))
 
       do neu_num=1,3 
          if (Q_diff(ix1,neu_num).eq.0.0d0) then
             Q_eff(ix1,neu_num) = 0.0d0
             gamma_eff(ix1,neu_num,2) = 0.0d0
             ave_enr_nu(ix1,neu_num) = 0.0d0
          else
             ave_enr_nu(ix1,neu_num) = Q_eff(ix1,neu_num)/R_eff(ix1,neu_num)
          endif

          if (R_diff(ix1,neu_num).eq.0.0d0) then
             R_eff(ix1,neu_num) = 0.0d0
             gamma_eff(ix1,neu_num,1) = 0.0d0
             ave_enr_nu(ix1,neu_num) = 0.0d0
          else
             ave_enr_nu(ix1,neu_num) = Q_eff(ix1,neu_num)/R_eff(ix1,neu_num)
          endif
       enddo
        ...
        Q_tot(ix1) =  (sum(-Q_eff(ix1,:)))
        R_tot(ix1) =  -(R_eff(ix1,1)-R_eff(ix1,2))
    enddo ! end loop for every points
!
!   write(*,*) Q_eff(:,1) , "Q_eff"
!stop
...
!stop
      ...
      if (do_heating .and. lx(ix1, r_) .lt. shock_radius) then !shock_radius  is in cm
          !here we sacrifice computational efficiency for clarity...
!        write(*,*) "pass do_heating"

          lepton_blocking(ix1,1) = 1.0d0/(1.0d0 + exp(eta_nucleons(ix1,&
             1) - get_fermi_integral(5,eta_nu(ns_location1(1),&
             1)  )/ get_fermi_integral(4,eta_nu(ns_location1(1),1))))
          lepton_blocking(ix1,2) = 1.0d0/(1.0d0 + exp(-eta_nucleons(ix1,&
             1) - get_fermi_integral(5,eta_nu(ns_location1(2),&
             2) )/ get_fermi_integral(4,eta_nu(ns_location1(2),2))))

          F(1:2) = (4.275d0*lleak_tau(ix1,&
             1:2)+1.15d0)*exp(-2.0d0*lleak_tau(ix1,1:2))*lepton_blocking(ix1,&
             1:2)


             heat_rad(ix1,1) = heat_fac*heat_const * lprim(ix1,&
                 rho_) * mass_fraction(ix1,1)* lum(ix_prev1,&
                1) * lIarea(ix1) * heat_erms(1)**2 * F(1) * lvol(ix1) &
                /(lprim(ix1, alp_)**2)
             heat_rad(ix1,2) = heat_fac*heat_const * lprim(ix1,&
                 rho_) * mass_fraction(ix1,2)* lum(ix_prev1,&
                2) * lIarea(ix1) * heat_erms(2)**2 * F(2) * lvol(ix1) &
                /(lprim(ix1, alp_)**2)

              heat_rad(ix1,:) =min(lum(ix_prev1,:)/lprim(ix1, alp_)**2,&
                 heat_rad(ix1,:))

          !in MeV/cm^3/s (same units as Q)
          heat_rad(ix1,1) = heat_rad(ix1,1) / lvol(ix1) * erg_to_mev
          heat_rad(ix1,2) = heat_rad(ix1,2) / lvol(ix1) * erg_to_mev
...
!                  mev_to_erg*lvol(ix^D)*alp(ix^D)**2*W(ix^D)*(1.0d0+v1(ix^D)*lx1(i))*lpsi(ix^D)**6
            lum(ix1,:) = lum(ix_prev1,:) + (Q_eff(ix1,:)-heat_rad(ix1,&
               :))*mev_to_erg*lvol(ix1)*lprim(ix1, alp_)**2*lprim(ix1,&
                psi_)**6

          depsdt(ix1) = (sum(-Q_eff(ix1,:)+heat_rad(ix1,:)))/lprim(ix1,&
              rho_)*mev_to_erg
          dyedt(ix1) = -(R_eff(ix1,1)-R_eff(ix1,2)-heat_rad(ix1,&
             1)/heat_em(1)+heat_rad(ix1,2)/heat_em(2))* massn_cgs/lprim(ix1,&
              rho_)
          
          heat_net(ix1) =  (-Q_eff(ix1,1)-Q_eff(ix1,2)+heat_rad(ix1,&
             1)+heat_rad(ix1,2))*mev_to_erg*lvol(ix1)

          heat_net_total = heat_net_total + max(heat_net(ix1),0.0d0)
...
!          heat_rad(i,2) = heat_rad(i,2) / lrho(i) * mev_to_erg
          ...
          if (lx(ix1, r_) .lt. shock_radius) then
!          !just cooling
              lum(ix1,:) = lum(ix_prev1,:) + Q_eff(ix1,&
                 :)* mev_to_erg*lvol(ix1)*lprim(ix1, alp_)**2*lprim(ix1,&
                  psi_)**6

              depsdt(ix1) = -sum(Q_eff(ix1,1:3))*mev_to_erg/lprim(ix1, rho_)
              dyedt(ix1) = -(R_eff(ix1,1)-R_eff(ix1,2))*massn_cgs/lprim(ix1,&
                  rho_)
          else
!          !outside shocked region
              lum(ix1,:) = lum(ix_prev1,:)
          endif
       endif
...
! put into imrpoeved leakage
       ...
       if (lprim(ix1, ye_).le.eos_yemin*1.01d0) then
          !need to surpress any cooling/heating if dyedt.lt.0 near boundary
          if (dyedt(ix1).lt.0.0d0) then
             dyedt(ix1) = 0.0d0
             depsdt(ix1) = 0.0d0
          endif
       endif

      if (lprim(ix1, ye_).ge.eos_yemax*0.99d0) then
          !need to surpress any cooling/heating if dyedt.lt.0 near boundary
          if (dyedt(ix1).gt.0.0d0) then
             dyedt(ix1) = 0.0d0
             depsdt(ix1) = 0.0d0
          endif
       endif
...
!       coolingsource for S_r, S_theta, S_phi
          lcoolingsource(ix1, idir) = lprim(ix1, alp_)*lprim(ix1,&
              psi_)**6 *lprim(ix1, rho_) * rho_gf &
             *depsdt(ix1)*(eps_gf/time_gf)*& !this is the lQ_tot
                                   (lprim(ix1, beta(idir))*lfac(ix1)/lprim(ix1,&
                                       alp_)  + gamma_ij(ix1, idir,&
                                       idir)*lfac(ix1)*((lprim(ix1,&
                                       W_vel(idir))/lfac(ix1)) - lprim(ix1,&
                                       beta(idir))/lprim(ix1, alp_)) )
         enddo
!                      for tau
          lcoolingsource(ix1, ndir+1) = lprim(ix1,&
              psi_)**6*lfac(ix1)*lprim(ix1, alp_)*lprim(ix1,&
              rho_) * rho_gf *depsdt(ix1)*eps_gf/time_gf
!                      for ye_con
          lcoolingsource(ix1, ndir+2) = lprim(ix1, psi_)**6*lprim(ix1,&
              alp_)*lprim(ix1, rho_) * rho_gf *dyedt(ix1)/time_gf 
...
!
   do ix1 = ixOmin1,ixOmax1 
        if (lx(ix1,r_) .le. 5.0d7) then
                lum_500(1) = max(lum(ix1,1), lum_500(1))
                lum_500(2) = max(lum(ix1,2), lum_500(2))
                lum_500(3) = max(lum(ix1,3), lum_500(3))
        endif
    enddo ! end loop for every points
...
!          filename = trim(/users/ho-yin.ng/patrick_gmunu/gmunu/tests/collapse/1D_collapse_leakage/output)//"/lum_nu.dat"
    if (mype == 0) then
          open(667,file=filename,status='unknown',position='append')
          write(667,*) global_time,lum(ixOmax1,1),lum(ixOmax1,2), lum(ixOmax1,&
             3), shock_radius/1.0d5, pns_radius/1.0d5
          close(667) 
    endif
...
!######################################################################
  subroutine compute_emission(ixImin1,ixImax1, ixOmin1,ixOmax1)
    ...
    double precision :: enr_m, enr_p, enr_tilde_m, enr_tilde_p, const_brems,&
        ksi_brems
    double precision get_fermi_integral
    ...
    !electron & positron capture
       R_loc(ix1,1) = beta*eta_pn(ix1)*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,eta_nucleons(ix1,1))
       Q_loc(ix1,1) = beta*eta_pn(ix1)*lprim(ix1,&
           temp_)**6*get_fermi_integral(5,eta_nucleons(ix1,1))
       R_loc(ix1,2) = beta*eta_np(ix1)*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,-eta_nucleons(ix1,1))
       Q_loc(ix1,2) = beta*eta_np(ix1)*lprim(ix1,&
           temp_)**6*get_fermi_integral(5,-eta_nucleons(ix1,1))

       !e-e+ pair processes from Ruffert et al.
       block_factor_e = 1.0d0+exp(eta_nu(ix1,1)-0.5d0*( get_fermi_integral(4,&
          eta_nucleons(ix1,1))/get_fermi_integral(3,eta_nucleons(ix1,&
          1)) + get_fermi_integral(4,-eta_nucleons(ix1,&
          1))/get_fermi_integral(3,-eta_nucleons(ix1,1)) ))
       block_factor_a = 1.0d0+exp(eta_nu(ix1,2)-0.5d0*( get_fermi_integral(4,&
          eta_nucleons(ix1,1))/get_fermi_integral(3,eta_nucleons(ix1,&
          1)) + get_fermi_integral(4,-eta_nucleons(ix1,&
          1))/get_fermi_integral(3,-eta_nucleons(ix1,1)) ))
       block_factor_x = 1.0d0+exp(eta_nu(ix1,3)-0.5d0*( get_fermi_integral(4,&
          eta_nucleons(ix1,1))/get_fermi_integral(3,eta_nucleons(ix1,&
          1)) + get_fermi_integral(4,-eta_nucleons(ix1,&
          1))/get_fermi_integral(3,-eta_nucleons(ix1,1)) ))
       
       enr_m = 8.0d0*pi/hc_mevcm**3*lprim(ix1, temp_)**4*get_fermi_integral(3,&
          eta_nucleons(ix1,1))
       enr_p = 8.0d0*pi/hc_mevcm**3*lprim(ix1, temp_)**4*get_fermi_integral(3,&
          -eta_nucleons(ix1,1))

       enr_tilde_m = 8.0d0*pi/hc_mevcm**3*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,eta_nucleons(ix1,1))
       enr_tilde_p = 8.0d0*pi/hc_mevcm**3*lprim(ix1,&
           temp_)**5*get_fermi_integral(4,-eta_nucleons(ix1,1))
...
!       Q_loc(ix^D,2) = Q_loc(ix^D,2) + R_pair*0.5d0*(enr_tilde_m*enr_p+enr_m*enr_tilde_p)/(enr_m*enr_p)
       ...
       R_pair =  pair_const/(9.0d0*block_factor_x**2)*((Cv-Ca)**2+(Cv+&
          Ca-2.0d0)**2)

       R_loc(ix1,3) = R_loc(ix1,3) + R_pair
       Q_loc(ix1,3) = Q_loc(ix1,3) + R_pair*0.5d0*(enr_tilde_m*enr_p+&
          enr_m*enr_tilde_p)/(enr_m*enr_p)

       !plasmon decay from Ruffert et al.
       gamma = gamma_0*sqrt((pi**2+3.0d0*eta_nucleons(ix1,1)**2)/3.0d0)
       block_factor_e = 1.0d0 + exp(eta_nu(ix1,&
          1)-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))
       block_factor_a = 1.0d0 + exp(eta_nu(ix1,&
          2)-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))
       block_factor_x = 1.0d0 + exp(eta_nu(ix1,&
          3)-(1.0d0+0.5d0*gamma**2/(1.0d0+gamma)))

       gamma_const = pi**3*sigma_0*clight*lprim(ix1,&
           temp_)**8/(me_mev**2*3.0d0*fsc*hc_mevcm**6)* &
...
!       Q_loc(ix^D,3) = Q_loc(ix^D,3) + R_gamma*0.5d0*lprim(ix^D, temp_)*(2.0d0+gamma**2/(1.0d0+gamma))
       ...
       if (do_NNBrem) then
          R_pair = 0.3333d0*const_brems * ( mass_fraction(ix1,&
             1)**2 + mass_fraction(ix1,2)**2 +(28.d0/3.d0)*mass_fraction(ix1,&
             1)*mass_fraction(ix1,2))*lprim(ix1, rho_)**2*lprim(ix1,&
              temp_)**4.5d0
          Q_pair = R_pair * lprim(ix1, temp_)*3.0d0
  
          Q_pair = Q_pair *4.0d0
          R_pair = R_pair *4.0d0
          
          Q_loc(ix1, 3) =  Q_loc(ix1,3) + Q_pair 
          R_loc(ix1, 3) =  R_loc(ix1,3) + R_pair 
        endif        
...
!######################################################################
  subroutine compute_diffusion(ixImin1,ixImax1, ixOmin1,ixOmax1)
...
!stop "start of diffusion" 
    do ix1 = ixOmin1,ixOmax1 
       ...
       !scattering
       scattering_kappa = lprim(ix1, rho_)*avo*0.25d0*sigma_0/me_mev**2
       kappa_tilde_nu_scat(ix1,1,1) = mass_fraction(ix1,1)*scattering_kappa
       kappa_tilde_nu_scat(ix1,1,2) = mass_fraction(ix1,2)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,1) = mass_fraction(ix1,1)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,2) = mass_fraction(ix1,2)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,1) = mass_fraction(ix1,1)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,2) = mass_fraction(ix1,2)*scattering_kappa
       
       scattering_kappa = lprim(ix1, rho_)*avo*0.0625d0*sigma_0/me_mev**2* &
          mass_fraction(ix1,5)*(1.0d0-mass_fraction(ix1,6)/mass_fraction(ix1,&
          5))**2 !only have 1 factor of A because kappa multiples the number fraction, not mass fractions
       kappa_tilde_nu_scat(ix1,1,3) = mass_fraction(ix1,4)*scattering_kappa
       kappa_tilde_nu_scat(ix1,2,3) = mass_fraction(ix1,4)*scattering_kappa
       kappa_tilde_nu_scat(ix1,3,3) = mass_fraction(ix1,4)*scattering_kappa

       eta_pn(ix1) = avo*lprim(ix1, rho_)*(mass_fraction(ix1,&
          1)-mass_fraction(ix1,2))/(exp(eta_hat(ix1))-1.0d0)
       eta_pn(ix1) = max(eta_pn(ix1),0.0d0)
       eta_np(ix1) = avo*lprim(ix1, rho_)*(mass_fraction(ix1,&
          2)-mass_fraction(ix1,1))/(exp(-eta_hat(ix1))-1.0d0)
       eta_np(ix1) = max(eta_np(ix1),0.0d0)
...
!       endif
       ...
       if (lprim(ix1, rho_).lt.1.0d11) then
          !non degenerate here, use mass fractions as chemical potentials fail at low densities
          eta_pn(ix1) = avo*lprim(ix1, rho_)*mass_fraction(ix1,2)
          eta_np(ix1) = avo*lprim(ix1, rho_)*mass_fraction(ix1,1)
       endif

       !absorption
!       abs_kappa = lrho(i)*avo*
       abs_kappa = (1.0d0+3.0d0*alpha**2)*0.25d0*sigma_0/me_mev**2
       block_factor = 1.0d0 + exp(eta_nucleons(ix1,1)-get_fermi_integral(5,&
          eta_nu(ix1,1))/ get_fermi_integral(4,eta_nu(ix1,1)))
       kappa_tilde_nu_abs(ix1,1,1) = eta_np(ix1)*abs_kappa/block_factor
       kappa_tilde_nu_abs(ix1,2,1) = 0.0d0 !no absorption of a-type on neutrons
       kappa_tilde_nu_abs(ix1,3,1) = 0.0d0 !no absorption of x-type neutrinos
       kappa_tilde_nu_abs(ix1,1,2) = 0.0d0 !no absorption of e-type on protons
       block_factor = 1.0d0 + exp(-eta_nucleons(ix1,1)-get_fermi_integral(5,&
          eta_nu(ix1,2))/ get_fermi_integral(4,eta_nu(ix1,2)))
       kappa_tilde_nu_abs(ix1,2,2) = eta_pn(ix1)*abs_kappa/block_factor
       kappa_tilde_nu_abs(ix1,3,2) = 0.0d0 !no absorption of x-type neutrinos
       kappa_tilde_nu_abs(ix1,1,3) = 0.0d0 !no absorption on nuclei
       kappa_tilde_nu_abs(ix1,2,3) = 0.0d0 !no absorption on nuclei
       kappa_tilde_nu_abs(ix1,3,3) = 0.0d0 !no absorption on nuclei

       !sum up opacities to get zeta_single (again, factoring out energy dependence)
       zeta_single(ix1,1) = kappa_tilde_nu_scat(ix1,1,&
          1) + kappa_tilde_nu_scat(ix1,1,2) + kappa_tilde_nu_scat(ix1,1,&
          3) + kappa_tilde_nu_abs(ix1,1,1) + kappa_tilde_nu_abs(ix1,1,&
          2) + kappa_tilde_nu_abs(ix1,1,3)

       zeta_single(ix1,2) = kappa_tilde_nu_scat(ix1,2,&
          1) + kappa_tilde_nu_scat(ix1,2,2) + kappa_tilde_nu_scat(ix1,2,&
          3) + kappa_tilde_nu_abs(ix1,2,1) + kappa_tilde_nu_abs(ix1,2,&
          2) + kappa_tilde_nu_abs(ix1,2,3)

       zeta_single(ix1,3) = kappa_tilde_nu_scat(ix1,3,&
          1) + kappa_tilde_nu_scat(ix1,3,2) + kappa_tilde_nu_scat(ix1,3,&
          3) + kappa_tilde_nu_abs(ix1,3,1) + kappa_tilde_nu_abs(ix1,3,&
          2) + kappa_tilde_nu_abs(ix1,3,3)
    enddo ! end loop for every points
...
!    do i=n1-ghosts1,ghosts1+1,-1
       ...
       chi_single(ix1,1) = chi_single(ix_next1,1)  + zeta_single(ix1,1)* dr
       chi_single(ix1,2) = chi_single(ix_next1,2)  + zeta_single(ix1,2)* dr
       chi_single(ix1,3) = chi_single(ix_next1,3)  + zeta_single(ix1,3)* dr
    enddo ! end loop for every points
    
    do ix1 = ixOmin1,ixOmax1 
       !now we can determine diffusion rates
       rate_const = 4.0d0*pi*clight*zeta_single(ix1,&
          1)/(hc_mevcm**3*6.0d0*chi_single(ix1,1)**2)
       R_diff(ix1,1) = rate_const*lprim(ix1, temp_)*get_fermi_integral(0,&
          eta_nu(ix1,1))
       Q_diff(ix1,1) = rate_const*lprim(ix1, temp_)**2*get_fermi_integral(1,&
          eta_nu(ix1,1))
       
       rate_const = 4.0d0*pi*clight*zeta_single(ix1,&
          2)/(hc_mevcm**3*6.0d0*chi_single(ix1,2)**2)
       R_diff(ix1,2) = rate_const*lprim(ix1, temp_)*get_fermi_integral(0,&
          eta_nu(ix1,2))
       Q_diff(ix1,2) = rate_const*lprim(ix1, temp_)**2*get_fermi_integral(1,&
          eta_nu(ix1,2))
       
       rate_const = 16.0d0*pi*clight*zeta_single(ix1,&
          3)/(hc_mevcm**3*6.0d0*chi_single(ix1,3)**2)
       R_diff(ix1,3) = rate_const*lprim(ix1, temp_)*get_fermi_integral(0,&
          eta_nu(ix1,3))
       Q_diff(ix1,3) = rate_const*lprim(ix1, temp_)**2*get_fermi_integral(1,&
          eta_nu(ix1,3))
  ...
  subroutine finding_array_size(n1)
   use mod_global_parameters
   implicit none
!   integer, intent(in)             :: ixI^L, ixO^L
!      ixG^LL --> ixI^L   ;   ixM^LL --> ixO^L
   integer :: nM_local1, iigrid, igrid, nM1
   integer :: nG_local1, nnG1
...
!        nG_local1 = SIZE(ps(igrid)%x(ixGlo1:ixGhi1,1))+nG_local1
 ...
 !       write(*,*) nM_local1 , "nM_local1"
    enddo
        call MPI_ALLREDUCE(nM_local1, nM1, 1, mpi_integer, MPI_SUM, icomm,&
            ierrmpi)


 !call MPI_ALLREDUCE(nG_local1, nnG1, 1, mpi_integer, !                           MPI_SUM, icomm, ierrmpi)
...
!stop "n1 value "
  ...
  subroutine initialize_arrays(n1)
    use mod_global_parameters
    implicit none
    ...
    allocate(lprim(n1,nprim)) 
    allocate(lleak_tau(n1,3)) 
    ...
    lx_map = 0.0d0
    allocate(lfac(n1)) 
    allocate(gamma_ij(n1,3,3)) 
    allocate(lcoolingsource(n1,ndir+2)) 
    ...
    allocate(eta_nucleons(n1,3)) 
    allocate(eta_hat(n1)) 
    allocate(eta_nu(n1,3)) 
    allocate(lepton_blocking(n1,2)) 

    allocate(mass_fraction(n1,6)) 
    allocate(eta_pn(n1)) 
    allocate(eta_np(n1)) 

    allocate(zeta_single(n1,3)) 
    allocate(chi_single(n1,3)) 

    allocate(lprim_local(n1, nprim))
    allocate(lleak_tau_local(n1, 3))
  ...
  subroutine mapping_to_single_core(n1)
    use mod_global_parameters
...
!   double precision, intent(in)    :: cons(ixI^S, 1:ncons)
   integer ::  iigrid, igrid, ix1, i,j, index_prim, flavor, idir
...
!  find the r_min 
   do iigrid=1,igridstail; igrid=igrids(iigrid);
        coor_local(1) = min(minval(ps(igrid)%x(ixMlo1:ixMhi1, r_)),&
            coor_local(1))
   enddo  
       call MPI_ALLREDUCE(coor_local(1), coor(1), 1, mpi_double_precision,&
           MPI_MIN, icomm, ierrmpi)
...
!             stop
             do index_prim = 1, nprim
                     lprim_local(i,index_prim) = ps(igrid)%prim(ix1,&
                         index_prim)     
             enddo           
...
!write(*,*) lprim_local(:,rho_)
       ...
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, rho_),&
           lprim(ghostzones1+1:n1-ghostzones1, rho_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)

!stop
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, ye_),&
           lprim(ghostzones1+1:n1-ghostzones1, ye_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, temp_),&
           lprim(ghostzones1+1:n1-ghostzones1, temp_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, cs2_),&
           lprim(ghostzones1+1:n1-ghostzones1, cs2_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, press_),&
           lprim(ghostzones1+1:n1-ghostzones1, press_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, eps_),&
           lprim(ghostzones1+1:n1-ghostzones1, eps_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, ent_),&
           lprim(ghostzones1+1:n1-ghostzones1, ent_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, alp_),&
           lprim(ghostzones1+1:n1-ghostzones1, alp_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1, psi_),&
           lprim(ghostzones1+1:n1-ghostzones1, psi_),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_MAX, icomm,&
           ierrmpi)

!!  take 
      do idir = 1, ndir
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1,&
           W_vel(idir)), lprim(ghostzones1+1:n1-ghostzones1, W_vel(idir)),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_SUM, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1,&
           beta(idir)), lprim(ghostzones1+1:n1-ghostzones1, beta(idir)),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_SUM, icomm,&
           ierrmpi)
       call MPI_ALLREDUCE(lprim_local(ghostzones1+1:n1-ghostzones1,&
           vecX(idir)), lprim(ghostzones1+1:n1-ghostzones1, vecX(idir)),&
           n1-ghostzones1-ghostzones1, mpi_double_precision, MPI_SUM, icomm,&
           ierrmpi)
      enddo
!!     enddo 
...
! cells
  do i = 1, ghostzones1
      do index_prim = 1, nprim
                     lprim(i,index_prim) = lprim(ghostzones1*2+1-i,&
                         index_prim)
      enddo
...
! cells
  ...
  do i = n1, n1-ghostzones1+1, -1
      do index_prim = 1, nprim
                     lprim(i,index_prim) = lprim(n1-ghostzones1*2+j,&
                         index_prim)     
      enddo
...
!stop "end of mapping"
  ...
  subroutine inverse_mapping(n1)
    use mod_global_parameters
   implicit none
   integer, intent(in) :: n1
   integer :: igrid, iigrid, i, ix1, idir
    double precision ::c  r_min, r_min_local
...
!######################################################################
  subroutine find_ruf_tau(ixImin1,ixImax1, ixOmin1,ixOmax1)

    use mod_global_parameters

    implicit none
    integer, intent(in)             :: ixImin1,ixImax1, ixOmin1,ixOmax1
    integer jm1,ix1, ix_next1
    integer icount
    integer,parameter :: icount_max = 2000
    ! **** EOS ****
    double precision, dimension(ixImin1:ixImax1) :: kappa_const_scat_n
    double precision, dimension(ixImin1:ixImax1) :: kappa_const_scat_p
!    double precision, dimension(ixI^S) :: kappa_const_scat_h
    double precision, dimension(ixImin1:ixImax1) :: kappa_const_abs
    double precision :: csn_0,csp_0,t1,t2
    double precision :: xerr
    double precision, parameter :: xerr_out = 1.0d-12
    double precision, dimension(ixImin1:ixImax1,3) :: kappa_tot   
    double precision, dimension(ixImin1:ixImax1,3) :: kappa_tot_p 
    double precision, dimension(ixImin1:ixImax1,3) :: kappa_scat_n ! 1/cm
    double precision, dimension(ixImin1:ixImax1,3) :: kappa_scat_p ! 1/cm
!    double precision, dimension(ixI^S) :: kappa_scat_h ! 1/cm
    double precision, dimension(ixImin1:ixImax1) :: kappa_abs_n  ! 1/cm
    double precision, dimension(ixImin1:ixImax1) :: kappa_abs_p  ! 1/cm
    double precision, dimension(ixImin1:ixImax1) :: local_eta_nue
    double precision, dimension(ixImin1:ixImax1) :: local_eta_nua
    double precision, dimension(ixImin1:ixImax1) :: local_eta_nux
    double precision, dimension(ixImin1:ixImax1) :: eta_nue_eq
...
!stop 
    ...
    do ix1 = ixOmin1,ixOmax1 
       ...
       t1 = sigma_0 * avo * lprim(ix1, rho_) * (lprim(ix1, temp_)/me_mev)**2
       kappa_const_scat_n(ix1) = csn_0 * t1
       kappa_const_scat_p(ix1) = csp_0 * t1
       ! (A11) constant part
       kappa_const_abs(ix1) = (1.0d0+3.0d0*alpha**2)/4.0d0 * t1
    enddo ! end loop for every points
    ...
    do while(xerr.gt.xerr_out .and. icount.lt.icount_max)
       ...
       ! contributions
       ! nu_e; (A17)
       kappa_tot(ixOmin1:ixOmax1,1) = kappa_scat_p(ixOmin1:ixOmax1,&
          1) + kappa_scat_n(ixOmin1:ixOmax1,1) + kappa_abs_n(ixOmin1:ixOmax1)
       ! antis; (A18)
       kappa_tot(ixOmin1:ixOmax1,2) = kappa_scat_p(ixOmin1:ixOmax1,&
          2) + kappa_scat_n(ixOmin1:ixOmax1,2) + kappa_abs_p(ixOmin1:ixOmax1)

       ! nu_xl (A19)
       kappa_tot(ixOmin1:ixOmax1,3) = + kappa_scat_p(ixOmin1:ixOmax1,&
          3) + kappa_scat_n(ixOmin1:ixOmax1,3)
...
!        write(*,*)  "pass right way"
         ...
         do ix1 = ixOmax1, ixOmin1, -1 
               dr = lxi(ix1+1,r_) - lxi(ix1,r_)
               ix_next1 = ix1 + kr(1,1)
               lleak_tau(ix1, 1:3) = lleak_tau(ix_next1,1:3) + kappa_tot(ix1,&
                  1:3) * dr * lprim(ix1, psi_)**2
         enddo ! end loop for every points
       endif
!        write(*,*) lleak_tau(5,:), "lleak_tau"
       ...
       do ix1 = ixOmin1,ixOmax1 
          local_eta_nux(ix1) = 0.0d0   ! (A2)
          ! (A5) equilibrium eta, we have rest masses in our chemical potentials
          ! no need to include mass difference in eta
          eta_nue_eq(ix1) = eta_nu(ix1,1)
          ! (A3); note that the ^0 etas are set to 0.0d0
          local_eta_nue(ix1) =  eta_nue_eq(ix1) * (1.0d0-exp(-lleak_tau(ix1,&
             1))) 
          ! (A4)
          local_eta_nua(ix1) = -eta_nue_eq(ix1) * (1.0d0-exp(-lleak_tau(ix1,&
             2))) 
         enddo ! end loop for every points
...
!        write(*,*)  local_eta_nue(5), local_eta_nua(5), "lcoal_eta_Nue, and nua"
       ...
       do ix1 = ixOmin1,ixOmax1 
          ...
          xlye = lprim(ix1, ye_)
          ! (A8)
          xyn = (1.0d0-xlye) / (1.0d0 + 2.0d0/3.0d0 * max(eta_nucleons(ix1,3),&
             0.0d0))
          xyp = xlye / (1.0d0 + 2.0d0/3.0d0*max(eta_nucleons(ix1,2),0.0d0))
          t1 = exp(-eta_hat(ix1))
          ! (A13)
          ...
          ! electron neutrinos
          t1 = get_fermi_integral(4+jm1,local_eta_nue(ix1)) / &
             get_fermi_integral(2+jm1,local_eta_nue(ix1))
          ! (A15)
          t2 = 1.0d0 + exp(eta_nucleons(ix1,1)-get_fermi_integral(5,&
             local_eta_nue(ix1)) / get_fermi_integral(4,local_eta_nue(ix1)))
          ! (A6)

          kappa_scat_n(ix1,1) = kappa_const_scat_n(ix1) * xyn  * t1
          kappa_scat_p(ix1,1) = kappa_const_scat_p(ix1) * xyp  * t1

          ! (A11)
          kappa_abs_n(ix1) = kappa_const_abs(ix1) * xynp * t1 / t2 

          ! anti-electron neutrinos
          t1 = get_fermi_integral(4+jm1,local_eta_nua(ix1)) / &
             get_fermi_integral(2+jm1,local_eta_nua(ix1))
          ! (A16)
          t2 = 1.0d0 + exp(-eta_nucleons(ix1,1)-get_fermi_integral(5,&
             local_eta_nua(ix1)) / get_fermi_integral(4,local_eta_nua(ix1)))
          ! (A6)
          kappa_scat_n(ix1,2) = kappa_const_scat_n(ix1) * xyn  * t1
          kappa_scat_p(ix1,2) = kappa_const_scat_p(ix1) * xyp  * t1

          ! (A12)
          kappa_abs_p(ix1) = kappa_const_abs(ix1) * xypn * t1 / t2 
          ! nux neutrinos
          t1 = get_fermi_integral(4+jm1,local_eta_nux(ix1)) / &
             get_fermi_integral(2+jm1,local_eta_nux(ix1))
          ! (A6)
          kappa_scat_n(ix1,3) = kappa_const_scat_n(ix1) * xyn * t1
          kappa_scat_p(ix1,3) = kappa_const_scat_p(ix1) * xyp * t1
...
!       write(*,*)  xynp*avo*lprim(5,rho_),  "eta_np"
       ...
       xerr = 0.0d0
       do ix1 = ixOmin1,ixOmax1 
          xerr = max(xerr,abs(kappa_tot(ix1,1)/kappa_tot_p(ix1,1)-1.0d0))
          xerr = max(xerr,abs(kappa_tot(ix1,2)/kappa_tot_p(ix1,2)-1.0d0))
          xerr = max(xerr,abs(kappa_tot(ix1,3)/kappa_tot_p(ix1,3)-1.0d0))
       enddo ! end loop for every points
...
!###################  i took equatorial  ns_lcoation  for 2D
  ...
  ns_location1(1:3) = ixOmin1

  do ix1 = ixOmin1,ixOmax1 
       !nu-spheres for heating          ns  assumes  shperical symmetric for all
       !                                dimensions  (ns_location_i, 1, 1)
       if (lleak_tau(ix1,1).gt.0.66666d0) then
          ns_location1(1) = ix1
       endif

       if (lleak_tau(ix1,2).gt.0.66666d0) then
          ns_location1(2) = ix1
       endif

       if (lleak_tau(ix1,3).gt.0.66666d0) then
          ns_location1(3) = ix1
       endif
  enddo 

    heat_erms(1) = lprim(ns_location1(1), temp_)*sqrt(get_fermi_integral(5,&
       local_eta_nue(ns_location1(1)))/get_fermi_integral(3,&
       local_eta_nue(ns_location1(1))))

    heat_erms(2) = lprim(ns_location1(2), temp_)*sqrt(get_fermi_integral(5,&
       local_eta_nua(ns_location1(2)))/get_fermi_integral(3,&
       local_eta_nua(ns_location1(2))))

    heat_erms(3) = lprim(ns_location1(3), temp_)*sqrt(get_fermi_integral(5,&
       0.0d0)/ get_fermi_integral(3,0.0d0))

    heat_em(1) = lprim(ns_location1(1), temp_)*get_fermi_integral(5,&
       local_eta_nue(ns_location1(1)))/get_fermi_integral(4,&
       local_eta_nue(ns_location1(1)))
    heat_em(2) = lprim(ns_location1(2), temp_)*get_fermi_integral(5,&
       local_eta_nua(ns_location1(2)))/get_fermi_integral(4,&
       local_eta_nua(ns_location1(2)))
    heat_em(3) = lprim(ns_location1(3), temp_)*get_fermi_integral(5,&
       0.0d0)/ get_fermi_integral(4,0.0d0) !not used

    !set degeneracy factors to interpolated values
    eta_nu(ixOmin1:ixOmax1,1) = local_eta_nue(ixOmin1:ixOmax1)
    eta_nu(ixOmin1:ixOmax1,2) = local_eta_nua(ixOmin1:ixOmax1)
    eta_nu(ixOmin1:ixOmax1,3) = local_eta_nux(ixOmin1:ixOmax1)
...
!######################################################################
  ...
  subroutine finding_intermediate_variables(ixImin1,ixImax1, ixOmin1,ixOmax1)
   ! use mod_global_parameters
    implicit none

    integer, intent(in) :: ixImin1,ixImax1, ixOmin1,ixOmax1
    double precision    :: W2v2(ixImin1:ixImax1)
    double precision    :: lfac2(ixImin1:ixImax1)    
    integer :: idir, ix1

     ! calculate lfac  (spherical coor)
         do ix1 = ixOmin1,ixOmax1 
           lfac(ix1) = 1.0d0
           gamma_ij(ix1, 1,1) = 1.0d0*lprim(ix1, psi_)**4
           gamma_ij(ix1, 2,2) = lx(ix1,r_)**2*lprim(ix1, psi_)**4
           gamma_ij(ix1, 3,3) = gamma_ij(ix1,2,2) 
       
               W2v2 = 0.0d0
            do idir = 1, ndir
               W2v2(ix1) = W2v2(ix1) + gamma_ij(ix1,idir,idir)*lprim(ix1,&
                   W_vel(idir))**2
            enddo
     
            lfac2(ix1) = W2v2(ix1) + 1
     
            lfac(ix1) = dsqrt( lfac2(ix1) )
         enddo ! end loop for every points
...
function get_fermi_integral(ifermi,eta)
  ...
  if (eta.gt.1.D-3) then  
     select case (ifermi)
     case (0)
        fermi_integral_analytical = log10(1.0d0+exp(eta))
     case (1)
        fermi_integral_analytical = (eta**2/2.0D0 + &
           1.6449d0)/(1.0D0+EXP(-1.6855d0*eta))
     case (2)
        fermi_integral_analytical = (eta**3/3.0D0 + &
           3.2899d0*eta)/(1.0D0-EXP(-1.8246d0*eta))
     case (3)
        fermi_integral_analytical = (eta**4/4.0D0 + 4.9348d0*eta**2+11.3644d0) &
           / (1.0D0+EXP(-1.9039d0*eta))        
     case (4)
        fermi_integral_analytical = (eta**5/5.0D0 + &
           6.5797d0*eta**3+45.4576d0*eta) / (1.0D0-EXP(-1.9484d0*eta))        
     case (5)
        fermi_integral_analytical = (eta**6/6.0D0 + 8.2247d0*eta**4 + &
           113.6439d0*eta**2 + 236.5323d0)/(1.0D0+EXP(-1.9727d0*eta))
     end select
     ...
     case (0)
        fermi_integral_analytical = log10(1.0d0+exp(eta))
     case (1)
        fermi_integral_analytical = EXP(eta)/(1.0D0+&
           0.2159d0*EXP(0.8857d0*eta))
     case (2)
        fermi_integral_analytical = 2.0D0*EXP(eta)/(1.0D0+&
           0.1092d0*EXP(0.8908d0*eta))
     case (3)
        fermi_integral_analytical = 6.0D0*EXP(eta)/(1.0D0+&
           0.0559d0*EXP(0.9069d0*eta))
     case (4)
        fermi_integral_analytical = 24.0D0*EXP(eta)/(1.0D0+&
           0.0287d0*EXP(0.9257d0*eta))
     case (5)
        fermi_integral_analytical = 120.0D0*EXP(eta) / (1.0D0 + &
           0.0147d0*EXP(0.9431d0*eta))
     end select
