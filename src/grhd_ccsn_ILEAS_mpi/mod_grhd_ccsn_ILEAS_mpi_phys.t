...
module mod_grhd_ccsn_ILEAS_phys
  use mod_physics
  use mod_grhd_ccsn_ILEAS_phys_parameters

  implicit none
  private

  ! Public methods
  public :: grhd_ccsn_ILEAS_phys_init

contains

  !> Read this module's parameters from a file
  subroutine grhd_ccsn_ILEAS_read_params(files)
    use mod_global_parameters
    character(len=*), intent(in) :: files(:)
    integer                      :: n

    namelist /grhd_ccsn_ILEAS_list/ evolve_hydro, {^NOONED oneDcore, r_core, } &
                           use_GR, tolerance, iter_max, lfac_max

    do n = 1, size(files)
       rewind(unitpar)
       open(unitpar, file=trim(files(n)), status="old")
       read(unitpar, grhd_ccsn_ILEAS_list, end=111)
111    close(unitpar)
    end do

  end subroutine grhd_ccsn_ILEAS_read_params

  !> Write this module's parameters to a snapsoht
  subroutine grhd_ccsn_ILEAS_write_info(fh)
    use mod_global_parameters
    ...
    names(1) = "gamma"
    values(1) = 1.0d0!grhd_ccsn_ILEAS_gamma
    call MPI_FILE_WRITE(fh, values, n_par, MPI_DOUBLE_PRECISION, st, er)
    call MPI_FILE_WRITE(fh, names, n_par * name_len, MPI_CHARACTER, st, er)
  end subroutine grhd_ccsn_ILEAS_write_info


  !> Initialize the module
  subroutine grhd_ccsn_ILEAS_phys_init()
    use mod_global_parameters
    use mod_grhd_ccsn_ILEAS_phys_convert
    use mod_grhd_ccsn_ILEAS_phys_flux
    use mod_grhd_ccsn_ILEAS_phys_add_source
    use mod_grhd_ccsn_ILEAS_phys_one_d_core

    integer :: itr, idir, i

    call grhd_ccsn_ILEAS_read_params(par_files)

    physics_type = "grhd_ccsn_ILEAS"
...
!    allocate(leak_tau(1:3))
!    do i = 1,3
!    leak_tau(i) = var_set_consvar('leak_tau', var_type = hydro_var)
!    enddo
...
!  idir is mom(idir), idir+1 is tau, idir+2 is ye _con
    ...
    phys_get_dt              => grhd_ccsn_ILEAS_get_dt
    phys_get_lfac2           => grhd_ccsn_ILEAS_get_lfac2
    phys_get_tilde_S         => grhd_ccsn_ILEAS_get_tilde_S
    phys_get_csound2         => grhd_ccsn_ILEAS_get_csound2
    phys_get_cmax            => grhd_ccsn_ILEAS_get_cmax
    phys_update_eos          => grhd_ccsn_ILEAS_update_eos
    phys_check_params        => grhd_ccsn_ILEAS_check_params
    phys_check_prim          => grhd_ccsn_ILEAS_check_prim
    phys_write_info          => grhd_ccsn_ILEAS_write_info
    phys_handle_small_values => grhd_ccsn_ILEAS_handle_small_values

    call grhd_ccsn_ILEAS_phys_convert_init()
    call grhd_ccsn_ILEAS_phys_flux_init()
    call grhd_ccsn_ILEAS_phys_add_source_init()
    {^NOONED call grhd_ccsn_ILEAS_phys_one_d_core_init() }
    ...
    if ( .not.evolve_hydro ) then
       ...
       flux_type(:, leak_tau(1:3)) = flux_nul
  ...
  end subroutine grhd_ccsn_ILEAS_phys_init

  subroutine grhd_ccsn_ILEAS_check_params
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_ILEAS_check_params

  !> Update the eos (prim) variables, p, temp, entropy, cs2
  subroutine grhd_ccsn_ILEAS_update_eos(ixI^L, ixO^L, prim)
    use mod_global_parameters
    use mod_eos
    ...
    {do ix^D = ixO^LIM^D \}
       call grhd_ccsn_ILEAS_update_eos_one_point(prim(ix^D,:))
    {enddo^D&\}
  end subroutine grhd_ccsn_ILEAS_update_eos

  !> Calculate cmax_idim within ixO^L
  subroutine grhd_ccsn_ILEAS_get_cmax(prim, x, ixI^L, ixO^L, idim, cmax)
    use mod_global_parameters
    ...
    call grhd_ccsn_ILEAS_get_lambda(ixI^L, ixO^L, idim, prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), lambda(ixI^S,1:2))
  ...
  end subroutine grhd_ccsn_ILEAS_get_cmax

  subroutine grhd_ccsn_ILEAS_get_csound2(ixI^L,ixO^L,prim,csound2)
    use mod_global_parameters
    use mod_eos
    integer, intent(in)             :: ixI^L, ixO^L
    ...
    {enddo^D&\}
  end subroutine grhd_ccsn_ILEAS_get_csound2

  subroutine grhd_ccsn_ILEAS_get_lfac2(ps_in,ixI^L,ixO^L,lfac2)
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_ILEAS_get_lfac2

  ! get psi^6 (\gamma_{ij}S^{ij}), which is part of the source terms in cfc_alp
  subroutine grhd_ccsn_ILEAS_get_tilde_S(ps_in,ixI^L,ixO^L,tilde_S)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    type(state), intent(in)         :: ps_in
    ...
    associate(prim=>ps_in%prim,x=>ps_in%x)
    call grhd_ccsn_ILEAS_get_intermediate_variables(ixI^L, ixO^L, prim(ixI^S, 1:nprim), x(ixI^S, 1:ndim), &
  ...
  end subroutine grhd_ccsn_ILEAS_get_tilde_S

  subroutine grhd_ccsn_ILEAS_get_dt(w, ixI^L, ixO^L, dtnew, dx^D, x)
    use mod_global_parameters
  ...
  end subroutine grhd_ccsn_ILEAS_get_dt

  !> Returns 0 in argument flag where values are ok
  subroutine grhd_ccsn_ILEAS_check_prim(ixI^L, ixO^L, prim, flag)
    use mod_global_parameters
    integer, intent(in)          :: ixI^L, ixO^L
  ...
  end subroutine grhd_ccsn_ILEAS_check_prim
end module mod_grhd_ccsn_ILEAS_phys
