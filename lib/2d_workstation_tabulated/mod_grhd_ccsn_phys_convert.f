module mod_grhd_ccsn_phys_convert
  use mod_physics
  ...
  ! Public methods
  public :: grhd_ccsn_phys_convert_init

contains

  !> Initialize the module
  subroutine grhd_ccsn_phys_convert_init()
    ! initialize the k_max and v_max
    v_max = dsqrt(1.0d0 - 1.0d0 / lfac_max**2)
    k_max = 2.0d0 * v_max / (1.0d0 + v_max**2) ! k_max < 1
    phys_to_conserved        => grhd_ccsn_to_conserved
    phys_to_primitive        => grhd_ccsn_to_primitive
  end subroutine grhd_ccsn_phys_convert_init

  !> Transform primitive variables into conservative ones
  subroutine grhd_ccsn_to_conserved(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    where ( cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, D_) < small_D )
       cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, D_) = small_D
    end where

    ! Conserved electron frac ye_con_
    cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, ye_con_) =  prim(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2, ye_) * cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, D_) 

    ! Convert velocity to covariant momentum
    do idir = 1, ndir
       cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, mom(idir)) = cons(ixOmin1:ixOmax1,&
          ixOmin2:ixOmax2, D_) * h(ixOmin1:ixOmax1,&
    ...
    cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, tau_) = cons(ixOmin1:ixOmax1,&
       ...
       ixOmin2:ixOmax2) - prim(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
        press_) - cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, D_) 
    ...
    cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, ye_con_) = cons(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2, ye_con_) * psi6(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
    do idir = 1, ndir
       cons(ixOmin1:ixOmax1,ixOmin2:ixOmax2, mom(idir)) = cons(ixOmin1:ixOmax1,&
          ixOmin2:ixOmax2, mom(idir)) * psi6(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
    end do
  end subroutine grhd_ccsn_to_conserved

  !> Transform conservative variables into primitive ones 
  subroutine grhd_ccsn_to_primitive(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
     ixOmin2,ixOmax1,ixOmax2, cons, prim, x)
    use mod_global_parameters
    use mod_geometry
    ...
    double precision                :: D, S, tau, ye_con
    ...
    double precision                :: rho_bar, eps_bar, ye_bar, temp
    ...
    cons_tmp(ixOmin1:ixOmax1,ixOmin2:ixOmax2, tau_) = cons(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2, tau_) / psi6(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
    cons_tmp(ixOmin1:ixOmax1,ixOmin2:ixOmax2, ye_con_) = cons(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2, ye_con_) / psi6(ixOmin1:ixOmax1,ixOmin2:ixOmax2)
    do idir = 1, ndir
    ...
    do ix2 = ixOmin2,ixOmax2 
      adjustment = .False.
! only ye no need rootfinding
       prim(ix1,ix2, ye_) = cons_tmp(ix1,ix2, ye_con_)/cons_tmp(ix1,ix2, D_)
         if (prim(ix1,ix2, ye_) .lt. eos_yemin) then
         if (mype == 0) then
         write(*,*) "con2prim   ye < yemin"
         endif
         prim(ix1,ix2, ye_) = eos_yemin
         endif

         if (prim(ix1,ix2, ye_) .gt. eos_yemax) then
         if (mype == 0) then
         write(*,*) "con2prim   ye < yemax"
         endif
         prim(ix1,ix2, ye_) = eos_yemax
         endif
      ...
      if ( (cons_tmp(ix1,ix2, D_) <= small_D) .or. (cons_tmp(ix1,ix2,&
         ...
         lfac = 1.0d0
         ui_bar  = 0.0d0
!         rho_bar = max(small_rho, eos_rhomin)
         rho_bar = small_rho
!         eps_bar = max(small_eps, eos_epsmin)
         eps_bar = small_eps
         adjustment = .True. ! adjust all conserved variables
      else
         temp = prim(ix1,ix2, temp_)
         ...
         ye_con = cons_tmp(ix1,ix2, ye_con_)  
         ...
         if (dabs(k) <= smalldouble) then
            ...
            if (k > k_max) then 
               ...
               cons_tmp(ix1,ix2, mom(:)) = cons_tmp(ix1,ix2,&
                   mom(:)) * rescale_factor
               cons(ix1,ix2, mom(:)) = cons(ix1,ix2, mom(:)) * rescale_factor
            endif
...
!            eps_bar = max(small_eps, eos_epsmin)
         ...
         enddo
         call grhd_ccsn_update_eos_one_point(prim(ix1,ix2, 1:nprim))       
       
         ! if any adjustments, recompute conserved varables consistently
         if ( adjustment ) then
            a_bar = prim(ix1,ix2, press_) / (prim(ix1,ix2,&
                rho_)*(1.0d0+prim(ix1,ix2, eps_)))
            a_bar = max( min( 1.0d0, a_bar ) , 0.0d0) !strictly require a is physical
            h_bar = (1.0d0+prim(ix1,ix2, eps_))*(1.0d0+a_bar)
            ! Conserved density D
            cons(ix1,ix2, D_) = lfac * prim(ix1,ix2, rho_) 
            ! Conserved ele fraction
            cons(ix1,ix2, ye_con_) = cons(ix1,ix2, D_) * prim(ix1,ix2, ye_) 
            ! Convert velocity to covariant momentum
            do idir = 1, ndir
            ...
            ! Conserved energy - D = tau
            cons(ix1,ix2, tau_) = cons(ix1,ix2, D_) * lfac * h_bar - prim(ix1,&
               ix2, press_) - cons(ix1,ix2, D_) 
            ! conformal transformation
            cons(ix1,ix2, D_) = cons(ix1,ix2, D_) * psi6(ix1,ix2)
            cons(ix1,ix2, ye_con_) = cons(ix1,ix2, ye_con_) * psi6(ix1,ix2)
    ...
    contains
       subroutine get_vars(z, W_out, rho_out, eps_out, h_out)
          ...
          ye_bar = ye_con / D
          ye_bar = max( min( eos_yemax, ye_bar), eos_yemin)
          
          temp_bar = temp
!          if (present(ye_in)) ye_bar = ye_in / D
  ...
  end subroutine grhd_ccsn_to_primitive
  
end module mod_grhd_ccsn_phys_convert
