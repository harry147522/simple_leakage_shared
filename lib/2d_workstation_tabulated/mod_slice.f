            ...
            do jgrid=1,Morton_sub_stop(ipe)-Morton_sub_start(ipe)+1
               ...
               if (ipe == mype ) then 
                  call MPI_SEND(ps_sub(jgrid)%x,1,type_subblock_x_io,0,itag,&
                     icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%prim,1,type_subblock_io,0,itag+1,&
                     icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%xC,1,type_subblockC_x_io,0,&
                     itag+2,icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%primC,1,type_subblockC_io,0,&
                     itag+3,icomm,ierrmpi)
                  call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,&
                     0,itag+4,icomm,ierrmpi)
               end if
               if (mype == 0) then
                  call MPI_RECV(ps_sub(Njgrid+1)%x,1,type_subblock_x_io,ipe,&
                     itag,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%prim,1,type_subblock_io,ipe,&
                     itag+1,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%xC,1,type_subblockC_x_io,ipe,&
                     itag+2,icomm,status,ierrmpi)
                  call MPI_RECV(ps_sub(Njgrid+1)%primC,1,type_subblockC_io,ipe,&
                     itag+3,icomm,status,ierrmpi)
                  call MPI_RECV(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,&
      ...
      case('vtuCC') ! celldata
         ...
         do iw=1,nprim+nwauxio
            ...
            write(slice_fh,'(200(1pe14.6))') &
               (roundoff_minmax(ps_sub(jgrid)%prim(ix1,iw)*normconv(iw),&
               minvalue,maxvalue),ix1=ixCCmin1,ixCCmax1)
            write(slice_fh,'(a)')'</DataArray>'
         enddo
         write(slice_fh,'(a)')'</CellData>'
      ...
      case('vtu') ! pointdata
         ...
         do iw=1,nprim+nwauxio
            ...
            write(slice_fh,'(a,a,a)')'<DataArray type="Float64" Name="',&
               TRIM(wnamei(iw)),'" format="ascii">'
            write(slice_fh,'(200(1pe14.6))') &
               (roundoff_minmax(ps_sub(jgrid)%primC(ix1,iw)*normconv(iw),&
               minvalue,maxvalue),ix1=ixCmin1,ixCmax1)
            write(slice_fh,'(a)')'</DataArray>'
         enddo
         write(slice_fh,'(a)')'</PointData>'
 ...
 !write cell corner coordinates in a backward dimensional loop, always 3D output
         ...
         do ipe=1,npe-1
            do jgrid=1,Morton_sub_stop(ipe)-Morton_sub_start(ipe)+1
               ...
               if (ipe == mype ) then 
                  call MPI_SEND(ps_sub(jgrid)%x,1,type_subblock_x_io,0,itag,&
                     icomm,ierrmpi)
                  call MPI_SEND(ps_sub(jgrid)%prim,1,type_subblock_io,0,itag,&
                     icomm,ierrmpi)
                  call MPI_SEND(normconv,nprim+nwauxio+1,MPI_DOUBLE_PRECISION,&
                     0,itag,icomm,ierrmpi)
               end if
               if (mype == 0) then
                  call MPI_RECV(ps_sub(Njgrid+1)%x,1,type_subblock_x_io,ipe,&
                     itag,icomm,status,ierrmpi)
      ...
      double precision, parameter :: minvalue = 1.0d-99, maxvalue = 1.0d+99
            ...
            do ix1=ixsubMlo(1),ixsubMhi(1)
               ...
               do iw = 1,nprim+nwauxio-1
                  ...
                  write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,&
               ...
               write(data,"(es14.6)")roundoff_minmax(ps_sub(jout)%prim(ix1,&
                  nprim+nwauxio)*normconv(nprim+nwauxio),minvalue,maxvalue)
    ...
    subroutine put_slice_dat
      ...
      do jgrid=1,Njgrid
         ...
         call MPI_FILE_IWRITE_AT(slice_fh,offset,ps_sub(jgrid)%prim,1,&
...
!         call MPI_FILE_WRITE(slice_fh,neqpar+nspecialpar,1,MPI_INTEGER,status,ierrmpi)
  ...
  subroutine fill_subnode(igrid,active,jgrid,dir,xslice,normconv)
    ...
    select case (dir)
    case (1)
       ps_sub(jgrid)%prim(ixCCmin2:ixCCmax2,&
          1:nprim+nwexpand) = wCC_TMP(ixslice,ixCCmin2:ixCCmax2,&
          1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin2:ixCCmax2,1:ndim) = xCC_TMP(ixslice,&
          ixCCmin2:ixCCmax2,1:ndim)
       ps_sub(jgrid)%primC(ixCmin2:ixCmax2,1:nprim+nwexpand) = wC_TMP(ixslice,&
          ixCmin2:ixCmax2,1:nprim+nwexpand)
       ps_sub(jgrid)%xC(ixCmin2:ixCmax2,1:ndim) = xC_TMP(ixslice,&
          ixCmin2:ixCmax2,1:ndim)
    case (2)
       ps_sub(jgrid)%prim(ixCCmin1:ixCCmax1,&
          1:nprim+nwexpand) = wCC_TMP(ixCCmin1:ixCCmax1,ixslice,&
          1:nprim+nwexpand)
       ps_sub(jgrid)%x(ixCCmin1:ixCCmax1,1:ndim) = xCC_TMP(ixCCmin1:ixCCmax1,&
          ixslice,1:ndim)
       ps_sub(jgrid)%primC(ixCmin1:ixCmax1,&
          1:nprim+nwexpand) = wC_TMP(ixCmin1:ixCmax1,ixslice,1:nprim+nwexpand)
  ...
  subroutine dealloc_subnode(jgrid)
    ...
    deallocate(ps_sub(jgrid)%prim,ps_sub(jgrid)%x,ps_sub(jgrid)%primC,&
       ps_sub(jgrid)%xC)
